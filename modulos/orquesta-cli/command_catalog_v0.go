...
var cliCommandCatalogV0 = []cliCommandCatalogEntryV0{
	...
	{[]string{"autoprogramacion", "goal", "observar"}, []string{"--server-url URL", "--run-ref RUN_REF", "--json"}, []string{"--server-url URL", "--run-ref RUN_REF", "--json"}, "Autoprogramming observe goal REST", CliCommandEffectWriteV0, "observa Goal goal-first sin loop legacy", "observes goal-first Goal without legacy loop", CliCommandStateActiveV0, wrapCLIHandlerNoRunnerV0(runCLIAutoprogrammingObserveGoalV0)},
	{[]string{"autoprogramacion", "supervisar"}, []string{"--server-url URL", "--run-ref RUN_REF", "--director-execution-mode legacy_director_loop", "--max-ticks 1", "--json"}, []string{"--server-url URL", "--run-ref RUN_REF", "--director-execution-mode legacy_director_loop", "--max-ticks 1", "--json"}, "Run supervisor REST", CliCommandEffectWriteV0, "supervisa ticks acotados legacy", "supervises bounded legacy ticks", CliCommandStateActiveV0, wrapCLIHandlerNoRunnerV0(runCLIAutoprogrammingSuperviseV0)},
