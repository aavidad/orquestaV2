...
Evidencia esperada: politica valida tiempos y acciones; rechaza runtime/provider/model/HOME/OAuth/PID/DB.
Estado: validada para LSE-001 el 2026-05-06
```
...
Tipo: unit | contract
Comando: go test -count=1 ./modulos/orquesta-core-leases
Evidencia esperada: `AgentLeaseExpiredV0` se deriva solo de decisiones no-continue; conserva refs compactas, `observed_at` externo y `recommended_action`; stopped/failed generan senal terminal sin proceso real; la traduccion es idempotente, clona evidencia y rechaza DB/runtime/provider/HOME/OAuth/modelo/secretos.
Estado: revalidada para cierre de promocion candidata el 2026-05-06
```
