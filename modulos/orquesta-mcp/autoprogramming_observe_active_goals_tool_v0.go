...
func MCPAutoprogrammingObserveActiveGoalsDescriptorV0() MCPAutoprogrammingObserveActiveGoalsToolDescriptorV0 {
	return MCPAutoprogrammingObserveActiveGoalsToolDescriptorV0{
		...
		Output:      "ok:{operation_ref?,observations?[]{run_ref,goal_ref?,goal_status?,context_budget_total_bytes?,static_prompt_bytes?,dynamic_context_bytes?,code_context_cache_status?,recommended_action?,evidence_refs?},issues?,next_actions?,diagnostics?[]{code,scope?,message?,evidence_refs?},evidence_refs?}|error:{errores_publicos,operation_ref?,next_actions?,diagnostics?[]{code,scope?,message?,evidence_refs?},evidence_refs?}",
