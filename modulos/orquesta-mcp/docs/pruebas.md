...
`submit_artifact` solo por `DomainWorkArtifactSubmitterPortV0`, evalua
...
`active_work_count`, `active_works` tipados con refs/evidencias y
`goal_actions` con `action_taken`/evidencias cuando existan. Falla de forma
...
Tipo: contract
Comando: go test -count=1 ./modulos/orquesta-mcp ./modulos/orquesta-app-runner ./modulos/orquesta-app-planner
Evidencia esperada: `orquesta.apps.ejecutar_orquestacion.v0` conserva compatibilidad legacy por puertos inyectados solo cuando el caller envia `director_execution_mode=legacy_director_loop`, declara `evidence_refs` compactas en la salida `ok`; con modo vacio o `goal_first` devuelve `legacy_director_loop_required` y `preferred_entrypoint=orquesta.apps.arrancar_director.v0` sin ejecutar el loop.
...
`goal_spec_summaries` tipada con refs, hash, tipos y contadores publicos; sin
executor devuelve `mcp_transport_tool_unbound`, con executor inyectado invoca el
puerto fake sin filtrar specs completas, y el bridge HTTP
...
`run_status`, `director_execution_mode`, `recommended_action`,
`closure_status`, refs de
...
compacta de goal/cierre que la ruta de app-director, incluyendo
`recommended_action` cuando el estado requiere esperar, reparar, replanificar o
continuar fase. Sin executor devuelve `mcp_transport_tool_unbound` por
transporte.
