...
const (
	...
	mcpAutoprogrammingActionTerminalArtifactMissingV0                  = MCPGoalFirstTerminalArtifactMissingAfterCompleteV0
	...
	mcpAutoprogrammingEvidenceTerminalArtifactMissingV0                = "evidence-ref-autoprogramming-status-terminal-artifact-missing-after-complete"
...
func mcpAutoprogrammingRepairReceiptRequiresReworkActionsV0(
	observedByRunRef map[string]*MCPDirectorStatsToolResultV0,
) []MCPAutoprogrammingActionableRunV0 {
	out := make([]MCPAutoprogrammingActionableRunV0, 0)
	for runRef, observed := range observedByRunRef {
		if !mcpAutoprogrammingObservedRepairReceiptRequiresReworkV0(observed) {
			continue
		}
		action := MCPAutoprogrammingActionableRunV0{
			...
			RunRef:            strings.TrimSpace(runRef),
			RunStatus:         mcpAutoprogrammingObservedRunStatusV0(observedByRunRef, runRef),
			Reason:            "repair_receipt_requires_rework: receipt repair was attempted but still requires causal rework",
			RecommendedAction: "replan",
			EvidenceRefs: []string{
				mcpAutoprogrammingEvidenceRepairReceiptRequiresReworkV0,
			},
		}
		action = mcpAutoprogrammingActionableRunWithObservedStatsV0(action, observed)
		action = mcpAutoprogrammingActionableRunWithObservedGoalV0(action, observed)
...
func mcpAutoprogrammingTerminalArtifactMissingActionsV0(
	observedByRunRef map[string]*MCPDirectorStatsToolResultV0,
) []MCPAutoprogrammingActionableRunV0 {
	out := make([]MCPAutoprogrammingActionableRunV0, 0)
	for runRef, observed := range observedByRunRef {
		if !mcpAutoprogrammingObservedTerminalArtifactMissingV0(observed) {
			continue
		}
		action := MCPAutoprogrammingActionableRunV0{
			Code:              mcpAutoprogrammingActionTerminalArtifactMissingV0,
			Severity:          "blocked",
			RunRef:            strings.TrimSpace(runRef),
			RunStatus:         mcpAutoprogrammingObservedRunStatusV0(observedByRunRef, runRef),
			Reason:            "terminal_artifact_missing_after_goal_complete: terminal goal state declares artifact paths that are missing from the write_set",
			RecommendedAction: "replan",
			EvidenceRefs: []string{
				mcpAutoprogrammingEvidenceTerminalArtifactMissingV0,
			},
		}
		action = mcpAutoprogrammingActionableRunWithObservedStatsV0(action, observed)
		action = mcpAutoprogrammingActionableRunWithObservedGoalV0(action, observed)
...
func mcpAutoprogrammingObservedRepairReceiptRequiresReworkV0(
	observed *MCPDirectorStatsToolResultV0,
) bool {
	if observed == nil {
		return false
	}
	if observed.Goal != nil && containsStringMCPV0(observed.Goal.IssueCodes, MCPGoalFirstRepairReceiptRequiresReworkV0) {
		return true
	}
	if observed.Stats == nil {
		return false
	...
	for _, issue := range observed.Stats.Progress.Issues {
		if strings.TrimSpace(issue.Code) == MCPGoalFirstRepairReceiptRequiresReworkV0 {
			return true
		}
	}
	return false
...
func mcpAutoprogrammingObservedTerminalArtifactMissingV0(
	observed *MCPDirectorStatsToolResultV0,
) bool {
	if observed == nil {
		return false
	}
	if observed.Goal != nil && containsStringMCPV0(observed.Goal.IssueCodes, MCPGoalFirstTerminalArtifactMissingAfterCompleteV0) {
		return true
	}
	if observed.Stats == nil {
		return false
	...
	for _, issue := range observed.Stats.Progress.Issues {
		if strings.TrimSpace(issue.Code) == MCPGoalFirstTerminalArtifactMissingAfterCompleteV0 {
			return true
		}
	}
	return false
...
func mcpAutoprogrammingGoalFirstBlockedActionsV0(
	...
	for _, state := range states {
		...
		checkpointOnlyConsumptionWarning := !activeTimeoutBackendActive &&
			mcpAutoprogrammingCheckpointOnlyConsumptionWarningV0(action, observed, policy)
		...
		if checkpointOnlyConsumptionWarning && !checkpointOnlyHighConsumption {
			action.Code = mcpAutoprogrammingActionCheckpointOnlyConsumptionWarningV0
			action.Severity = "warning"
			action.Reason = "checkpoint_only_consumption_warning: backend goal is active with rising token usage after checkpoint but no next artifact or domain receipt yet; require the next artifact soon before high-consumption replan"
			action.RecommendedAction = "observe_goal_backend_require_next_artifact"
			action.EvidenceRefs = compactStringsMCPV0(append(
				action.EvidenceRefs,
				mcpAutoprogrammingEvidenceCheckpointOnlyConsumptionWarningV0,
			))
		}
...
func mcpAutoprogrammingCheckpointOnlyConsumptionWarningV0(
	action MCPAutoprogrammingActionableRunV0,
	observed *MCPDirectorStatsToolResultV0,
	policy MCPAutoprogrammingGoalProgressPolicyV0,
) bool {
