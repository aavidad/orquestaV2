...
func TestMCPRunControlExecutorV0StopForcedConfirmaBackendBlockedV0(t *testing.T) {
	runRef := "run-ref-run-control-goal-blocked-001"
	goalRef := "goal-ref-run-control-goal-blocked-001"
	port := &fakeMCPRunControlPortV0{
		readState:  runControlStateForMCPTestV0(runRef, orquestaruncontrol.RunControlStatusRunningV0, false),
		stopStatus: orquestaruncontrol.RunControlStatusStoppedV0,
	}
	goalBackend := &fakeMCPRunControlGoalBackendStateV0{
		results: []MCPDirectorStatsToolResultV0{
			mcpRunControlGoalBackendStatsForTestV0(runRef, goalRef, "active"),
			mcpRunControlGoalBackendStatsForTestV0(runRef, goalRef, orquestagoal.GoalStatusBlockedV0),
		},
	}
	executor := MCPRunControlToolExecutorV0{
		Port:             port,
	...
	result, err := executor.Execute(context.Background(), MCPRunControlToolInputV0{
		RequestID: "req-run-control-goal-blocked-001",
		Action:    "stop",
		RunRef:    runRef,
		Forced:    true,
	})
	...
	if result.Estado != MCPRunControlEstadoOKV0 ||
		...
		result.GoalStatusBefore != "active" ||
		result.GoalStatusAfter != orquestagoal.GoalStatusBlockedV0 ||
		!result.GoalControlSignalConfirmed ||
...
func TestMCPRunControlExecutorV0StopForcedConfirmaBackendProviderLimitedV0(t *testing.T) {
	runRef := "run-ref-run-control-goal-provider-limited-001"
	goalRef := "goal-ref-run-control-goal-provider-limited-001"
	port := &fakeMCPRunControlPortV0{
		readState:  runControlStateForMCPTestV0(runRef, orquestaruncontrol.RunControlStatusRunningV0, false),
		stopStatus: orquestaruncontrol.RunControlStatusStoppedV0,
	}
	goalBackend := &fakeMCPRunControlGoalBackendStateV0{
		results: []MCPDirectorStatsToolResultV0{
			mcpRunControlGoalBackendStatsForTestV0(runRef, goalRef, "active"),
			mcpRunControlGoalBackendStatsForTestV0(runRef, goalRef, "providerLimited"),
		},
	}
	executor := MCPRunControlToolExecutorV0{
		Port:             port,
	...
	result, err := executor.Execute(context.Background(), MCPRunControlToolInputV0{
		RequestID: "req-run-control-goal-provider-limited-001",
		Action:    "stop",
		RunRef:    runRef,
		Forced:    true,
	})
	...
	if result.Estado != MCPRunControlEstadoOKV0 ||
		result.Status != string(orquestaruncontrol.RunControlStatusStoppedV0) ||
		result.GoalStatusAfter != "providerLimited" ||
		!result.GoalControlSignalConfirmed ||
		result.RecommendedAction != "" ||
		len(result.Diagnostics) != 0 ||
		len(result.Errores) != 0 {
...
func TestMCPRunControlExecutorV0StopReconcilesExternalCleanupConEvidenciaSinForce(t *testing.T) {
	...
	if strings.Contains(port.complete.Reason, "forced") ||
		...
		t.Fatalf("complete terminal degrada cleanup externo a forced: %+v", port.complete)
	}
	state, err := goalStates.LoadGoalWorkStateV0(context.Background(), runRef)
	if err != nil {
		t.Fatalf("LoadGoalWorkStateV0: %v", err)
...
func TestMCPRunControlExecutorV0StopReconcilesExternalCleanupConGoalStatsRunningStaleSinProcesoV0(t *testing.T) {
	runRef := "run-ref-run-control-goal-backend-stale-no-process-stop-001"
	goalRef := "goal-ref-run-control-goal-backend-stale-no-process-stop-001"
	externalGoalRef := "thread-ref-" + goalRef
	goalStates := &mcpGoalStateStoreForTestV0{states: map[string]orquestagoal.GoalWorkStateV0{}}
	if err := goalStates.SaveGoalWorkStateV0(context.Background(), orquestagoal.GoalWorkStateV0{
		RunRef:          runRef,
		...
		Spec: orquestagoal.GoalWorkSpecV0{
			RunRef:       runRef,
			GoalRef:      goalRef,
			Objective:    "Reconciliar cleanup externo aunque goal stats conserve running stale sin proceso.",
			DirectorKind: orquestagoal.GoalDirectorKindCodexGoalV0,
			WriteSet:     []orquestagoal.GoalWriteScopeV0{{Path: "docs"}},
		},
		LaunchReceipt: orquestagoal.GoalLaunchReceiptV0{
	...
	before := mcpRunControlGoalBackendStatsForTestV0(runRef, goalRef, orquestagoal.GoalStatusRunningV0)
	before.Stats = mcpRunControlStaleNoProcessStatsForTestV0(runRef)
	after := mcpRunControlGoalBackendStatsForTestV0(runRef, goalRef, orquestagoal.GoalStatusRunningV0)
	after.Stats = mcpRunControlStaleNoProcessStatsForTestV0(runRef)
	goalBackend := &fakeMCPRunControlGoalBackendStateV0{
		results: []MCPDirectorStatsToolResultV0{before, after},
	}
	executor := MCPRunControlToolExecutorV0{
		Port:             port,
		GoalBackendState: goalBackend,
	...
	result, err := executor.Execute(context.Background(), MCPRunControlToolInputV0{
		RequestID:    "req-run-control-goal-backend-stale-no-process-stop-001",
		Action:       "stop",
		RunRef:       runRef,
		RequestedBy:  "orquesta-director",
		Forced:       false,
	...
	if result.Estado != MCPRunControlEstadoOKV0 ||
		result.Status != string(orquestaruncontrol.RunControlStatusStoppedV0) ||
		result.GoalStatusAfter != orquestagoal.GoalStatusRunningV0 ||
		result.RecommendedAction != "replan_narrow_context" ||
		port.complete.TargetStatus != orquestaruncontrol.RunControlStatusStoppedV0 ||
		containsMCPRunControlDiagnosticForTestV0(result.Diagnostics, "control_not_propagated_to_goal_backend") ||
		!containsMCPRunControlDiagnosticForTestV0(result.Diagnostics, "goal_state_terminal_reconciled_after_external_cleanup") ||
		!containsStringMCPTestV0(result.EvidenceRefs, "evidence-ref-run-control-goal-external-cleanup-reconciled") {
		t.Fatalf("result=%+v complete=%+v", result, port.complete)
	}
...
func TestMCPRunControlExecutorV0CancelReconcilesExternalCleanupConEvidenciaSinForce(t *testing.T) {
	runRef := "run-ref-run-control-goal-backend-missing-soft-cancel-001"
	goalRef := "goal-ref-run-control-goal-backend-missing-soft-cancel-001"
	externalGoalRef := "thread-ref-" + goalRef
	goalStates := &mcpGoalStateStoreForTestV0{states: map[string]orquestagoal.GoalWorkStateV0{}}
	if err := goalStates.SaveGoalWorkStateV0(context.Background(), orquestagoal.GoalWorkStateV0{
		RunRef:          runRef,
		...
		Spec: orquestagoal.GoalWorkSpecV0{
			RunRef:       runRef,
			GoalRef:      goalRef,
			Objective:    "Cancelar tras cleanup externo desde la accion recomendada por status.",
			DirectorKind: orquestagoal.GoalDirectorKindCodexGoalV0,
			WriteSet:     []orquestagoal.GoalWriteScopeV0{{Path: "docs"}},
		},
		LaunchReceipt: orquestagoal.GoalLaunchReceiptV0{
	...
	result, err := executor.Execute(context.Background(), MCPRunControlToolInputV0{
		RequestID:    "req-run-control-goal-backend-missing-soft-cancel-001",
		Action:       "cancel",
		RunRef:       runRef,
		RequestedBy:  "orquesta-director",
		Forced:       false,
...
func TestMCPRunControlExecutorV0PauseNoReconciliaExternalCleanupAunqueTraigaEvidencia(t *testing.T) {
	runRef := "run-ref-run-control-goal-backend-missing-soft-pause-001"
	goalRef := "goal-ref-run-control-goal-backend-missing-soft-pause-001"
	externalGoalRef := "thread-ref-" + goalRef
	goalStates := &mcpGoalStateStoreForTestV0{states: map[string]orquestagoal.GoalWorkStateV0{}}
	if err := goalStates.SaveGoalWorkStateV0(context.Background(), orquestagoal.GoalWorkStateV0{
		RunRef:          runRef,
		...
		Spec: orquestagoal.GoalWorkSpecV0{
			RunRef:       runRef,
			GoalRef:      goalRef,
			Objective:    "No reconciliar cleanup externo desde acciones no terminales.",
			DirectorKind: orquestagoal.GoalDirectorKindCodexGoalV0,
			WriteSet:     []orquestagoal.GoalWriteScopeV0{{Path: "docs"}},
		},
		LaunchReceipt: orquestagoal.GoalLaunchReceiptV0{
	...
	result, err := executor.Execute(context.Background(), MCPRunControlToolInputV0{
		RequestID:    "req-run-control-goal-backend-missing-soft-pause-001",
		Action:       "pause",
		RunRef:       runRef,
		RequestedBy:  "orquesta-director",
		Reason:       "pausar tras observar cleanup externo sin reconciliar",
		Forced:       false,
		EvidenceRefs: []string{mcpAutoprogrammingEvidenceGoalBackendMissingAfterExternalCleanupV0},
	})
	...
	if result.Estado != MCPRunControlEstadoOKV0 ||
		...
		containsMCPRunControlDiagnosticForTestV0(result.Diagnostics, "goal_state_terminal_reconciled_after_external_cleanup") {
		t.Fatalf("pause no debe reconciliar cleanup externo: result=%+v pause=%+v complete=%+v", result, port.pause, port.complete)
	}
	state, err := goalStates.LoadGoalWorkStateV0(context.Background(), runRef)
	if err != nil {
		t.Fatalf("LoadGoalWorkStateV0: %v", err)
	...
	if state.Status != orquestagoal.GoalStatusRunningV0 ||
		...
		t.Fatalf("state mutado por pause con evidencia de cleanup externo: %+v", state)
	}
}

func TestMCPRunControlExecutorV0ResumeNoReconciliaExternalCleanupAunqueTraigaEvidencia(t *testing.T) {
	runRef := "run-ref-run-control-goal-backend-missing-soft-resume-001"
	goalRef := "goal-ref-run-control-goal-backend-missing-soft-resume-001"
	externalGoalRef := "thread-ref-" + goalRef
	goalStates := &mcpGoalStateStoreForTestV0{states: map[string]orquestagoal.GoalWorkStateV0{}}
	if err := goalStates.SaveGoalWorkStateV0(context.Background(), orquestagoal.GoalWorkStateV0{
		RunRef:          runRef,
		...
		Spec: orquestagoal.GoalWorkSpecV0{
			RunRef:       runRef,
			GoalRef:      goalRef,
			Objective:    "No reconciliar cleanup externo al reanudar.",
			DirectorKind: orquestagoal.GoalDirectorKindCodexGoalV0,
			WriteSet:     []orquestagoal.GoalWriteScopeV0{{Path: "docs"}},
		},
		LaunchReceipt: orquestagoal.GoalLaunchReceiptV0{
	...
	result, err := executor.Execute(context.Background(), MCPRunControlToolInputV0{
		RequestID:    "req-run-control-goal-backend-missing-soft-resume-001",
		Action:       "resume",
		RunRef:       runRef,
		RequestedBy:  "orquesta-director",
		Reason:       "reanudar sin reconciliar cleanup externo",
		Forced:       false,
		EvidenceRefs: []string{mcpAutoprogrammingEvidenceGoalBackendMissingAfterExternalCleanupV0},
	})
	...
	if result.Estado != MCPRunControlEstadoOKV0 ||
		...
		containsMCPRunControlDiagnosticForTestV0(result.Diagnostics, "goal_state_terminal_reconciled_after_external_cleanup") {
		t.Fatalf("resume no debe reconciliar cleanup externo: result=%+v resume=%+v complete=%+v", result, port.resume, port.complete)
	}
	state, err := goalStates.LoadGoalWorkStateV0(context.Background(), runRef)
	if err != nil {
		t.Fatalf("LoadGoalWorkStateV0: %v", err)
...
func mcpRunControlStaleNoProcessStatsForTestV0(
	runRef string,
) *orquestacionnucleoapp.DirectorRunStatsV0 {
