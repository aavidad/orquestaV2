...
func MCPRunControlDescriptorV0() MCPRunControlToolDescriptorV0 {
	return MCPRunControlToolDescriptorV0{
		...
		Output:      "ok:{run_ref,action,status,previous_status?,final_status?,goal_ref?,external_goal_ref?,goal_status_before?,goal_status_after?,goal_control_signal_sent?,goal_control_signal_confirmed?,recommended_action?,checkpoint_recorded,forced?,evidence_refs?,diagnostics?[]{code,scope?,message?,evidence_refs?}}|error:{errores_publicos,forced?,evidence_refs?,diagnostics?[]{code,scope?,message?,evidence_refs?}}",
