...
func TestNuevaAppHTMLHandlerV0GETMuestraFormularioUsableSinDelegar(t *testing.T) {
	...
	for _, want := range []string{
		...
		`data-guided-action="architecture_onion" data-help="Elige Onion`,
		`data-guided-action="architecture_microservices"`,
		`data-guided-action="architecture_data_pipeline"`,
		`data-guided-action="quality_public" data-help="Eleva pruebas y accesibilidad`,
		`data-guided-action="quality_regulated"`,
		`data-guided-action="accessibility_none"`,
		`id="guided-active-question"`,
		`id="guided-session-status"`,
		`id="guided-pending-list"`,
		`id="guided-answer"`,
		`data-guided-answer`,
		`data-wizard-bot`,
		`id="wizard-bot-input"`,
		`data-wizard-bot-send`,
		`/api/v0/apps/intake/wizard-bot`,
		`function submitWizardBot()`,
		`function ensureWizardBotSession()`,
		`id="wizard-i18n-json"`,
		`data-wizard-rich`,
		`Preguntas recomendadas`,
		`data-wizard-question-ref="wizard-q-tipo-app"`,
		`data-wizard-option-value="web"`,
		`<strong>Recomendada</strong>`,
		`data-wizard-rich-defaults`,
		`hexagonal_puertos_adaptadores`,
		`function renderRichWizard(wiz)`,
		`wizard_answers:[{question_ref:ref,user_choice:value}]`,
		...
		`guided-final-blocked`,
		`data-guided-ready-state="lista_para_solicitar"`,
		`data-guided-incomplete-launch="La sesion guiada aun tiene preguntas pendientes."`,
		`data-guided-msg-connectors="Conectores frecuentes incorporados como integraciones editables."`,
		`pending.map(field=>`,
		...
		`<option value="" title="Sin preferencia visible.`,
		`<option value="clean_architecture" title="Organiza entidades`,
		`value="event_driven" title="Comunica cambios por eventos`,
		...
		`value="vectorial" title="Embeddings y busqueda semantica`,
		`value="objetos_blob"`,
		...
		`data-help="Objetivo WCAG AA para interfaces publicas`,
		...
		`data-connector-quick value="api"`,
		`data-connector-quick value="auth"`,
		`data-connector-quick value="notifications"`,
		`data-connector-quick value="payments"`,
		`data-connector-quick value="search"`,
		`data-connector-quick value="analytics"`,
		`data-apply-connectors`,
		`function integrationTypeAlreadySelected(type)`,
		`const connectorTemplates={`,
		`function applySelectedConnectors()`,
		`wizard.querySelectorAll('[data-apply-connectors]')`,
		`Integracion 4`,
		`Integracion 6`,
		`value="maps" title="Mapas, geocodificacion`,
		`value="file_export"`,
		`value="messaging"`,
		`value="llm"`,
		`value="storage"`,
...
func TestNuevaAppHTMLV0RenderizaPanelGoalFirstConActualizacion(t *testing.T) {
	...
	vm.Director = &WebNuevaAppDirectorV0{
		...
		GoalStatus:            "running",
		EvidenceRefs:          []string{"evidence-ref-web-goal-001"},
	}
	...
	for _, want := range []string{
		...
		`data-goal-observe`,
		`data-goal-artifacts-section`,
		`data-goal-domain-receipts-section`,
		`data-goal-evidence-section`,
		`data-goal-closure-issues-section`,
		`evidence-ref-web-goal-001`,
