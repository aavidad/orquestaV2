...
func TestWebNuevaAppErrorViewModelV0LocalizaConnectorPolicyIssue(t *testing.T) {
	...
	if err.Code != orquestafactory.ErrConectorRequeridoNoDisponible ||
		...
		err.Message != "La integracion debe describir una capacidad, sin proveedor, credencial ni backend impuesto." {
		t.Fatalf("connector policy issue no localizado: %+v", err)
	}
}
