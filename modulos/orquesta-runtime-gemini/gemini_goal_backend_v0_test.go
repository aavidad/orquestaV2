...
func TestGeminiGoalBackendV0ObserveLeeResultadoDurableDelRuntimeV0(t *testing.T) {
	root := t.TempDir()
	backend := GeminiGoalBackendV0{
		ProjectWorkDir: filepath.Join(root, "project"),
		RuntimeWorkDir: filepath.Join(root, "runtime"),
	...
	resultPath := geminiGoalRuntimeResultPathV0(backend.RuntimeWorkDir, spec.GoalRef)
	if err := os.MkdirAll(filepath.Dir(resultPath), 0o700); err != nil {
		t.Fatalf("MkdirAll: %v", err)
	}
...
func TestGeminiGoalBackendV0RuntimeReceiptPrecedeFallbackWriteSetV0(t *testing.T) {
	root := t.TempDir()
	backend := GeminiGoalBackendV0{ProjectWorkDir: filepath.Join(root, "project"), RuntimeWorkDir: filepath.Join(root, "runtime")}
	spec := geminiGoalSpecForTestV0()
	if _, err := backend.LaunchGoalWorkV0(context.Background(), spec); err != nil {
		t.Fatalf("LaunchGoalWorkV0: %v", err)
	}
	runtimePath := geminiGoalRuntimeResultPathV0(backend.RuntimeWorkDir, spec.GoalRef)
	legacyPath := filepath.Join(backend.ProjectWorkDir, "docs", GeminiGoalResultFileNameV0)
