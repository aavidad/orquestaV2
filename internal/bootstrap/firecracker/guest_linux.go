...
// RunTestGuest composes the PID 1 guest process and its private supervisor mode.
func RunTestGuest(arguments []string) int {
	if exitCode, handled := firecrackerguest.DispatchPID1Supervisor(arguments); handled {
		return exitCode
	}
	if os.Getpid() != 1 {
		return 2
