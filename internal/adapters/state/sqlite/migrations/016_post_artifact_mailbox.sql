-- V21 gives post-artifact child delivery its own durable scheduler action.
-- The action is deliberately governance-v0: it is a causal state transition,
-- not a new external effect authority.
...
CREATE TABLE outbox (
 ...
 kind TEXT NOT NULL CHECK(kind IN ('launch_agent','observe_agent','stop_agent','deliver_mailbox','prepare_workspace','commit_change','attest_test','integrate_change','admit_mailbox')),
 ...
 CHECK(
  (kind IN ('launch_agent','observe_agent','prepare_workspace','admit_mailbox')
...
CREATE TABLE action_consumption_receipts (
 ...
 FOREIGN KEY(goal_ref,mailbox_message_ref,plan_generation,work_item_ref,execution_ref,work_item_generation)
  REFERENCES mailbox_envelopes(goal_ref,ref,plan_generation,parent_work_item_ref,recipient_execution_ref,recipient_work_item_generation) ON DELETE RESTRICT,
 CHECK((kind IN ('launch_agent','observe_agent','stop_agent','prepare_workspace','commit_change','attest_test','integrate_change','admit_mailbox')
...
CREATE UNIQUE INDEX action_consumption_scheduler_fence_idx
 ...
 WHERE kind IN ('launch_agent','observe_agent','stop_agent','prepare_workspace','commit_change','attest_test','integrate_change','admit_mailbox');
