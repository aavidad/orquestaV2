-- STOP-RECOVERY-CLAIM-STATE-05: el mismo outbox puede cercar la lectura de
-- recovery de un Stop ambiguo. No crea otro EffectAttempt ni habilita receipt
-- terminal tardio; esa escritura conserva una compuerta posterior separada.
...
WHEN (NEW.recovery_effect_attempt_ref IS NOT OLD.recovery_effect_attempt_ref AND NOT (
 ...
 OR (NEW.recovery_effect_attempt_ref IS NOT NULL AND NOT EXISTS (
  ...
  WHERE attempt.ref=NEW.recovery_effect_attempt_ref AND NEW.governance_version=1
   ...
   AND NEW.effect_intent_ref=attempt.intent_ref AND attempt.action_ref=NEW.ref
   AND attempt.action_fence<NEW.fence AND attempt.claim_lease_until IS NOT NULL
   AND intent.action_ref=NEW.ref
