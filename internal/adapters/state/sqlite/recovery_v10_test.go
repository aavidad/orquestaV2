...
WHERE type = 'table' AND name IN ('principals', 'project_memberships', 'authorization_receipts')`).Scan(&v10Tables); err != nil {
	...
	assertRecoverySchemaVersion(t, migrated.db, recoverySchemaV23)
	var bound int
...
func TestV10RecoveryRestoresExactV10IdentitySnapshot(t *testing.T) {
	...
	if receipt.SchemaRef != migrationSchemaRef(migrations[:recoverySchemaV23]) {
	...
	defer raw.Close()
	assertRecoverySchemaVersion(t, raw, recoverySchemaV23)
