...
WHERE type = 'table' AND name IN ('principals', 'project_memberships', 'authorization_receipts')`).Scan(&v10Tables); err != nil {
	...
	assertRecoverySchemaVersion(t, migrated.db, recoverySchemaV38Environment)
	var bound int
...
func TestV10RecoveryRestoresExactV10IdentitySnapshot(t *testing.T) {
	...
	if receipt.SchemaRef != migrationSchemaRef(migrations[:recoverySchemaV38Environment]) {
	...
	defer raw.Close()
	assertRecoverySchemaVersion(t, raw, recoverySchemaV38Environment)
