...
WHERE type = 'table' AND name IN ('principals', 'project_memberships', 'authorization_receipts')`).Scan(&v10Tables); err != nil {
	...
	assertRecoverySchemaVersion(t, migrated.db, recoverySchemaLatest)
	var bound int
...
func TestV10RecoveryRestoresExactV10IdentitySnapshot(t *testing.T) {
	...
	if receipt.SchemaRef != migrationSchemaRef(migrations[:recoverySchemaLatest]) {
	...
	defer raw.Close()
	assertRecoverySchemaVersion(t, raw, recoverySchemaLatest)
