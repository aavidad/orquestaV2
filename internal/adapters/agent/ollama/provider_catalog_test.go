...
func TestProviderCatalogPublishesOnlyObservedModelIDs(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		_, _ = writer.Write([]byte(`{"models":[{"name":"qwen3:8b"},{"name":"coder-looking:latest"}]}`))
	}))
	defer server.Close()
	source := newLoopbackTestSource(t, server.URL, func() time.Time { return now }, time.Minute, 1<<10)

	observation, err := source.ObserveProviderCatalog(context.Background())
	if err != nil {
		t.Fatal(err)
	}
	if method != http.MethodGet || path != "/api/tags" || host != strings.TrimPrefix(server.URL, "http://") {
	...
	if observation.Availability != ports.ProviderAvailabilityUnknown ||
		...
		t.Fatalf("catalog response invented facts: %+v", observation)
	}
	for _, model := range observation.Models {
		if len(model.CapabilityRefs) != 0 || len(model.ReasoningEfforts) != 0 {
			t.Fatalf("model name invented capability or effort: %+v", model)
		}
	}
}
...
func TestProviderCatalogRejectsInvalidBounds(t *testing.T) {
	now := time.Date(2026, 8, 21, 11, 30, 0, 0, time.UTC)
	valid := validConfig("http://127.0.0.1:11434", func() time.Time { return now })
...
func TestProviderCatalogFailsClosedOnPayloadAndFreshnessFailures(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "malformed", body: `{"models":[`, maximum: 100, want: ErrProviderCatalogInvalid},
		{name: "duplicate", body: `{"models":[{"name":"m"},{"name":"m"}]}`, maximum: 100, want: ErrProviderCatalogInvalid},
		{name: "trailing", body: `{"models":[]} {}`, maximum: 100, want: ErrProviderCatalogInvalid},
		{name: "oversized", body: `{"models":[]}` + strings.Repeat(" ", 20), maximum: 13, want: ErrProviderCatalogResponseTooLarge},
	}
