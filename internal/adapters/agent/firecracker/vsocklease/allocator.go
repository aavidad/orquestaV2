...
const (
	...
	transactionPhaseTemporalOnly
	transactionPhaseBusinessSavepoint
	transactionPhaseFinished
)

func (guard *transactionGuard) beginTemporalOnly() error {
	if guard == nil || guard.phase != transactionPhaseOpen {
		return allocatorError("transaction_invalid")
	}
	guard.phase = transactionPhaseTemporalOnly
	return nil
}

func (guard *transactionGuard) protectTemporalFrontier(ctx context.Context) error {
	if guard == nil || guard.phase != transactionPhaseTemporalOnly {
		return allocatorError("transaction_invalid")
	}
	...
	guard.phase = transactionPhaseBusinessSavepoint
	return nil
}

func (guard *transactionGuard) commit(ctx context.Context) error {
	if guard == nil || guard.phase != transactionPhaseBusinessSavepoint {
		return allocatorError("transaction_invalid")
	}
...
func (guard *transactionGuard) finishReturn(returned error) error {
	...
	case transactionPhaseOpen:
		...
		return allocatorError("transaction_invalid")
	case transactionPhaseTemporalOnly:
		if returned == nil {
			returned = allocatorError("transaction_invalid")
		}
		return guard.commitTemporalOnly(returned)
	case transactionPhaseBusinessSavepoint:
		if returned == nil {
			returned = allocatorError("transaction_invalid")
		}
...
func (guard *transactionGuard) commitTemporalOnly(returned error) error {
	if guard == nil || guard.phase != transactionPhaseTemporalOnly {
		return allocatorError("transaction_invalid")
	}
	guard.phase = transactionPhaseFinished
	if err := guard.allocator.commit(context.Background(), guard.connection); err != nil {
		return err
	}
	return returned
}
...
func Open(ctx context.Context, config Config) (*Allocator, error) {
	if config.DB == nil ||
		...
		config.MinimumLeaseDuration <= 0 ||
		config.MaximumLeaseDuration < config.MinimumLeaseDuration ||
		config.Now == nil {
...
func (allocator *Allocator) Reserve(
	...
	if err := ports.ValidateAgentMicroVMVsockCIDReservationRequest(request); err != nil ||
		...
		request.LeaseDuration < allocator.config.MinimumLeaseDuration ||
	...
	if err := transaction.beginTemporalOnly(); err != nil {
		return ports.AgentMicroVMVsockCIDLease{}, err
	}
	...
	if found {
		if operation.Kind != "reserve" || operation.RequestDigest != requestDigest {
			return ports.AgentMicroVMVsockCIDLease{}, allocator.commitRejection(
				ctx, transaction, allocatorError("idempotency_conflict"),
			)
		}
		...
		if record.State != reservationStateActive || !record.Lease.ExpiresAt.After(now) {
			return ports.AgentMicroVMVsockCIDLease{}, allocator.commitRejection(
				ctx, transaction, allocatorError("reservation_replay_denied"),
			)
		}
	...
	if !ok {
		return ports.AgentMicroVMVsockCIDLease{}, allocator.commitRejection(
			ctx, transaction, allocatorError("capacity_reached"),
		)
	}
	fencingToken, err := nextFencingToken(ctx, connection, request.PoolRef, guestCID)
	if err != nil {
		if ErrorCode(err) == "agent_firecracker_vsock_cid.fencing_exhausted" {
			return ports.AgentMicroVMVsockCIDLease{}, allocator.commitRejection(
				ctx, transaction, err,
			)
		}
	...
	if !ok {
		return ports.AgentMicroVMVsockCIDLease{}, allocator.commitRejection(
			ctx, transaction, allocatorError("lease_time_unrepresentable"),
		)
	}
...
func (allocator *Allocator) Renew(
	...
	if operation, found, err := readOperation(
		...
		if record.State != reservationStateActive || !record.Lease.ExpiresAt.After(now) {
			return ports.AgentMicroVMVsockCIDLease{}, allocator.commitRejection(
				ctx, transaction, allocatorError("lease_inactive"),
			)
		}
		if err := transaction.commit(ctx); err != nil {
			return ports.AgentMicroVMVsockCIDLease{}, err
	...
	if err := validateCurrentOwner(record, request.PoolRef, request.ScopeDigest,
		...
		return ports.AgentMicroVMVsockCIDLease{}, allocator.commitRejection(ctx, transaction, err)
	}
	if record.State != reservationStateActive || !record.Lease.ExpiresAt.After(now) {
		return ports.AgentMicroVMVsockCIDLease{}, allocator.commitRejection(
			ctx, transaction, allocatorError("lease_inactive"),
	...
	if record.Lease.Revision != request.ExpectedRevision {
		return ports.AgentMicroVMVsockCIDLease{}, allocator.commitRejection(
			ctx, transaction, allocatorError("stale_revision"),
		)
	}
	...
	if !newExpiry.After(record.Lease.ExpiresAt) {
		return ports.AgentMicroVMVsockCIDLease{}, allocator.commitRejection(
			ctx, transaction, allocatorError("renewal_not_extending"),
		)
	}
...
func (allocator *Allocator) Release(
	...
	if err := validateCurrentOwner(record, request.PoolRef, request.ScopeDigest,
		request.OwnerRef, request.FencingToken); err != nil {
		return ports.AgentMicroVMVsockCIDReleaseReceipt{}, allocator.commitRejection(ctx, transaction, err)
	}
	if record.State != reservationStateActive || !record.Lease.ExpiresAt.After(now) {
		return ports.AgentMicroVMVsockCIDReleaseReceipt{}, allocator.commitRejection(
			ctx, transaction, allocatorError("lease_inactive"),
...
func durableOperationTime(
	...
	observedUnixNano := observed.UnixNano()
	if observed.IsZero() || observedUnixNano <= 0 ||
		!time.Unix(0, observedUnixNano).UTC().Equal(observed) {
		return time.Time{}, allocatorError("clock_invalid")
	}
	var highWaterUnixNano int64
	err := connection.QueryRowContext(ctx, `
SELECT high_water_unix_nano
FROM agent_microvm_vsock_cid_clock
WHERE pool_ref = ?`, poolRef).Scan(&highWaterUnixNano)
	switch {
	case errors.Is(err, sql.ErrNoRows):
		if _, err := connection.ExecContext(ctx, `
INSERT INTO agent_microvm_vsock_cid_clock (pool_ref, high_water_unix_nano)
VALUES (?, ?)`, poolRef, observedUnixNano); err != nil {
			return time.Time{}, allocatorError("store_unavailable")
		}
	case err != nil:
		return time.Time{}, allocatorError("store_unavailable")
	case highWaterUnixNano <= 0:
		return time.Time{}, allocatorError("store_corrupt")
	case observedUnixNano < highWaterUnixNano:
		return time.Time{}, allocatorError("clock_regressed")
	case observedUnixNano > highWaterUnixNano:
		result, err := connection.ExecContext(ctx, `
UPDATE agent_microvm_vsock_cid_clock
SET high_water_unix_nano = ?
WHERE pool_ref = ? AND high_water_unix_nano = ?`,
			observedUnixNano, poolRef, highWaterUnixNano,
		)
		if err != nil {
			return time.Time{}, allocatorError("store_unavailable")
		}
...
func (allocator *Allocator) mapReadErrorAfterClock(
	...
	if ErrorCode(mapped) == "agent_firecracker_vsock_cid.lease_not_found" {
		return allocator.commitRejection(ctx, transaction, mapped)
	}
	return mapped
}
...
var schemaStatements = []string{
	`CREATE TABLE agent_microvm_vsock_cid_clock (
    pool_ref TEXT PRIMARY KEY,
    high_water_unix_nano INTEGER NOT NULL CHECK (high_water_unix_nano > 0)
) STRICT`,
...
var schemaObjects = []struct {
	...
	{Kind: "table", Name: "agent_microvm_vsock_cid_clock"},
