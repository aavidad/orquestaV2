...
func runtimeMailboxBatchBudget() time.Duration {
	if runtimeMailboxBatchBudgetOverride > 0 {
		return runtimeMailboxBatchBudgetOverride
	}
	ms := controlPlaneConfigIntOrDefault("runtime_mailbox_batch_budget_ms", 1200)
	if ms <= 0 {
		ms = 1200
	}
...
func runtimeMailboxBatchDebugf(format string, args ...any) {
	base := parseBoolDebug(strings.TrimSpace(os.Getenv("ORQUESTA_DEBUG")), false)
	if !parseBoolDebug(strings.TrimSpace(os.Getenv("ORQUESTA_DEBUG_CONTROL_PLANE")), base) {
		return
	}
	log.Printf("orquesta[runtime-mailbox] "+format, args...)
}
...
func resetAutonomiaRuntimeRecoveryGate() {
	autonomiaRuntimeRecoveryGate.Reset()
}
...
func autonomiaRuntimeRecoveryInterval() time.Duration {
	seconds := controlPlaneConfigIntOrDefault("autonomia_runtime_recovery_interval_seconds", 180)
	if seconds <= 0 {
		seconds = 180
	}
	return time.Duration(seconds) * time.Second
...
func autonomiaRuntimeRecoveryShouldAttempt(agente string, proyectoID int64, estadoSesion string) bool {
	if strings.EqualFold(strings.TrimSpace(estadoSesion), "pausada") {
		return true
	}
	agente = strings.ToLower(strings.TrimSpace(agente))
	if agente == "" || proyectoID <= 0 {
	...
	key := agente + "|" + strconv.FormatInt(proyectoID, 10)
	return autonomiaRuntimeRecoveryGate.Allow(key, autonomiaRuntimeRecoveryInterval())
}
...
type runtimeMailboxBatchPhase struct {
	name       string
	deferStage string
	fn         func() (int, error)
}
...
func procesarRuntimeMailboxPhases(phases []runtimeMailboxBatchPhase, budget time.Duration) (int, error) {
	...
	for idx, phase := range phases {
		...
		if budget > 0 && time.Since(start) >= budget {
			db.Audit("orquesta", "runtime_mailbox_batch_deferred", "runtime_mailbox", 0,
				fmt.Sprintf("budget=%s total=%d phase=%s", budget, total, strings.TrimSpace(phase.name)))
			return total, nil
		}
		phaseStart := time.Now()
		n, err := phase.fn()
		...
		if budget > 0 && time.Since(start) >= budget && idx < len(phases)-1 {
			db.Audit("orquesta", "runtime_mailbox_batch_deferred", "runtime_mailbox", 0,
				fmt.Sprintf("budget=%s total=%d phase=%s", budget, total, strings.TrimSpace(phase.name)))
			return total, nil
		}
...
func procesarSignalSoloSupervision(item *db.RuntimeTranscriptEntry) (string, error) {
	notes := make([]string, 0, 2)
...
func ejecutarPlanSupervisorSignalTranscript(item *db.RuntimeTranscriptEntry, policy *supervisionapp.Policy, proyecto *db.Proyecto) (string, error) {
	...
	if note, err := asegurarTareasSignalTranscript(item, policy, proyecto, supervisor); err != nil {
		return "", err
	} else {
...
func asegurarTareasSignalTranscript(item *db.RuntimeTranscriptEntry, policy *supervisionapp.Policy, proyecto *db.Proyecto, supervisor *db.Agente) (string, error) {
	if !contextoSignalTranscriptActivo(item, policy, proyecto) {
		return "", nil
	}
	notes := make([]string, 0, 2)
	if note, err := ejecutarPlanReplanSignalTranscript(item, policy, proyecto, supervisor); err != nil {
		return "", err
	} else {
		notes = agregarNotaAutonomia(notes, note)
	}
	if note, err := ejecutarPlanCredencialesSignalTranscript(item, policy, proyecto, supervisor); err != nil {
		return "", err
	} else {
		notes = agregarNotaAutonomia(notes, note)
	}
...
func ejecutarPlanCredencialesSignalTranscript(item *db.RuntimeTranscriptEntry, policy *supervisionapp.Policy, proyecto *db.Proyecto, supervisor *db.Agente) (string, error) {
	...
	if existe {
		return "credentials_task_exists", nil
	}
	target := resolverAgenteObjetivoReplanAutonomiaSignal(policy, supervisor, agenteSignalTranscript(item))
	id, err := crearTareaCredencialesAutonomiaSignal(policy, proyecto, item, target)
	if err != nil {
		return "", err
	}
	return fmt.Sprintf("credentials_task_created:%d", id), nil
}

func existeTareaCredencialesAutonomiaPendiente(proyectoID int64) (bool, error) {
	if proyectoID <= 0 {
		return false, nil
	}
	tareas, err := tareasService.List(db.FiltroTareas{ProyectoID: &proyectoID})
	...
	for _, tarea := range tareas {
		...
		if esTareaCredencialesAutonomia(tarea) {
			return true, nil
		}
	}
	return false, nil
...
func crearTareaCredencialesAutonomiaSignal(policy *supervisionapp.Policy, proyecto *db.Proyecto, item *db.RuntimeTranscriptEntry, target string) (int64, error) {
	if policy == nil || proyecto == nil || item == nil {
		return 0, nil
	}
	id, err := tareasService.Create(tareasapp.CreateTaskInput{
		Titulo:      autonomiaCredentialsTaskTitle,
		Descripcion: construirDescripcionTareaCredencialesAutonomia(policy, proyecto, item),
		Modulo:      "autonomia",
		Prioridad:   db.PrioridadAlta,
		CreadoPor:   "orquesta",
		Agente:      target,
		Proyecto:    proyecto.Slug,
		Notas:       construirNotasTareaCredencialesAutonomiaSignal(item),
	})
	if err != nil {
		return 0, err
	}
...
func construirNotasTareaCredencialesAutonomiaSignal(item *db.RuntimeTranscriptEntry) string {
	if item == nil {
...
func esTareaCredencialesAutonomia(tarea *db.Tarea) bool {
	...
	if strings.EqualFold(strings.TrimSpace(tarea.Titulo), autonomiaCredentialsTaskTitle) {
		return true
	}
...
func construirDescripcionTareaCredencialesAutonomia(policy *supervisionapp.Policy, proyecto *db.Proyecto, item *db.RuntimeTranscriptEntry) string {
	partes := []string{
		"Orquesta ha detectado en el transcript una señal de credentials_request: el agente declara que faltan credenciales, acceso o un recurso externo.",
		"Comprueba si el bloqueo puede resolverse de forma autónoma con cuentas compartidas, perfiles seed+clone, OAuth existente, secretos ya provisionados o reasignación a otro agente con acceso.",
		"Si el recurso externo falta de verdad, deja trazabilidad explícita del bloqueo, evita que el frente se quede parado y abre o mueve el trabajo al siguiente frente útil disponible.",
	}
	if slugProyectoAutonomia(proyecto) != "" {
		partes = append(partes, "Proyecto: "+slugProyectoAutonomia(proyecto)+".")
	}
...
func accionSupervisorSignalTranscript(item *db.RuntimeTranscriptEntry) string {
	switch clasificacionSignalTranscript(item) {
	case "cli_query":
...
func construirInstruccionSupervisionSignalTranscript(policy *supervisionapp.Policy, proyecto *db.Proyecto, supervisor string, item *db.RuntimeTranscriptEntry) string {
	...
	if detalle := detalleSupervisorSignalTranscript(item); detalle != "" {
...
func detalleSupervisorSignalTranscript(item *db.RuntimeTranscriptEntry) string {
	switch clasificacionSignalTranscript(item) {
	case "cli_query":
...
func filtrarSesionesAutonomiaRelevantes(sesiones []*db.Sesion) []*db.Sesion {
	...
	sort.SliceStable(out, func(i, j int) bool {
		if prioridadSesionAutonomia(out[i]) != prioridadSesionAutonomia(out[j]) {
			return prioridadSesionAutonomia(out[i]) > prioridadSesionAutonomia(out[j])
		}
...
func resolverProyectoAutoasignacionDesdeAsignacionPausada(agente string) (*db.Proyecto, error) {
	...
	for _, asignacion := range asignaciones {
		...
		score := 0
		if proyectoUsaContinuidadMicrocicloPremium(asignacion.ProyectoID) {
			score += 100
		}
...
func procesarAutonomiaSesionActivaConSnapshot(sesion *db.Sesion, snapshot *autonomiaBatchSnapshot) (int, error) {
	...
	autonomiaTickDebugf("agente=%s proyecto_id=%d tick_output duration=%s accion=%s", strings.TrimSpace(sesion.Agente), valorProyectoID(sesion.ProyectoID), time.Since(decisionStart).Round(time.Millisecond), strings.TrimSpace(out.AccionRecomendada))
...
func procesarPrechecksAutonomiaSesionActiva(sesion *db.Sesion, snapshot *autonomiaBatchSnapshot) (int, error) {
	...
	checks := []autonomiaPrecheck{
		...
		{name: "compactacion_exclusividad", fn: func() (int, error) { return procesarCompactacionExclusividadPremiumSesionActiva(sesion, snapshot) }},
		{name: "recuperacion_bloqueadas", fn: func() (int, error) { return procesarRecuperacionTareasBloqueadasSesionActiva(sesion, snapshot) }},
		{name: "compactacion_frentes", fn: func() (int, error) { return procesarCompactacionFrentesPremiumSesionActiva(sesion, snapshot) }},
		{name: "derivacion_semilla", fn: func() (int, error) { return procesarDerivacionSemillaPremiumSesionActiva(sesion, snapshot) }},
	}
	for _, check := range checks {
		start := time.Now()
		n, err := check.fn()
		autonomiaTickDebugf("agente=%s proyecto_id=%d precheck=%s duration=%s procesadas=%d err=%v", strings.TrimSpace(sesion.Agente), valorProyectoID(sesion.ProyectoID), check.name, time.Since(start).Round(time.Millisecond), n, err != nil)
		if err != nil || n > 0 {
			if n > 0 {
...
func procesarRecuperacionRuntimeRemotoDegradadoSesion(sesion *db.Sesion, proyecto *db.Proyecto, handle *db.RuntimeHandle, runtime *db.RuntimeInstance) (int, error) {
	return orquestacionAgentesService.RecoverRemoteDegradedRuntimeSession(sesion, proyecto, handle, runtime)
}
...
func intentarHandoffAutonomoTarea(proyecto *db.Proyecto, tareaID int64, origen, relevo, verificationKey, motivo, resumen, nota, cierre string) (bool, error) {
	...
	origen = strings.TrimSpace(origen)
	relevo = strings.TrimSpace(relevo)
...
func handoffCanFallback(err error) bool {
	...
	switch {
	case strings.Contains(text, "no tiene sesión abierta"):
		return true
	case strings.Contains(text, "no tiene sesion abierta"):
		return true
	case strings.Contains(text, "no tiene runtime handle activo"):
		return true
	default:
		return false
	}
