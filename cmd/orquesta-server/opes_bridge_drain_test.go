...
func TestRunOPESDrainOnceV0GoalFirstObserveErrorConservaCodigoPublicoV0(t *testing.T) {
	opesServer := newLocalHTTPServerForTestV0(t, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		_ = json.NewEncoder(w).Encode([]map[string]any{{
			"id":             "job-ref-goal-observe-error-001",
			"type":           "draft_content_block",
			"status":         "pending",
			"execution_mode": "external",
			"payload_json":   `{"topic_id":"topic-ref-001","title":"Bloque goal observe error"}`,
			"requested_by":   "opes",
		}})
	}))
	defer opesServer.Close()
	...
	orquestaServer := newLocalHTTPServerForTestV0(t, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/api/v0/runs/supervise":
			supervisions++
			t.Fatalf("goal-first con observe_goal fallido no debe llamar a runs/supervise")
		default:
			t.Fatalf("orquesta request inesperada %s %s", r.Method, r.URL.Path)
		}
	}))
	...
	if err := opesBridgeRecordSubmittedV0(
		context.Background(),
		ledger,
		orquestaopesconnector.ExternalJobV0{ID: "job-ref-goal-observe-error-001"},
		"run-ref-goal-observe-error-001",
		"change-ref-goal-observe-error-001",
		externalBridgeInputRunMetadataV0{
			RoutePolicy:           "goal_first",
			DirectorExecutionMode: "goal_first",
			GoalRef:               "goal-ref-goal-observe-error-001",
			ExternalGoalRef:       "thread-ref-goal-observe-error-001",
			NextActions:           []string{"observe_goal"},
		},
	); err != nil {
		t.Fatalf("record ledger: %v", err)
	...
	if err != nil ||
		...
		!strings.Contains(summary.Results[0].SupervisionStopReason, "codex_app_server_control_socket_missing") {
		t.Fatalf("observes=%d supervisions=%d summary=%+v err=%v", observes, supervisions, summary, err)
	}
}
...
func TestRunOPESDrainOnceV0AudioConPrepareStaleNoPosteaOrquestaV0(t *testing.T) {
	opesServer := newLocalHTTPServerForTestV0(t, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		_ = json.NewEncoder(w).Encode([]map[string]any{{
			"id":             "job-ref-audio-prepare-stale-001",
			"type":           "generate_audio_asset",
			"status":         "pending",
			"execution_mode": "external",
			"payload_json":   opesAudioPayloadWithStalePrepareForDrainTestV0(),
			"requested_by":   "opes",
		}})
	}))
	defer opesServer.Close()
	...
	orquestaServer := newLocalHTTPServerForTestV0(t, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		posts++
		t.Fatalf("no debe postear a Orquesta con prepare stale: %s %s", r.Method, r.URL.Path)
	}))
	defer orquestaServer.Close()
	...
	if posts != 0 ||
		...
		len(summary.Results) != 1 ||
		summary.Results[0].Status != "phase_precondition_missing" ||
		summary.Results[0].CurrentPhase != "prepare" ||
		summary.Results[0].OperationalReason != orquestaopesbridge.OPESAudioWorkflowPrepareStaleRequiredReasonV0 ||
		summary.Results[0].AudioCounters["audio_manifest_refs"] != 1 ||
		summary.Results[0].AudioCounters["text_hash_refs"] != 2 ||
		len(summary.Errors) != 1 ||
		summary.Errors[0].Reason != orquestaopesbridge.OPESAudioWorkflowPrepareStaleRequiredReasonV0 ||
		!containsStringForTestV0(summary.Results[0].NextActions, "retry_from_phase=prepare") {
		t.Fatalf("posts=%d summary=%+v", posts, summary)
	}
}
...
func TestRunOPESDrainOnceV0AudioAlreadySubmittedProviderHeartbeatTimeoutProyectaSinReenviarV0(t *testing.T) {
	...
	opesServer := newLocalHTTPServerForTestV0(t, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.URL.Path == "/api/jobs" && r.Method == http.MethodGet:
			_ = json.NewEncoder(w).Encode([]map[string]any{{
				"id":             "job-ref-audio-provider-timeout-001",
				"type":           "generate_audio_asset",
				"status":         "pending",
				"execution_mode": "external",
				"payload_json":   opesDerivedPayloadForDrainTestV0("generate_audio_asset"),
		...
		case r.URL.Path == "/api/jobs/job-ref-audio-provider-timeout-001" && r.Method == http.MethodGet:
			jobRefreshes++
			...
			if jobRefreshes >= 1 {
				...
				response["provider_reason"] = "provider_timeout"
			}
	...
	orquestaServer := newLocalHTTPServerForTestV0(t, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		_ = json.NewEncoder(w).Encode(map[string]string{
			"run_ref": "run-ref-audio-provider-timeout-001",
			"estado":  "accepted",
		})
	}))
	defer orquestaServer.Close()
...
func opesAudioPayloadWithStalePrepareForDrainTestV0() string {
	return `{"program_id":"program-ref-operadores-001","topic_id":"topic-ref-operadores-001","document_plan_artifact_id":"artifact-plan-operadores-001","scope":"tema completo","text_public_status":"pass","audio_regeneration_mode":"selective_by_sidecar","audio_manifest_ref":"audio-manifest-ref-operadores-001","source_content_ref":"assembled-topic-ref-operadores-001","current_text_hash_ref":"hash-ref-html-final-v2","prepared_text_hash_ref":"hash-ref-html-final-v1"}`
}
...
func TestSmokeOPESDerivativesRESTWrapperScopeProbeFakeServerV0(t *testing.T) {
	...
	cmd.Env = cleanOPESSmokeEnvForDrainTestV0(os.Environ())
	outDir := filepath.Join(t.TempDir(), "out")
	scopeProbeFile := filepath.Join(t.TempDir(), "custom", "opes_scope_probe.json")
	cmd.Env = append(cmd.Env,
		"ORQUESTA_OPES_DERIVATIVES_FAKE_SERVER=1",
		"ORQUESTA_OPES_DERIVATIVES_SMOKE_MODE=scope-probe",
		"ORQUESTA_OPES_BRIDGE_PROGRAM_ID=program-ref-fake-operario-001",
		"ORQUESTA_OPES_BRIDGE_LIMIT=1",
		"ORQUESTA_OPES_BRIDGE_SCOPE_PROBE_OUTPUT="+scopeProbeFile,
	...
	if !strings.Contains(output, "scope_probe_status=ok") ||
		...
		!strings.Contains(output, "scope_probe_output="+scopeProbeFile) {
		t.Fatalf("stdout=%s stderr=%s", output, stderr.String())
	}
...
func TestSmokeOPESDerivativesRESTWrapperPreflightRealAceptaScopeProbeJSONV0(t *testing.T) {
	if _, err := exec.LookPath("python3"); err != nil {
		t.Skip("python3 no disponible")
	}
...
func TestSmokeOPESDerivativesRESTWrapperPreflightRealNoExigeRemoteQASinRevisionRemotaV0(t *testing.T) {
	stdout, stderr, err := runSmokeOPESDerivativesPreflightForTestV0(t,
		"ORQUESTA_OPES_BRIDGE_JOB_TYPE_SEQUENCE=update_topic_registry,review_director_consolidation,assemble_topic",
		"ORQUESTA_OPES_BRIDGE_PROGRAM_ID=program-ref-operadores-preflight",
		"ORQUESTA_OPES_BRIDGE_SCOPE_FILTER_CONFIRMED=1",
		"ORQUESTA_OPES_BRIDGE_SCOPE_FILTER_EVIDENCE_REF=evidence-ref-scope-probe-preflight",
		"ORQUESTA_CODEX_GOAL_BACKEND=app_server_tmux",
...
func TestSmokeOPESDerivativesRESTWrapperPreflightRealBloqueaScopeProbeJSONNominalV0(t *testing.T) {
	if _, err := exec.LookPath("python3"); err != nil {
		t.Skip("python3 no disponible")
	}
	probeFile := filepath.Join(t.TempDir(), "opes_derivatives_scope_probe.json")
