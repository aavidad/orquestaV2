...
type openClawRecoveryPlanStep = serverOperationalRecoveryStep

type openClawRecoveryPlan = serverOperationalRecoveryPlan
...
func buildOpenClawNextRecoveryPlan(info serverOperationalInfo) *openClawRecoveryPlan {
	return buildServerOperationalRecoveryPlan(info, buildOpenClawNextRecoveryAction(info))
}
...
func buildMCPOpenClawOperatorOverview(supervisor string) (string, error) {
	...
	nextRecoveryPlan, _ := payload["next_recovery_plan"].(*openClawRecoveryPlan)
	...
	if nextRecoveryPlan != nil {
		b.WriteString("\nRecovery plan:\n")
		b.WriteString("- ")
		b.WriteString(strings.TrimSpace(nextRecoveryPlan.Action))
		if summary := compactMCPLine(strings.TrimSpace(nextRecoveryPlan.Summary), 180); summary != "" {
			b.WriteString(" · ")
			b.WriteString(summary)
		}
		if nextRecoveryPlan.AutoExecutable {
			b.WriteString(" · auto")
		}
		b.WriteString("\n")
	}
