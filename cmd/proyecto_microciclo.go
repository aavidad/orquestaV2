...
func construirInboxMicrocicloMarkdown(proyecto *db.Proyecto, tarea *db.Tarea) string {
	...
	lines := []string{
		...
		"- Primer movimiento obligatorio: aplica un patch minimo en uno de los archivos del write-set antes de cualquier rg, sed o lectura adicional.",
		"- Si el simbolo exacto no esta claro tras leer la inbox, usa entonces: rg -n 'procesarRuntimeMailboxSessionResumeBatchConMailbox|resolverBootstrapRuntimeLeasePendiente' cmd/controlplane_support.go db/controlplane_entities.go runtimeagente/driver.go",
