...
| `modulos/orquesta-goal` | Contrato neutral `GoalWorkSpecV0`/`GoalLaunchReceiptV0`/`GoalWorkResultV0`, lifecycle puro `StartGoalWorkV0`/`ObserveGoalWorkV0` y pasada residente `ObserveActiveGoalWorksV0` para delegar el loop interno a un runtime con goal persistente. | Alineada. Orquesta gobierna contexto/cierre; el runtime dirige dentro del goal; las composiciones proyectan cierre a runs/colas/dominios sin reabrir el loop director historico. |
