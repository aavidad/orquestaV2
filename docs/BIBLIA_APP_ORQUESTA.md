...
- antes de cada tanda operativa debe usarse una preparación canónica del daemon. El camino oficial es `orquesta server preparar-sesion`: reinicio limpio del servidor y purga segura de residuos terminales (`runtime_handles` cerrados/fallidos y `runtime_orders` terminales antiguas) del proyecto
...
- cuando el operador ya conoce manualmente la identidad OAuth real de un agente nuevo, Orquesta debe permitir fijarla por la misma vía canónica del servidor, sin esperar a que exista `runtime_handle` o `presupuesto_sesion`. El camino oficial es `agente observar-cuenta <agente> --email ... [--usuario ...]` o `POST /api/agentes/{agente}/observar-cuenta`; esa observación queda persistida como respaldo de `cuenta observada`, no como cuota ni telemetría de presupuesto
...
- cuando el operador pida saldo "al momento", el camino canonico es `agente presupuesto --refresh [--agente]` o `POST /api/agentes/presupuesto/refrescar`. Ese refresh no consulta fuentes externas arbitrarias: reutiliza la observacion viva del daemon sobre `runtime_handles` y persiste el snapshot canonico antes de listar
- ese refresh bajo demanda solo puede prometer verdad fresca si existe runtime observable gestionado por Orquesta. Si el agente consume saldo fuera de la app o no tiene handle visible, Orquesta debe devolver la mejor telemetria observada disponible y marcarla como `stale`; nunca puede inventar una verdad "actualizada" que no ha observado
- para runtimes Claude compatibles con `claw-code-dev-rust`, Orquesta puede observar por la via canonica del launcher Rust dos cosas: identidad OAuth (`credentials.json`) y uso/coste acumulado de la sesion local (`.claude/sessions/*.json`). Eso sirve para refrescar `cuenta` y `uso observado`, incluso sin `runtime_handle` activo si existe una ultima sesion conocida con `cwd` util.
- esa via Rust no puede inventar una cuota restante semanal o temporal si el launcher no la publica. En Claude, `uso observado` y `credenciales validas` no equivalen a `saldo restante`. Si no existe fuente real de cuota, Orquesta debe conservar la ultima cuota fiable conocida o dejar la cuota en modo derivado/estimado; nunca convertir coste acumulado en balance restante.
- `claude_rust_session_observed` es por tanto una fuente de observacion auxiliar y canonica para identidad/uso, no un reemplazo magico de `weekly/5h`. Si existe una cuota observada previa mas fuerte, esa cuota sigue mandando en el bloqueo visible mientras el snapshot Rust aporta solo frescura de cuenta/uso.
- un `cooldown` o `reanimar_at` derivado de telemetria observada no puede quedarse ciego hasta "dentro de dias". El daemon debe revalidar al menos cada hora a los agentes bloqueados o con telemetria `stale`, para levantar el bloqueo en cuanto aparezca evidencia nueva
- antes de reanimar un agente o asignarle trabajo nuevo, Orquesta debe hacer preflight de presupuesto por la misma via canonica del daemon. Si tras ese preflight el bloqueo sigue siendo visible, la reanimacion/asignacion se rechaza
- la capa visible debe distinguir `bloqueo confirmado` de `bloqueo estimado`: si la cuota sigue bloqueando pero la telemetria observada está `stale` o sin `checked_at`, el estado visible no puede presentarlo como verdad firme; debe marcarse como bloqueo estimado hasta revalidación
...
- en autonomia operativa, el `SupervisorAgente` configurado debe resolverse de forma case-insensitive solo para llegar al nombre canonico persistido; una vez resuelto, toda activacion, sesion, runtime handle y asignacion debe operar con el nombre canonico y no con la cadena original de la policy
...
- `worktree_drift` debe promocionarse a acción prioritaria del supervisor antes que reservar más backlog libre; una base vieja es un riesgo mayor que quedarse sin otra tarea reservada
...
- una vez encolada esa guidance durable, OpenClaw no debe repetir `revisar_worktree_desfasada` en la cola hasta que la deuda se drene o cambie el estado de la worktree
...
- `server start` tampoco debe dar el arranque por bueno solo porque `healthz` y `statefile` ya existan: la API server-first mínima (`/api/status`) tiene que responder con JSON decodificable antes de declarar el daemon listo.
...
- la jerarquía canónica de presupuesto para Codex es: primero `semanal`, luego `5h`. Si `semanal=0`, el agente queda agotado aunque `5h` siga teniendo saldo; la ventana corta deja de importar. Solo si `semanal>0` se consulta `5h`, y esa ventana corta pasa a mandar operativamente mientras siga viva.
- si la cuenta no expone ventana temporal, Orquesta no puede inventarla ni derivarla como si fuese real. En ese caso la única verdad operativa es `semanal`, y tanto la cuota efectiva como el reset visible deben salir de esa ventana.
- la `ventana efectiva` no es un mínimo ciego entre todas las cifras visibles. Debe respetar esa jerarquía: `semanal` decide si el agente sigue en juego; `5h` decide si puede trabajar ahora mismo; y las derivadas diaria/semanal solo rellenan cuando falta observación más rica. Si faltan snapshots ricos, se puede derivar, pero debe quedar claro qué ventana manda.
- la proyección visible del agente debe obedecer esa misma jerarquía. Si la `ventana efectiva` queda en `weekly=0`, el agente pasa a `agotado`; si la semanal sigue viva pero la `5h` cae a `0`, el agente pasa a `enfriamiento`. No puede seguir apareciendo como `activo` solo porque la sesión o el runtime sigan vivos.
- `handoff_preventivo` no es un bloqueo visible. Significa “sigue usable pero conviene relevarlo pronto”; no puede proyectarse como `estado_cuota=enfriamiento`, ni sacarlo de `status`, `/api/status` o `/api/agentes/presupuesto?activos=true`.
- la CLI remota de `status` no puede “rescatar” agentes desde el campo compat `agentes` cuando el servidor ya ha enviado `agentesActivos: []`. El fallback de compatibilidad solo aplica si ese campo no existe; si existe vacío, esa vaciedad es la verdad del servidor.
- en `status`, `/api/status` y superficies derivadas, `AgentesActivos` significa `conectados y disponibles`, no `sesión viva a cualquier precio`. Un agente con `Activo=true` pero `EstadoCuota!=activo` sale del bloque de conectados y se muestra en `En enfriamiento/cuota`.
- `status` y sus filtros de agentes activos no pueden usar una inspección local laxa basada solo en `heartbeat`. La fuente visible canónica son las `sesiones operativas` validadas con runtime/handle; si no hay sesión operativa, no se resucita al agente por compatibilidad.
- del mismo modo, `En progreso ahora mismo` no puede mezclar trabajo ejecutable con trabajo retenido por cuota. Si una tarea ya aparece en `Retenidas por cuota`, desaparece del bloque de ejecución presente para no duplicar ni fingir actividad.
- incluso con telemetría observada `stale`, si el presupuesto visible deja evidencia suficiente de bloqueo (`PresupuestoEstado=agotado`, `RemainingCredits<=0` o semanal visible `0%`), el agente sale del pool y se proyecta como `agotado` o `enfriamiento` en la vista visible.
- cuando exista esa evidencia suficiente de bloqueo observado, el `efectivo` visible también debe caer a `0` en la ventana correcta. No se puede mantener un `efectivo` derivado alto mientras el agente ya aparece retenido por cuota/cooldown.
- cuando un agente está fuera del pool por cuota y existe `reanimar_at`, la vista debe empezar por el `cooldown hasta ...`. Ese dato operativo manda sobre porcentajes derivados o telemetría stale y evita leer como contradictorio un `weekly 95%` junto a una pausa real todavía vigente.
- si `reanimar_at` persistido ya caducó o no llega, pero la cuota visible conoce un reset futuro (`presupuesto_reset_at` o reset de alguna de sus ventanas), la vista debe seguir mostrando `cooldown hasta ...` con ese mejor reset visible. No se deja a un agente en `enfriamiento` o `agotado` sin horizonte temporal cuando el presupuesto ya lo conoce.
- la reanimación automática no puede limpiar `estado_cuota` ni reactivar a ciegas cuando vence `reanimar_at`. Antes de volver a `activo`, Orquesta debe recalcular la cuota visible; si la `ventana efectiva` sigue en `0%`, el cooldown se sostiene hasta `PresupuestoResetAt` y no se encola ni `resume` ni `start`.
- el planificador automático no puede repartir trabajo por “primera tarea libre” cuando eso crea solapes tontos. Si hay varias tareas equivalentes, debe:
...
- además del briefing general, Orquesta debe exponer una cola estrecha de revisión/integración para el supervisor (`OpenClaw`) con gates y señales recientes, de forma que pueda arbitrar aceptación y merge sin tragarse todo el contexto general cada vez.
- esa cola estrecha también debe incluir solicitudes de merge vivas (`pendiente`, `validando`, `aprobado`, `ejecutando`, `fallido`) para que el supervisor vea en una sola lectura si un gate ya desembocó en integración o si todavía falta arbitrar ese paso.
- cuando existan dos frentes activos que pisan el mismo `módulo` con agentes distintos, la cola estrecha del supervisor debe señalar ese riesgo de colisión antes de integrar nada.
- además del recurso y el prompt en Markdown, Orquesta debe exponer esa misma cola de revisión del supervisor como herramienta MCP estructurada en JSON para que `OpenClaw` pueda automatizar decisiones sin parsear texto libre.
- esa salida estructurada debe incluir también `recommended_actions` derivadas del estado vivo: una sugerencia de acción siguiente por gate, señal, merge o colisión, para acercar a `OpenClaw` al papel de jefe técnico sin inventar una segunda lógica de negocio.
- además debe exponer una `next_action` única y una `action_queue` ordenada, para que el supervisor pueda actuar con la menor interpretación posible sobre el caso más urgente.
- un MCP “completo” para Orquesta no es solo lectura: debe cubrir también acciones canónicas de revisión e integración desde el mismo servidor, al menos para listar/resolver `review_gates` y listar/guardar `git_merges`.
...
- El store `.clawd-agents` del launcher Rust de Claude puede alimentar Orquesta, pero no sustituye la BD:
  ...
  - la ruta del store debe resolverse por:
    - `CLAWD_AGENT_STORE`
    - `openclaw_claude_subagent_store`
