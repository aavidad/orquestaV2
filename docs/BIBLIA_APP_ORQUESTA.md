...
- `session_candidates` y `observed_agent_sessions` no pueden contradecir al `status` canónico: si existe sesión operativa viva para el agente, la proyección observada debe marcarlo como activo aunque el flag persistido del agente aún no se haya refrescado
