...
| BUG-ORQ-20260710-208M | una ola local creada por `codex-launch-wave` mantiene su proceso Codex vivo, pero `codex-wave-status` y `codex-wave-tail` devuelven `blocked_registry_untrusted`; la observacion gobernada no puede confirmar ni recuperar una ejecucion propia | [incidencia 208M](incidencias/incidencia_orquesta_codex_wave_registry_untrusted_2026-07-10.md): revisar la cadena de `process_proof`/registro, reproducir con runtime aislado y no declarar autonomia mientras el monitor falle |
