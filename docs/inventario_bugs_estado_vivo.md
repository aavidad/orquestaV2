...
| BUG-ORQ-20260711-208AA | en verificacion E2E: un goal real termino con codigo y tests focales verdes, pero el attestor carecia de toolchain/cache y despues de `PATH`; Orquesta trataba errores operativos del attestor como test rojo y lanzaba rework inutil | integrados preflight, snapshot de modulos verificado, `PATH` minimo desde comandos declarados y clasificacion/replay de infraestructura sin rework; falta cierre API `accepted`. Evidencia en la [incidencia 208AA](incidencias/incidencia_orquesta_attestor_toolchain_infra_lanza_rework_2026-07-11.md) |
| BUG-ORQ-20260711-208AC | en verificacion E2E: el observador residente reutilizaba un timeout de dos segundos y cancelaba la atestacion independiente antes de que un test Go real terminase | separada la ventana durable del residente (15 minutos) de la respuesta HTTP acotada; falta E2E con dos recibos y cierre `accepted`, y queda como mejora estructural un scheduler concurrente de atestaciones. Evidencia en la [incidencia 208AC](incidencias/incidencia_orquesta_attestor_cancelado_por_timeout_observador_2026-07-11.md) |
