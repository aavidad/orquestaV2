...
| BUG-ORQ-20260811-710 | abierto; rama preservada y no integrable directamente | Agente MicroVM/backend Docker, lifecycle físico compartido | la rama remota `trabajo/backend-docker` implementó un backend funcional en paralelo a Firecracker, pero además del adapter específico añadió un segundo puerto de 17 operaciones, servicio de lifecycle/recovery, API y persistencia SQLite propios; mientras tanto `main` avanzó el ejecutor físico neutral y B12 por otra línea | el DAG ordenaba Docker después del primer Codex Firecracker, pero la rama partió antes de ese cierre y modeló el aislamiento como servicio hermano completo en vez de converger primero en una autoridad neutral única | la rama limpia y publicada `agente_microvm@2ead546` tiene 41 commits propios desde `74cb1d3`; `main@963d145` tiene 30 propios y trabajo físico activo. El canary Docker 29.5.2 acredita launch, replay, sesión terminal y cleanup, por lo que el trabajo se conserva; la comparación muestra `MotorContenedorV1` paralelo a `MotorMicrovm`, `ServicioContenedoresV1` y repositorios propios dentro de unas 12.900 líneas añadidas | no rebasear ni fusionar mientras `main` esté activo; estabilizar el ejecutor/lifecycle neutral, extraer los contratos comunes una sola vez y portar solo Engine, aislamiento, identidad y traducción Docker específicos, conservando los smokes de ambos backends |
