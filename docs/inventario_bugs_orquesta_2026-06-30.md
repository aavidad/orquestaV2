...
| BUG-ORQ-20260725-466 | abierto | V23/capability provisioning microVM para `TestAttestor` | Firecracker 1.16.1 y jailer root `0755` existen, pero faltan imagen guest mínima sellada, cgroup/jailer delegado acreditado y ACL vsock | presencia de binario no es proveedor operativo | evidencia host 2026-07-25 y decisión dual; KVM exige RW, vsock no tiene ACL | frente activo y acotado: sustituir solo Bubblewrap en `TestAttestor` para recuperar V23 mediante KVM RW, imagen mínima, cgroup/jailer fail-closed, virtio-serial sin red y smoke boot/ejecución/cleanup; no fallback intra-atestación ni runtime general de agentes |
| BUG-ORQ-20260725-467 | abierto, diferido | arquitectura/runtime general de agentes y almacenamiento efímero | todavía no se ha decidido ni acreditado migrar agentes a Firecracker, ejecutar workspaces/rootfs/caches de agentes en RAM/tmpfs o introducir checkpoints frecuentes | una microVM de `TestAttestor` de V23 no demuestra que el runtime completo de agentes tenga contratos de aislamiento, coste, persistencia y recovery adecuados | deuda explícita de la decisión `docs/decision_atestacion_bubblewrap_microvm_2026-07-25.md`; no hay estado implementado ni evidencia de proveedor general | no abrir antes de la cadena obligatoria `TestAttestor Firecracker -> recuperar V23 -> Orquesta autoprogramable`; entonces evaluar capability separada, contratos de checkpoint/restart y pruebas de composición |
