...
- `BUG-ORQ-20260704-166` queda reabierto tras reintento real Sueldos: Orquesta
  ya maneja el checkpoint `invalid`, pero no reconcilia un
  `orquesta_goal_result.v0` durable `complete` existente en el write-set cuando
  el runtime sigue publicando `goal_status=running`; la API devuelve
  `partial_artifacts_written` y no cierra el run.
- Avance 2026-07-04: `BUG-ORQ-20260704-165` queda parcialmente reducido para
  ...
  reconciliacion goal-first completa.
...
| BUG-ORQ-20260704-166 | abierto | Nueva App/goal-first checkpoint invalido y recibo durable no reconciliado | al crear `/home/alberto/Trabajo/Sueldos/generated-apps/mapa-de-gasto-publico` via `POST /api/v0/apps/director` con `director_execution_mode=goal_first`, Orquesta acepto el run y lanzo el goal, pero primero solo materializo `checkpoint_started.txt` y un `orquesta_goal_result.v0` con `status=invalid` y `missing_refs=[source_tree,handoff_report,technical_stack_manifest,go_app,tests]`; en el reintento real del 2026-07-04 ya existe un `orquesta_goal_result.v0` durable `status=complete`, `artifact_refs=3`, `artifact_paths=32`, `materialized_artifacts=3`, `missing_refs=[]` y test `passed`, pero `/api/v0/apps/director/goal/observe` devuelve `estado=error`, `run_status=activa`, `goal_status=running`, `closure_status=blocked`, `summary=codex_app_server_goal_status_active`, `partial_artifacts_written` y `codex_goal_observation_rejected` | la ruta de nueva app goal-first ya cubre el checkpoint invalido, pero la reconciliacion de recibos durables terminales no gana frente al estado runtime `active/running` o queda degradada por la proyeccion de artefactos parciales; tras `codex_app_server_tmux_pane_exit_timeout`, el observer debe aceptar el recibo `complete` si el write-set valida, o devolver `codex_app_server_runtime_write_set_violation` con paths accionables si el guard lo bloquea | `docs/incidencia_sueldos_goal_first_app_invalid_checkpoint_2026-07-04.md`; run `run-spec-mapa-de-gasto-publico-req-mapa-de-gasto-publico-43c20b877af23ecc450d324a25c1e616`; goal `goal-ref-app-director-run-spec-mapa-de-gasto-publico-req-mapa-de-gasto-publico-43c20b877af23ecc450d324a25c1e616`; external goal `019f2a37-df1f-7e00-a60b-dab168d79d38`; request `request-ref-sueldos-gasto-publico-osm-observe-retry-20260704-001`; fichero `/home/alberto/Trabajo/Sueldos/generated-apps/mapa-de-gasto-publico/docs/orquesta_goal_result_goal-ref-app-director-run-spec-mapa-de-gasto-publico-req-mapa-de-gasto-publico-4.json`; tests previos `TestObserveAppDirectorGoalV0LanzaReworkGoalSiNuevaAppSoloCheckpointInvalidV0`, `TestServerCodexAppServerGoalBackendV0NormalizaMissingRefsRaizEnChecklistV0`, `TestStackGoalMaterializedRefsSourceV0NormalizaMissingRefsRaizEnReceiptDurable` | Reabrir reparacion: mantener lo ya arreglado para `invalid`, anadir caso de campo donde existe recibo durable terminal `complete` bajo write-set y el runtime sigue `running`; `observe_goal` debe fusionar el recibo y cerrar `goal_status=complete/run_status=cerrada/closure_status=accepted`, o bloquear con `runtime_write_set_violation` estructurado. Hasta entonces el caso Sueldos queda bloqueado y el workaround manual no cuenta como entrega final Orquesta |
...
| BUG-ORQ-20260701-065/088 | mixto: BUG-065 abierto; BUG-088 cerrado funcionalmente | Director stats/status publico | actualizacion 2026-07-03: `orquesta.director.stats.v0` ya publicaba `external_job.evidence_refs/diagnostics` y `goal.evidence_refs/issue_codes`, pero el descriptor compacto no declaraba esos campos | clientes MCP/HTTP compactos podian consumir `director.stats` como si no transportase evidencias de cleanup, alto consumo, receipt o diagnosticos external-work, perdiendo contexto antes de decidir `runs/control` o rework | test `TestMCPDirectorStatsToolDescriptorV0ExponeContratoCompacto`; `go test -count=1 ./modulos/orquesta-mcp -run 'TestMCPDirectorStatsToolDescriptorV0ExponeContratoCompacto'` | Avance: el descriptor declara `external_job?{status,status_reason?,issue_refs?,evidence_refs?,diagnostics?}` y `goal?{goal_ref,status,closure_status?,issue_codes?,evidence_refs?}`; `docs/contratos.md` alinea el contrato con esos campos. Estado vigente: BUG-065 permanece abierto para coordinacion automatica completa; BUG-088 quedo cerrado funcionalmente por el smoke real alto consumo/checkpoint |
| BUG-ORQ-20260701-065/088 | mixto: BUG-065 abierto; BUG-088 cerrado funcionalmente | Run queue/status publico | actualizacion 2026-07-03: `orquesta.run_queue.priority.v0` transportaba `evidence_refs` en candidatos rankeados, terminales y actualizados, pero el descriptor compacto no lo declaraba | consumidores compactos de cola podian perder la evidencia causal de rescate, supersedes, cleanup o decision antes de priorizar o coordinar `runs/control` | test `TestMCPRunQueuePriorityDescriptorV0DeclaraEvidenciaDeCandidatos`; `go test -count=1 ./modulos/orquesta-mcp -run 'TestMCPRunQueuePriorityDescriptorV0DeclaraEvidenciaDeCandidatos'` | Avance: el descriptor y `docs/contratos.md` declaran `evidence_refs?` en `ranked`, `terminal` y `updated`, conservando la evidencia de prioridad/cola sin cambiar reglas de scheduling. Estado vigente: BUG-065 permanece abierto para coordinacion automatica completa; BUG-088 quedo cerrado funcionalmente por el smoke real alto consumo/checkpoint |
| BUG-ORQ-20260701-066/088 | mixto: BUG-066 abierto; BUG-088 cerrado funcionalmente | External work/status publico | actualizacion 2026-07-03: `orquesta.external_work.run.v0` transportaba `external_goal_ref` y `evidence_refs`, pero el descriptor compacto solo declaraba `goal_ref`, `next_actions` y endpoints | consumidores de trabajo externo podian arrancar un goal-first y perder en discovery la evidencia/goal externo necesaria para observar, reconciliar alto consumo o coordinar `runs/control` sin mirar logs | test `TestMCPExternalWorkRunDescriptorV0DeclaraLegacyExplicito`; `go test -count=1 ./modulos/orquesta-mcp -run 'TestMCPExternalWorkRunDescriptorV0DeclaraLegacyExplicito'` | Avance: el descriptor y `docs/contratos.md` declaran `external_goal_ref?` y `evidence_refs?` en la salida ok de `external_work.run`; no cambia routing ni toca OPES productivo. Estado vigente: siguen pendientes criterios OPES done/settled y coordinacion automatica de BUG-066; BUG-088 ya no aporta pendiente de smoke real |
| BUG-ORQ-20260701-066/088 | mixto: BUG-066 abierto; BUG-088 cerrado funcionalmente | Arranque director/status publico | actualizacion 2026-07-03: `orquesta.apps.arrancar_director.v0` transportaba `external_goal_ref` y `evidence_refs`, pero el descriptor de arranque solo anunciaba `goal_ref/goal_status` | clientes que arrancan apps goal-first podian perder en discovery el goal externo y la evidencia inicial necesaria para observar, continuar o reconciliar estados de alto consumo/shutdown | test `TestMCPAppSpecDescriptorsV0PublicanRoutePolicy`; `go test -count=1 ./modulos/orquesta-mcp -run 'TestMCPAppSpecDescriptorsV0PublicanRoutePolicy'` | Avance: el descriptor declara `external_goal_ref?` y `evidence_refs?` en ok/error, y `docs/contratos.md` documenta evidencia compacta de arranque/receipt. Estado vigente: siguen pendientes criterios OPES done/settled y coordinacion automatica de BUG-066; BUG-088 ya no aporta pendiente de smoke real |
