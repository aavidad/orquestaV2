...
| BUG-ORQ-20260806-700 | abierto; contrato, modelo, señal física y esquema durable sibling publicados; transacciones productivas pendientes | Agente MicroVM/B11, detención física y reconciliación | la API sibling exponía una única detención sin modo y devolvía solo la ejecución actualizada; Orquesta no podía distinguir cooperativa de forzada ni obtener `receipt_ref`, instante físico confirmado y replay causal exacto | el motor ya producía un comprobante de retirada de recursos, pero el servicio y el ledger lo descartaban; la idempotencia conservaba resultado lógico, no el recibo físico | `agente_microvm@daf8469` fija el contrato público; `agente_microvm@db1b167` modela el ledger causal; `agente_microvm@767f9e8` añade una capacidad `signal-only` cercada que emite solo `TERM` cooperativa o `KILL` forzada, distingue `Entregada/YaAusente` y no espera, escala ni retira recursos; `agente_microvm@63d86c9` añade el esquema SQLite `STRICT` v17 y conserva replay/migración segura desde v16. Las cuatro piezas pasaron revisión premium y suites completas, pero la ruta productiva todavía no ejecuta las transacciones del ledger nuevo: faltan métodos SQLite, servicio/recovery, API/clientes y replay físico | no anunciar capacidad B11 hasta demostrar que cooperativa no escala por sí sola, forzada sí puede hacerlo, respuesta perdida reconcilia el mismo recibo y restart no fabrica timestamps ni autoridad |
