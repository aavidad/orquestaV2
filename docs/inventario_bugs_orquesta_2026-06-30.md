...
| BUG-ORQ-20260726-556 | cerrado en candidato/prueba; reinicio pendiente | perfil de servidor Orquesta y artefacto candidato | el perfil TOML y el subcomando `serve` eran correctos; el candidato `b655` se compiló por error desde `cmd/orquesta-server` en vez de la composición productiva `cmd/orquesta`, por lo que el artefacto no exponía el contrato esperado | la procedencia del binario candidato no quedó ligada a la raíz de composición antes de validar el profile; el fake no era causa del fallo y no debe modificarse para ocultarlo | `863fbf6a` añade guard de composición; el binario correcto `e949`, construido desde `cmd/orquesta` con `vcs.modified=false`, tiene SHA-256 `cdd44446…` | conservar el guard y verificar argv/artefacto real tras reinicio antes de declarar el perfil operativo |
