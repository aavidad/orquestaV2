...
| BUG-ORQ-20260725-462 | abierto; mitigación operativa preparada | V23/atestador bubblewrap y arranque por perfil | el atestador intentó capturar un árbol de 8.495 ficheros con `LimitNOFILE=8192` y `max_concurrent_runs=2`; su presupuesto máximo era de unos 4.048 descriptores por snapshot, por lo que abortó antes de ejecutar `go test` | el preflight solo acredita un snapshot vacío y el arranque por perfil no contrasta el límite heredado de descriptores con el número de entradas del HEAD autorizado; cada blob se mantiene en un `memfd` hasta cerrar el snapshot | Goal `goal:62581fb382dd39f57f63698aba7a1c89`, change-set `change-set:84a60306e912250d37ec19edb0d5194d`, acción `action:attest-test:execution:dbb5b561a95133dbfaf303a442eba392`; fórmula canónica `(soft-open-64)/concurrent-16`, cero proceso de test y prueba focal directa de `internal/intake` verde | arrancar la unidad con `LimitNOFILE=32768`, impedir que el wrapper inicie un daemon bubblewrap sin capacidad para el HEAD y diseñar el preflight durable previo al `EffectAttempt`; no reusar la acción ya puesta en cuarentena |
