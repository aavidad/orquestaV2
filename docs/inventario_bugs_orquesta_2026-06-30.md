...
| BUG-ORQ-20260806-693 | abierto; gate transversal | arquitectura reconstruida | el gate completo `TestRebuildArchitecture` falla por imports adapter -> `internal/adapters/protocol/codexwork` en `agentmicrovm/codexwork` y por `os.Environ` en `runner_linux_test.go` | observación del gate actual; no atribuida a PFC-04/PFC-06/PFC-07 ni a esta mini | fallo reproducible en el gate completo actual; no se modificó código | clasificar y corregir en write-set propio; no usar este cierre documental como resolución |
