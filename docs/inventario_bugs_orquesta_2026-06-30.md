...
| BUG-ORQ-20260728-586 | abierto; corrección en curso | snapshots Git para atestación | la primera atestación real V23.3 rechazó como `test_attestor.snapshot_stream_invalid` un changeset válido antes de invocar Firecracker | `snapshotTreeEntries` exigía orden lexicográfico Go sobre la salida `git ls-tree -r -t`, pero el orden canónico de Git compara un árbol como si su nombre terminase en `/`; la secuencia válida `cmd/orquesta-server/fichero`, árbol `cmd/orquesta`, `cmd/orquesta/fichero` quedaba falsamente desordenada | reproducción exacta sobre el árbol `091924765f1b5ae4df857b8eaf40c7e896ec2091`: primer falso descenso en la entrada 644, de `cmd/orquesta-server/worktree_snapshot_budget_env_v0_test.go` a `cmd/orquesta`; marcadores de workspace/changeset, commit, tree, diff digest, 9.333 entradas y 205.299.726 bytes eran válidos y estaban dentro de los límites 200.000/512 MiB | comparar con la clave canónica de Git —ruta más `/` para árboles, ruta literal para blobs— y añadir casos de colisión directorio/guion y negativos de desorden/duplicado sin relajar modos, paths, gitlinks, límites ni hashes; repetir snapshot real y atestación microVM |
