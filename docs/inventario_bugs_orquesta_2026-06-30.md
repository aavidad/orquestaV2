...
| BUG-ORQ-20260805-660 | abierto; C2 en corrección, sin commit | PFC-04, credencial Codex dentro del huésped | el primer C2 superó 30 tests, Clippy y build musl, pero podía bloquear PID 1 durante `connect`, emitir un ACK de purga desde `Drop`, ignorar una purga fallida y permitir que la ruta legacy `Ejecutar` leyera `auth.json` con el mismo UID 1000 | los seams cubrían protocolo y proyección lógica, pero no ownership completo del canal/montaje ni exclusión entre las dos superficies de ejecución del huésped | revisión independiente NO-GO P0 antes de commit; se detuvo una búsqueda amplia del revisor que había quedado viva y no se arrancó KVM | exigir conexión no bloqueante acotada, `Drop` sin ACK, purga explícita fatal si queda ambigua, detección de EOF, exclusión de ejecución legacy y pruebas negativas antes de integrar C2 |
