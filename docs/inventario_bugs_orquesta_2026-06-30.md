...
| BUG-ORQ-20260725-453 | cerrado | V23/control de acciones en atestación | cancelar una acción `awaiting_attestation` o `quarantined` devolvió `internal/sqlite.action_retirement_state_invalid`; la reproducción determinista adicional mostró que cancelar después del `effect_attempt` y antes del completion fabricaba `completed/application.action_retired` para un efecto posiblemente aplicado; la auditoría independiente reabrió un P1: tras crash/restart y expiración del claim, el receipt de cancelación violaba la lease antigua y el reclaim legítimo volvía ambiguo el intento ya durable | `RetireActionRefs` no distinguía dentro de la transacción SQLite entre una atestación aún no iniciada y un intento físico ya durable; el primer cierre conservaba la cuarentena, pero intentaba consumirla con la identidad/lease expirada y exigía que el fence del intento coincidiera con el fence vigente después de un reclaim | `4056c98e` y `52ba5d3a` cierran el intercalado inicial; `ee377fb3` toma control causal de la atestación expirada con token/worker de sistema, lease mínima nueva y `delivery_attempt` incrementado, conserva el fence vigente y el único `effect_attempt` histórico (`attempt.fence <= action.fence`), y bloquea completion/re-ejecución con la identidad vieja; `50a1d43a` cubre crash/restart, expiración inclusiva, cancel/quarantine/recovery, reclaim intermedio, completion tardío, intento de reejecución y ausencia de segundo efecto. Focal normal y `-race`, suite SQLite completa y `go vet` verdes | conservar el CAS de takeover, la consulta agregada total/causal y los negativos de tamper; una atestación con intento físico nunca puede convertirse en retiro ordinario ni recuperar autoridad física, y la cuarentena existente no se reescribe |
