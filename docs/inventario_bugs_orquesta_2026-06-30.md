...
| BUG-ORQ-20260630-023 | en investigacion | Shutdown/runtime real | en reintento OPES goal-first, `/server/shutdown` devolvio `shutdown_ready=true` pero el proceso no salio o el cliente agoto timeout con goal activo | shutdown cooperativo, hooks app-server tmux y estado HTTP pueden divergir en ejecucion real larga | `TAREA_OPES_ORQUESTA_GOAL_LAUNCHER_TMUX_EXITED_REVISION100_2026-06-29.md`; tests offline `TestRuntimeV0ServerShutdownReadyDetieneRuntimeHTTPV0`, `TestCodexAppServerTmuxBackendV0ShutdownMataSesionPropiaV0`, `TestRuntimeV0ShutdownTimeoutPublicaStopTimeoutV0` | Reproducir con Orquesta temporal goal-first + app_server_tmux y, si falla, fijar smoke real/timeout accionable sin tocar produccion |
