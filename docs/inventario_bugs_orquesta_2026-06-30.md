...
| BUG-ORQ-20260724-402 | cerrado en adaptador Codex V22; transporte goal-first separado en BUG-ORQ-20260725-459 | V22/selección de razonamiento por WorkItem | `AgentLaunchRequest` conservaba `reasoning_effort` por tarea, pero el adaptador Codex construía el proceso con el esfuerzo global de composición; una tarea simple no podía rebajar de forma efectiva el esfuerzo ni una delicada elevarlo por contrato individual | la metadata de gobierno y la selección física del proceso pertenecían a capas distintas y no existía un puente tipado por launch | `7401dcf2` proyecta el esfuerzo de cada request en el argv y lo persiste en V7; `d80e3fff` impide atribuir a V3–V6 el valor global vigente durante recovery y exige un intento V7 nuevo. Pruebas normales y `-race` del paquete Codex cubren `high↔xhigh`, reinicio, proceso vivo, cuarentena individual y retry V7 | conservar `reasoning_effort` como identidad inmutable V7 y no inferir historia ausente desde configuración actual; la propagación desde Goal-first sigue gobernada por BUG-ORQ-20260725-459 |
...
| BUG-ORQ-20260725-452 | abierto | V23/atestador y runtime independiente | un fallo real se publicó como estado genérico, sin causa útil para repetir o reparar; la primera caída del atestador sí quedó explicada por `NOFILE`, pero las de r2/r3 no. El E2E HOME del 2026-07-25 repitió el defecto: Orquesta persistió `codex.process_failed`, mientras una sonda directa del mismo contrato de perfil mostró `refresh_token_reused` y token expirado | `processing` y el adaptador colapsan el error del backend y el receipt/journal conserva el estado genérico, perdiendo la causa que necesita el operador | pilotos V23 2026-07-24/25 y E2E real HOME con `Codex12`; el proceso recibió el HOME correcto y generó `terminal.json` V6, pero la causa de autenticación solo quedó visible fuera de Orquesta. Recurrencia 2026-07-25: Goal `goal:62581fb382dd39f57f63698aba7a1c89`, ejecución `execution:dbb5b561a95133dbfaf303a442eba392`; el ledger conservó solo `application.effect_unknown_applied`, aunque el cálculo reproducible identifica `test_attestor.snapshot_limit_exceeded` antes de ejecutar Go | conservar causa estructurada del backend, separar preflight sin efecto de la ejecución física, reproducir con runtime persistente y no declarar cierre por el estado genérico |
| BUG-ORQ-20260725-453 | abierto | V23/control de acciones en atestación | cancelar una acción `awaiting_attestation` o `quarantined` devolvió `internal/sqlite.action_retirement_state_invalid`; `pause` sí funcionó | la transición de retirada no cubre esos estados activos de atestación/cuarentena; no es un fallo de `pause` | piloto V23 2026-07-24/25; respuesta observada `internal/sqlite.action_retirement_state_invalid` al cancelar y `pause` operativo | definir y probar la transición de cancelación para ambos estados, conservando semántica y evidencia de `pause` |
| BUG-ORQ-20260725-454 | abierto | V22/atestador bubblewrap de Go | bubblewrap dio verde cuando Go informó `[no tests to run]` | el runner acepta solo el código de salida y no interpreta la salida semántica de Go; el atestador legacy ya contiene detector equivalente, V22 no | piloto V23 2026-07-24/25; contraste con `modulos/orquesta-runtime-required-test/local_command_executor_v0.go` del árbol legado | portar un detector contractual y pruebas negativas; no aceptar una ejecución vacía como evidencia |
...
| BUG-ORQ-20260725-459 | abierto | V23/Goal-first/routing de modelos | el WorkItem pidió `reasoning_effort=xhigh`, pero el proceso Codex se lanzó con `model_reasoning_effort="high"` | `GoalWorkSpecV0` y `CodexGoalStartPacketV0` no transportan el esfuerzo causal; el launcher Goal-first recalcula la ruta por Goal/coste y sobreescribe el valor, mientras la ruta legacy sí lo conserva en `AgentLaunchRequest` | Goal `goal:8233d70d7734ceadcef7df339988529b`, ejecución `execution:ba25b2762c383afad20fb276b6c26346` y argv observado del PID Codex. Símbolos focales: `modulos/orquesta-goal/types_v0.go`, `modulos/orquesta-runtime-codex-goal/packet_v0.go` y `cmd/orquesta-server/codex_goal_app_server_v0.go` | transportar y acreditar el esfuerzo autorizado WorkItem/EffectIntent → Goal spec → packet → router/runtime; añadir prueba E2E del valor exacto. La ejecución actual sigue autorizada porque `high` cumple la exigencia de no usar bajo esfuerzo para programación |
| BUG-ORQ-20260725-460 | abierto | V23/runtime Go aislado | el agente V23 quedó varios minutos sin progreso mientras `go test` intentaba descargar automáticamente el toolchain; el proceso hijo se terminó cooperativamente y no dejó residuales | el workspace aislado no tenía garantizado un toolchain/cache local compatible y `GOTOOLCHAIN=auto` convirtió una prueba focal en una operación de red no acotada | ejecución `execution:ba25b2762c383afad20fb276b6c26346`; el agente conservó su diagnóstico y revirtió el candidato al detectar además conflictos de autoridad | provisionar toolchain y caches aisladas antes del launch o desactivar la descarga automática cuando el runtime no garantice red; añadir timeout y evidencia estructurada |
...
| BUG-ORQ-20260725-467 | abierto, diferido | arquitectura/runtime general de agentes y almacenamiento efímero | todavía no se ha decidido ni acreditado migrar agentes a Firecracker, ejecutar workspaces/rootfs/caches de agentes en RAM/tmpfs, introducir checkpoints frecuentes o red privada de agentes | una microVM de `TestAttestor` de V23 no demuestra que el runtime completo tenga contratos de aislamiento, coste, persistencia, recovery y egress adecuados | deuda explícita de la decisión `docs/decision_atestacion_bubblewrap_microvm_2026-07-25.md`, formalizada en `docs/arquitectura_comunicacion_agentes_firecracker_2026-07-25.md`: host filesystem/LAN inaccesibles, sin east-west/direct peer, colaboración exclusiva por gateway Orquesta autenticado + mailbox causal + CAS/artefactos por refs opacas y ACL Goal/parentesco; egress solo gateway/proxy, sin inbound, con bloqueo de host loopback/RFC1918/ULA/link-local/metadata/SSRF/puertos y NAT solo netns+nftables; no hay estado implementado | no abrir antes de la cadena obligatoria `TestAttestor Firecracker -> recuperar V23 -> Orquesta autoprogramable`; entonces evaluar capability separada, contratos de checkpoint/restart/egress y pruebas de composición; TestAttestor actual sigue sin red |
...
| BUG-ORQ-20260725-486 | abierto, bloqueo operativo | V23/toolchain guest Firecracker | build real demostró que `/usr/local/go` es Go 1.25.5 con `GOTOOLCHAIN=local`, aunque el repo declara `toolchain go1.25.11` y el Go habitual usa una copia auto del caché del usuario | la versión efectiva dependía de `GOTOOLCHAIN=auto`; no había toolchain 1.25.11 root-owned separado | imagen válida 1.25.5 `e007883f…` retenida como evidencia, no promovida; tar oficial 1.25.11 descargado y SHA-256 `34f143…ba30b` verificado | operador debe instalar `/srv/orquesta-self/toolchains/go1.25.11` root-owned con el comando entregado; reconstruir exacto y usar solo el nuevo digest |
...
| BUG-ORQ-20260726-496 | abierto, preparación corregida manualmente | V23/build de artefactos Firecracker, modos reproducibles | la reconstrucción declarada verde heredó `umask 0002` y publicó supervisor/launcher `0775`; el `--dry-run` los rechazó correctamente como `launcher_source_writable_by_group_or_world` | los bytes eran reproducibles, pero la preparación mecánica no normalizó metadata ejecutable ni la incluyó en su gate final | ambos binarios se normalizaron a `0755` sin cambiar SHA-256 y el dry-run posterior quedó verde; no llegaron a staging root con modo inseguro | fijar `umask 0022` o instalar outputs con modo explícito y hacer que el gate del build verifique dueño/modo/link count además de hashes |
...
| BUG-ORQ-20260726-500 | abierto; workaround de gate verificado | V23/tests launcher Firecracker, rutas UDS del harness | el gate focal en entorno sin `HOME` falló masivamente con `firecracker_launcher_unavailable`, aunque el mismo paquete quedaba verde en el entorno habitual | no existía dependencia de `HOME`: el harness anidó `TMPDIR` bajo una ruta larga del build y los sockets por test superaron el límite Linux `sun_path` de 108 bytes; `bind/connect` devolvían errores genéricos | el mismo gate, todavía sin `HOME` y con caches aisladas, queda verde al usar el temporal corto dedicado `/tmp/o7`; los artefactos se construyeron sin `HOME` y no incorporan esa ruta | centralizar un creador de temporales cortos, únicos y privados para tests UDS; conservar el gate sin `HOME` y no interpretar otra vez un `TMPDIR` largo como dependencia de identidad |
...
| BUG-ORQ-20260726-503 | aclarado; mejora de UX pendiente | tooling de perfiles Codex, aprobación interactiva | un agente arrancado manualmente con `codex-perfil` pidió confirmación repetida y parecía incompatible con una operación desatendida | `codex-perfil` aísla `HOME`/`CODEX_HOME` y reenvía argumentos, pero no decide sandbox ni aprobación; el runtime Orquesta sí proyecta `workspace-write` y `approval_policy=never`, mientras una TUI manual sin flags conserva su política interactiva | los doce perfiles informan login activo; para ejecución manual queda verificado `codex-perfil PERFIL -a never -s workspace-write ...`; no se recomienda el bypass total mientras Firecracker no sea la jaula efectiva | documentar la diferencia identidad/política y valorar un subcomando manual desatendido explícito; no cambiar silenciosamente el default interactivo ni duplicar la configuración canónica de Orquesta |
| BUG-ORQ-20260726-508 | cerrado | runtime Codex, migración V3–V6 a V7 y esfuerzo por ejecución | el primer cierre de esfuerzo por request podía recuperar una ejecución V3–V6 usando el esfuerzo global configurado en el reinicio y publicar con ello un binding V7; sus pruebas daban verde porque repetían esa inferencia, aunque el journal legacy nunca persistió el esfuerzo físico | se confundió una configuración actual con evidencia histórica inmutable y, en V3, se podía completar Goal/Work/generaciones desde la petición de replay pese a que el hash antiguo no los acreditaba | `d80e3fff` elimina toda creación productiva del binding legacy→V7: una ejecución viva se adopta y detiene por su identidad física durable, una ejecución sin proceso conserva terminal `execution_interrupted`, y el llamante recibe `legacy_execution_requires_new_attempt`; `Stop` V4–V6 funciona sin binding y V3 cuarentena con `legacy_control_metadata_unknown` sin escribir request/receipt de control sintéticos. Las pruebas cambian la configuración `high↔xhigh` antes del primer recovery, prueban dos procesos y stop individual, V3 con causalidad adversaria, ausencia de upgrade y retry V7 nuevo | no reconstruir campos ausentes ni desde defaults ni desde el replay; una futura migración solo podrá acreditar esfuerzo histórico mediante evidencia durable previa independiente, nunca por coincidencia con la configuración vigente |
