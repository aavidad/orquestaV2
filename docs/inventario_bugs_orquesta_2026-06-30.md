...
| BUG-ORQ-20260725-425 | solución en curso; pendiente commit/reseal/E | V22/E2E/orden de observación | el smoke con HEAD `4e2fc`/`ba5b`/`07ae` acreditó que mailbox A funcionó exactamente: `attempt=1`, `delivered=1`, `consumed=1`, ACK=1 y A terminó `succeeded` en revisión 11. Sin embargo, el harness esperó progreso de A y después de C antes de comprobar que el recipient seguía running; perdió la ventana de `sleep 20`, no creó backup ni restore, el servidor original continuó vivo y D perdió también su ventana running | la sincronización secuencial del fixture observaba demasiado tarde un estado transitorio que ya había ocurrido; no fue un fallo del producto, del contrato mailbox ni de adopción | solución en curso: observar y afirmar mailbox A inmediatamente después de B, reutilizar esa afirmación como progreso de A y esperar C solo después; ampliar la pausa determinista de A a 60 s, conservando D en 180 s | confirmar commit, reseal y E exacta; los estados transitorios deben observarse en el punto causal correspondiente, no después de esperas independientes |
