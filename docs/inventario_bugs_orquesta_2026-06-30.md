...
| BUG-ORQ-20260726-503 | abierto, P1 en adaptador V2; UX manual separada | tooling/perfiles y adaptador Codex nuevo, aprobación interactiva | un agente arrancado manualmente con `codex-perfil` pidió confirmación repetida; al revisar el runtime V2 se comprobó además que su argv productivo no fija explícitamente la política no interactiva | `codex-perfil` solo aísla `HOME`/`CODEX_HOME`; en V2 `internal/adapters/agent/codex` construye `codex exec` con sandbox pero sin `--ask-for-approval never`, mientras la afirmación anterior procedía del stack legacy congelado | el uso manual queda resuelto con flags explícitos y el adaptador V2 está en reparación xhigh con captura del argv real; no se da por seguro por defaults de CLI | cerrar solo cuando todas las rutas productivas, preflight y reanudación usen una política no interactiva explícita y los tests prueben que no dependen de configuración de usuario; Firecracker será otra frontera de seguridad, no sustituto de esta política |
