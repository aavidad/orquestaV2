...
| BUG-ORQ-20260805-662 | abierto; aislado como minitarea previa al broker | PFC-04, consumo físico one-shot de credencial | `credentials.Store.Use` reproduce una petición idempotente ejecutando otra vez el callback, por lo que usarlo directamente desde el broker permitiría una segunda entrega del mismo secreto | el store actual modela uso autorizado repetible, no consumo físico de un solo uso; el protocolo nuevo necesita semántica más estrecha y durable | auditoría de `internal/adapters/credentials/local/store_test.go` confirma el replay con callback; C4a se separó en contrato `UseOnce` y persistencia atómica antes de implementar el listener | acreditar consumo antes del callback y demostrar con replay, carrera y reapertura que callback fallido o exitoso no puede ejecutarse una segunda vez; no cambiar la semántica legacy de `Use` |
