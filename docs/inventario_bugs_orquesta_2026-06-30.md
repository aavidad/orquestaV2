...
| BUG-ORQ-20260729-599 | abierto; gate base reproducido | V21/i18n y aceptación acumulativa | `TestTypedExtractorFindsRegistryCatalogAndPublicDocsKeys` falla en el HEAD limpio porque el extractor tipado ya descubre las claves Wizard de dossier/gaps/catalog, pero la lista `actual` del ratchet V21 sigue limitada al catálogo previo | las superficies i18n de V23 crecieron sin actualizar atómicamente el inventario acumulativo que V21 usa como autoridad negativa | reproducción exacta: `go test -mod=vendor -count=1 ./acceptance -run '^TestTypedExtractorFindsRegistryCatalogAndPublicDocsKeys$'`; el fallo es anterior a `1e396b98` y no lo causa el codec | actualizar el ratchet desde la fuente tipada canónica, conservar negativos de literales/keys huérfanas y volver a ejecutar aceptación completa antes de sellar V23 |
| BUG-ORQ-20260729-600 | abierto; cancelación efectiva, proyección ilegible | Goals/cancelación temprana y lectura posterior | un Goal SQLite recién lanzado se canceló correctamente por una precondición SHA errónea: Goal, WorkItem y execution quedaron `canceled` y el control `confirmed`, pero `orquesta.goals.get` devolvió HTTP 500 inmediatamente después | por aislar; la proyección recuperada rechaza alguna evidencia residual del agente cancelado aunque el agregado y el control ya expresan un cierre coherente | Goal `goal:b7161aa4919e0560e19c741f8ab4a8ca`, execution `execution:47954e80c5abab8a177a4e5ad12f9dea`, control `control:170dbdd53d5913e06dcf26eceb7fc3c9`; se comprobó el cierre con SQLite de solo lectura y no se tocaron filas | añadir prueba de cancelación durante ejecución inmediatamente seguida de `GetGoal`, restart y recovery; una cancelación confirmada debe seguir siendo legible y no dejar proceso, sesión, workspace o evidencia fuera de contrato |
