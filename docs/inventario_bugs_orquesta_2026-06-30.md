...
| BUG-ORQ-20260726-527 | abierto; candidato de corrección pendiente | Firecracker/netns Linux | `openEmptyNetNamespace` exigía `/proc/net/ipv6_route` vacío y clasificó como `network_unsafe` un netns limpio en kernel 7.0 | kernel 7.0 crea dos sentinelas `::/0`, metric `ffffffff`, flags `0x00200200` (`RTF_NONEXTHOP|RTF_REJECT`) y dev `lo` incluso con interfaz down, sin dirección ni ruta netlink | E2E Firecracker falló como `network_unsafe`; candidato pendiente en `internal/adapters/attestor/firecrackerlauncher/netns_linux.go` usa parser bounded/fail-closed y tests de tabla | aceptar únicamente los dos sentinelas exactos; no declarar E2E real verde hasta ejecutar el candidato en host compatible |
...
| BUG-ORQ-20260726-544 | abierto; fail-closed, sin binding público | V23/Wizard, reconciliación de preguntas inmutables | una respuesta legítima a un padre puede cambiar recomendación o payload calculado de una pregunta ya persistida; la reevaluación V1 detecta la diferencia y devuelve conflicto en vez de reconciliar/versionar la pregunta | Intake conserva correctamente preguntas inmutables, pero el motor todavía no dispone de protocolo causal para sustituir, retirar o versionar una proyección derivada cuando cambian sus inputs | `internal/application/wizard_gaps_test.go#TestWizardGapsKeepsCausalReopenDespiteChangedProjectionConflict`; el preflight exacto evita corrupción y el servicio no está expuesto como comando | antes del binding público, diseñar reconciliación/versionado sobre el único Intake, sin segundo lifecycle/store y conservando decisiones, reapertura, replay y restart |
| BUG-ORQ-20260726-545 | cerrado por decisión pre-sello | V23/comando `orquesta.intakes.get` v1 | la salida añade campos opcionales de causalidad a objetos history bajo un schema con `additionalProperties:false`; un consumidor de un contrato ya sellado requeriría una versión pública nueva | el número v1 pertenecía al registro candidato V23, todavía no sellado, acreditado ni consumido externamente; mantener dos versiones antes del primer sello crearía compatibilidad ficticia | `24cb4114` conserva JSON histórico cuando derivation es cero y `c42b22b2` completa el mismo v1 candidato con todos los campos history que ya puede emitir el writer; handler real y commandgen quedan verdes | congelar este v1 completo al acreditar `AC-V23-WIZARD`; después del sello, cualquier ampliación incompatible con validadores estrictos exige versión pública nueva y ledger propio |

## Bugs cerrados del corte WIZ-20 causal del 2026-07-26

| ID | Estado | Área | Síntoma | Hipótesis arquitectónica | Evidencia/cierre | Acción |
|---|---|---|---|---|---|---|
| BUG-ORQ-20260726-546 | cerrado en candidato V23; pendiente sello | V23/comando `orquesta.intakes.get` v1 | una mutación con `QuestionsRevised=1` producía `history[].questions_revised`, pero el schema cerrado con `additionalProperties:false` no declaraba el campo y Dispatcher devolvía `output_contract_invalid` | dominio y contrato público candidato evolucionaron en revisiones distintas; el número v1 existía, pero V23 aún no estaba sellado ni tenía consumidor acreditado | `internal/commands/handlers_intake_test.go#TestIntakeCommandsBindAuthorityOutsidePayloadAndProjectPublicState` reproduce la salida real y el registro/generated declaran `questions_revised` y `questions_retired` | al no existir sello V23, completar v1 candidato antes de `AC-V23-WIZARD`; tras el sello, cualquier campo no aditivo compatible exige versión pública nueva |
| BUG-ORQ-20260726-547 | cerrado en candidato V23; pendiente sello | V23/Intake, identidad de derivación | `QuestionRevisions` aceptaba cualquier `DerivationIdentity` bien formada y permitía que otra versión/esquema reescribiera o restaurara una proyección ajena | se validaba forma/no-cero, no igualdad con la identidad causal de la última versión creadora | `internal/intake/question_revisions_test.go#TestQuestionRevisionRejectsDifferentValidCausalDerivation` cubre revisión, retiro y restauración con identidad sustituta; `Apply` rederiva identidad desde history/versiones append-only | conservar igualdad exacta de schema, versión y digest para toda transición de una pregunta; una evolución de evaluator no reclama refs históricas de otra identidad |
| BUG-ORQ-20260726-548 | cerrado en candidato V23; pendiente sello | V23/Wizard, ventana de reconciliación | una sola decisión reabierta habilitaba revisar todas las preguntas activas; un cambio simultáneo de `Facts.Surface` podía versionar una pregunta sin enlace causal con la decisión | la reapertura global se usaba como permiso de mutación y mezclaba inputs durables con facts/packs request-scoped | `internal/application/wizard_gaps_identity_test.go#TestWizardGapsReconciliationWindowRejectsUnrelatedFactsProjection` fija facts/packs y compara evaluator actual contra el contrafactual de la decisión anterior; solo diferencias posteriores a la versión quedan reconciliables | no persistir facts request-scoped por atajo ni confiar en flags globales; ampliar inputs durables solo con contrato tipado y replay propio |
| BUG-ORQ-20260726-549 | cerrado en candidato V23; pendiente sello | V23/Intake, retiro de preguntas derivadas | cuando el evaluator dejaba de emitir T1 por U1 `team -> personal`, T1 seguía activa; quitarla de la slice habría roto replay y restart, y reemitirla colisionaba con refs históricas | faltaba representar ausencia derivada como otra versión inmutable dentro del único Intake | `internal/intake/question_revisions_test.go#TestDerivedQuestionRetirementAndRestorationAreOneAppendOnlyVersionChain`, `internal/application/wizard_gaps_retirement_test.go#TestWizardGapsRetiresAndRestoresQuestionFromExactCausalEdge` y `internal/adapters/state/sqlite/intake_question_revisions_test.go#TestIntakeQuestionRetirementSurvivesRestartAndReplaysExactly` cubren tombstone, restauración, replay y restart | conservar tombstone `QuestionVersion.Retired` enlazado por `ReplacesRevision`; no crear tabla, store ni lifecycle paralelo y no dejar dependientes activos colgando |
| BUG-ORQ-20260726-550 | cerrado en candidato V23; pendiente sello | V23/WIZ-20, reapertura por revisión | revisar la pregunta padre invalidaba su propia respuesta por versión, pero decisiones hijas y nietas seguían actuales aunque dependían transitivamente del payload sustituido | la proyección aplicaba invalidación transitiva solo a cambios de respuesta, no a cambios de versión de preguntas | `internal/intake/dependencies_test.go#TestParentQuestionRevisionReopensOwnAndDependentDecisionsTransitively` demuestra padre, hijo y nieto reabiertos con la misma causa/revisión mientras una decisión independiente permanece válida | toda versión posterior invalida decisiones anteriores de la pregunta y de sus dependientes transitivos; decisiones posteriores o independientes se preservan |
