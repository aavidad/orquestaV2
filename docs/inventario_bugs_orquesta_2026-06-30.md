...
| BUG-ORQ-20260726-501 | abierto; causa localizada | V23/launcher Firecracker, namespace de red fijado | el cuarto intento físico superó primitivas y KVM, pero el launcher salió antes de crear `runs/socket` con `test_attestor.firecracker_launcher_network_unsafe`; el candidato quedó disabled/inactive, sin alias, microVM, VMM, cgroups hijos ni residuos | el instalador ya acepta correctamente el montaje `nsfs` seguro `root:root 0444`, pero `openEmptyNetNamespace` conserva la exigencia antigua e imposible `0600`; el mismo contrato quedó resuelto solo en la capa de primitivas por BUG-495 | unidad candidata `orquesta-firecracker-attestor-332afa472f29d25453352eae2fe3890d0df5643ea9cc65a71207d127299498b6.service`: primitivas `ok`, arranque `01:18:11.260`, corte limpio del launcher `01:18:11.426`; el avance hasta esta etapa acredita además el cierre físico de la causa KVM de BUG-499 | reutilizar en el launcher la regla de metadata segura de BUG-495, conservar `nsfs`, raíz, link count, pin de identidad y prueba real de red vacía; reconstruir artefactos y repetir E2E físico `1 + 16` |
