...
| BUG-ORQ-20260729-603 | cerrado en candidato; despliegue runtime pendiente | V14/V17/V21: cancelación, evidencia fallida y revocación de sesión | un Goal histórico cancelado dejó de ser legible con `sqlite.goal_record_artifact_scope_invalid`; una acción durable de revocar sesión acumuló diez intentos y el worker emitía `state.invalid`, aunque los nueve `prepare_workspace` del Goal nuevo estaban separadamente a la espera de aprobaciones explícitas | cancelar transformaba a `canceled` un WorkItem interrumpido por atestación fallida conservando su execution y evidencia exacta, pero el validador solo admitía esa evidencia mientras el item seguía `interrupted`; además, recovery exigía que una revocación histórica conservara la revisión actual del WorkItem en vez de rechazar solo generaciones futuras | snapshot archivado: Goal `goal:cea02be00047f6e695b75b5ba99ba5a6`, execution fallida `execution:8f355afa7ed869094196ca5da9278ad2` y acción activa `action:revoke-execution-session:execution:e6bba11442812376a515a977ede61a2f`; el candidato preserva únicamente fallos tipados ligados al instante y control de cancelación exactos, admite revocaciones históricas no futuras y acredita lectura, restart, retry e idempotencia en `TestSQLiteCancelFailedAttestationKeepsGoalReadableAndRevokesSessionAfterRestart`; `Open` y `GetGoal` pasan sobre una copia SQLite exacta | desplegar el binario corregido y observar que la revocación completa sin editar SQLite; cualquier `prepare_workspace` `sensitive/critical` seguirá bloqueado correctamente hasta una decisión explícita de otra autoridad |
