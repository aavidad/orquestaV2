...
| BUG-ORQ-20260806-700 | abierto; contrato y modelo sibling publicados, persistencia pendiente | Agente MicroVM/B11, detención física y reconciliación | la API sibling exponía una única detención sin modo y devolvía solo la ejecución actualizada; Orquesta no podía distinguir cooperativa de forzada ni obtener `receipt_ref`, instante físico confirmado y replay causal exacto | el motor ya producía un comprobante de retirada de recursos, pero el servicio y el ledger lo descartaban; la idempotencia conservaba resultado lógico, no el recibo físico | `agente_microvm@daf8469` fija el contrato público y `agente_microvm@db1b167` modela intención, snapshots históricos `deteniendo/detenida`, marca pre-efecto, sustitución cooperativa→forzada, receipt determinista y reconciliación read-only; revisión premium y suite completa quedaron verdes, pero aún faltan SQLite, motor, API y replay físico | no anunciar capacidad B11 hasta demostrar que cooperativa no escala por sí sola, forzada sí puede hacerlo, respuesta perdida reconcilia el mismo recibo y restart no fabrica timestamps ni autoridad |
