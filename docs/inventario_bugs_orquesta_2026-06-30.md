...
| BUG-ORQ-20260801-613 | corregido en código; acreditación Q4 pendiente | `ORC-28`/A05.2/A05.4, capacidad física previa al claim | el plan y el handoff declararon A05.4 cerrada y habilitaron A04.2 aunque el runtime no producía ni entregaba candidatos físicos al reclamo real | se contó la existencia del observador estático, la ordenación neutral y el arranque de controladores de cuota como si compusieran toda la cadena; `staticcapacity` no tenía consumidor productivo, `Orchestrator` no recibía fuentes o candidatos y el `ClaimRequest` conservaba una observación singular que `processing` no rellenaba | `e7f9e60e`+`993e3d03`: fuente bruta renovable, catálogo opaco por perfil, cuota vigente y `CapacityCandidates` reales; suites application/Codex/bootstrap, focales `-race` y guarda V38 verdes, sin arrancar Orquesta ni alterar Goals | Q4 debe materializar/revalidar/reservar dentro del mismo `BEGIN`; solo entonces cerrar la incidencia y acreditar A05/A04 sobre el consumidor transaccional, manteniendo V23 compatible hasta el cutover |
