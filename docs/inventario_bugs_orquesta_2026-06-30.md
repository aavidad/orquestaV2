...
| BUG-ORQ-20260711-227 | cerrado localmente/replay pendiente | Server/shutdown/snapshot causal | tras parar por control el unico goal de T9104, cuatro shutdown conservaron `stop_pending` con contadores a cero | el snapshot fresco reemplazaba trabajo activo pero conservaba `ShutdownGoalActions` derivadas de la observacion anterior | respuestas `shutdown-unbound*.json` retenidas en `/tmp/orquesta-bug226-t9104-runtime`; focal secuencial y suites `./cmd/orquesta-server ./modulos/orquesta-server`; [incidencia 227-228](incidencias/incidencia_orquesta_t9104_wakeup_material_progress_shutdown_stale_2026-07-11.md) | cada snapshot invalida acciones anteriores y downstream repuebla las actuales; falta replay real con shutdown cooperativo listo |
