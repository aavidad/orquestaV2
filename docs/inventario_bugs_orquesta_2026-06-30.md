...
| BUG-ORQ-20260726-562 | abierto; unidad recogida y SQLite intacta | adaptador `systemd-run`/expansión de argumentos | el primer smoke real del adaptador aceptó la unidad y la recogió inmediatamente; el journal registró `Invalid environment variable name evaluates to an empty string` para `10`, `11`, `12` y expresiones del cuerpo Bash, sin llegar a arrancar el profile | `systemd-run` expande variables en los argumentos por defecto; el cuerpo enviado como argv conservaba `${10}`, `${parent%/...}` y `${self_cgroup#/}`, que systemd reinterpretó antes de entregarlo a `/bin/bash` | InvocationID `90b821c3c64a41fc8836e1397572dbcb`; unidad `not-found`, sin PID ni logs de daemon, copia SQLite `quick_check=ok` y `user_version=16`. No se tocó la base ni la unidad viva | desactivar explícitamente la expansión de entorno de `systemd-run`, exigir ese argv en el fake y repetir el smoke real desde la misma copia v16; no escapar manualmente cada expansión ni aceptar el exit de `systemd-run` como readiness |
