...
| BUG-ORQ-20260804-617 | abierto; corrección de dominio en curso | V38/B10.0b, autoridad de eventos de sesión | el modelo durable incrementaba `revision_sesion` únicamente al cambiar de estado, mientras el contrato público exige una revisión de sesión estrictamente creciente por cada evento `stdout`, `stderr` o truncación; el primer borrador del puerto habría asignado secuencia en el repositorio | contrato, agregado y ledger se diseñaron en cortes distintos sin fijar primero quién posee la secuencia de eventos; dejarla al adaptador crearía una segunda autoridad y permitiría que snapshot y stream discrepasen | auditoría premium previa a integrar B10.0b2 detectó el P0; el borrador de puerto se detuvo antes de commit y se abrió B10.0b1.1 para que el agregado produzca secuencia y revisión, con CAS, cerca y overflow atómicos; no se arrancó KVM | cerrar el hecho de dominio, revisar que inicio/terminal/ambigüedad conservan una sola historia, adaptar después el puerto para persistir decisiones completas y cubrir replay, concurrencia, paginación y recuperación sin que SQLite invente revisiones |
