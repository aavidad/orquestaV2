...
- `BUG-ORQ-20260704-166` queda cerrado funcionalmente en codigo Orquesta para
  nueva app goal-first: un resultado durable `orquesta_goal_result.v0` con
  `status=invalid` y refs minimas de checkpoint se normaliza, se proyecta como
  cierre no aceptado y dispara rework causal si hay launcher/presupuesto. Queda
  pendiente solo smoke real Sueldos si se quiere validar campo.
...
| BUG-ORQ-20260704-166 | cerrado funcionalmente | Nueva App/goal-first checkpoint invalido | al crear `/home/alberto/Trabajo/Sueldos/generated-apps/mapa-de-gasto-publico` via `POST /api/v0/apps/director` con `director_execution_mode=goal_first`, Orquesta acepto el run y lanzo el goal, pero solo materializo `checkpoint_started.txt` y un `orquesta_goal_result.v0` con `status=invalid` y `missing_refs=[source_tree,handoff_report,technical_stack_manifest,go_app,tests]`; no hubo cierre Orquesta `complete/accepted` de app Go, UI, datos, docs tecnicas ni tests | la ruta de nueva app goal-first podia tratar un checkpoint invalido como resultado terminal opaco y no disparar replan/rework hacia programador cuando faltaban las refs estructurales de una app; ademas el cierre del backend `app_server_tmux` registro `codex_app_server_tmux_pane_exit_timeout`, residual cubierto por BUG-165/BUG-065 | `docs/incidencia_sueldos_goal_first_app_invalid_checkpoint_2026-07-04.md`; run `run-spec-mapa-de-gasto-publico-req-mapa-de-gasto-publico-43c20b877af23ecc450d324a25c1e616`; goal `goal-ref-app-director-run-spec-mapa-de-gasto-publico-req-mapa-de-gasto-publico-43c20b877af23ecc450d324a25c1e616`; external goal `019f2a37-df1f-7e00-a60b-dab168d79d38`; tests `TestObserveAppDirectorGoalV0LanzaReworkGoalSiNuevaAppSoloCheckpointInvalidV0`, `TestServerCodexAppServerGoalBackendV0NormalizaMissingRefsRaizEnChecklistV0`, `TestStackGoalMaterializedRefsSourceV0NormalizaMissingRefsRaizEnReceiptDurable` | Cierre: `missing_refs` raiz se normaliza a `checklist.missing_refs` en appserver y scanner materializado; el status terminal explicito del marcador/fichero gobierna el receipt; `ObserveAppDirectorGoalV0` permite rework automatico para `WorkKind=new_app` con `invalid|blocked` y faltantes estructurales `source_tree`, `handoff_report`, `technical_stack_manifest`, `go_app` o `tests`, lanzando `-rework-1` si hay launcher/presupuesto. Pendiente no bloqueante: relanzar smoke real Sueldos; el workaround manual no cuenta como entrega final |
