...
| BUG-ORQ-20260728-585 | abierto; corrección en curso | runtime Codex/supervisor y diagnóstico post-mortem | el primer intento de un WorkItem V23.3 en `Codex11` terminó como `codex.process_failed` después de 16,5 segundos, pero `last-message.json` quedó vacío y `terminal.json` solo conservó el código genérico; Orquesta pudo reintentar causalmente en `Codex13`, aunque ya no puede distinguir salida no cero, señal ni fallo transitorio de proveedor | el supervisor escribe en `completion-proof.json` exit code, señal, presencia del resultado y metadata acotada del diagnóstico, pero `process.go` descarta esa información al formar `terminalRecord` y borra el proof; el contrato durable no proyecta ni siquiera su metadata segura | auditoría read-only del intento `execution:075f0ab49a7a4214b76db303a313edf3`: sin timeout, `stop_agent`, OOM, throttling ni límite de PIDs; cgroup y supervisor quedaron limpios. La causa exacta ya es irrecuperable y no se leyó `auth.json` ni stderr sensible | proyectar en `terminal.json`, antes de borrar el proof y sin texto crudo, causa supervisada, exit code/señal, presencia de resultado, tamaño, SHA-256 y truncación del diagnóstico; conservar compatibilidad de replay con terminales antiguos y cubrir exit no cero, señal y ausencia de contenido sensible |
