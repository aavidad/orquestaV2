...
| BUG-ORQ-20260727-572 | cerrado en código; revalidación física pendiente | Firecracker/Jailer y permisos del VMM efímero | el candidato `ec6dfc98` superó red y readiness, pero `phase_one` terminó en menos de medio segundo pese a exigir 15 s de trabajo huésped; el parent cgroup registró un solo PID, 520192 bytes de pico y cero OOM, por lo que Firecracker no llegó a arrancar | el launcher copiaba tanto Jailer como Firecracker `root:root 0500`; Jailer bind-monta el VMM y baja a `JailUID:JailGID` antes de `exec`, dejando al proceso de jaula sin permiso de ejecución | `60ae04ec` conserva Jailer owner-only y copia el VMM como `root:JailGID 0550`: la jaula puede leer/ejecutar pero no modificar, y otros quedan sin acceso; la factory de prueba observa uid/gid/mode/nlink antes de `command.Start`, con negativo explícito para `root:root 0500`; paquete normal y focal race verdes | reconstruir el bundle desde el commit exacto y exigir E2E físico `1 + 16`; no ampliar a `0555`, no transferir ownership al jail UID y no reutilizar el candidato `ec6dfc98` |
