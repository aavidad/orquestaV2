...
| BUG-ORQ-20260729-600 | abierto; cancelación efectiva, proyección y revocación ilegibles | Goals/cancelación temprana, lectura posterior y sesión de ejecución | un Goal SQLite recién lanzado se canceló correctamente por una precondición SHA errónea: Goal, WorkItem y execution quedaron `canceled` y el control `confirmed`, pero `orquesta.goals.get` devolvió HTTP 500 inmediatamente después; la acción durable de revocar la sesión siguió reintentándose con `state.invalid` | por aislar; la proyección recuperada rechaza alguna evidencia residual del agente cancelado y el worker no reconoce como revocable la misma frontera terminal, aunque agregado y control ya expresan un cierre coherente | Goal `goal:b7161aa4919e0560e19c741f8ab4a8ca`, execution `execution:47954e80c5abab8a177a4e5ad12f9dea`, control `control:170dbdd53d5913e06dcf26eceb7fc3c9`; a las 11:14 la acción `action:revoke-execution-session:execution:47954e80c5abab8a177a4e5ad12f9dea` llevaba 17 intentos/fence 40 y explicaba los `worker_error code=state.invalid` periódicos; no quedaba proceso del agente y no se tocaron filas | añadir prueba de cancelación durante ejecución inmediatamente seguida de `GetGoal`, revocación, restart y recovery; una cancelación confirmada debe seguir siendo legible, completar o reconocer idempotentemente la revocación y no dejar proceso, sesión, workspace o evidencia fuera de contrato |
