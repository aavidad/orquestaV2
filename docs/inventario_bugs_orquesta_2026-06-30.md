...
| BUG-ORQ-20260725-453 | cerrado | V23/control de acciones en atestación | cancelar una acción `awaiting_attestation` o `quarantined` devolvió `internal/sqlite.action_retirement_state_invalid`; la reproducción determinista adicional mostró que cancelar después del `effect_attempt` y antes del completion fabricaba `completed/application.action_retired` para un efecto posiblemente aplicado; `pause` sí funcionaba | `RetireActionRefs` no distinguía dentro de la transacción SQLite entre una atestación aún no iniciada y un intento físico ya durable; la cuarentena terminal bien formada ya era idempotente, pero el intercalado begin/cancel podía degradarla a retiro ordinario | `4056c98e` revalida por `action_ref + intent_ref + fence`: cero intentos permite retiro, uno exacto conserva `quarantined/application.effect_unknown_applied` y cualquier combinación divergente falla cerrada; `52ba5d3a` cubre cancel antes de claim en Goal/WorkItem, cuarentena previa, pause y replay idénticos, ambos órdenes cancel/completion, completion tardío sin segundo receipt, tamper de fence, restart y Recovery SQLite; focal normal/race y vet verdes | conservar la consulta agregada total/exacta y el negativo de tamper; una atestación con intento físico nunca puede convertirse en retiro ordinario, y la cuarentena existente no se reescribe |
