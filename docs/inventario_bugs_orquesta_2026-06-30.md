...
| BUG-ORQ-20260726-499 | abierto; diagnóstico físico pendiente | V23/launcher Firecracker, arranque bajo unidad endurecida | el tercer intento superó instalación y `ExecStartPre`, pero el launcher content-addressed salió 96 ms después de que systemd declarase activa la unidad con `code=test_attestor.firecracker_launcher_unavailable`; el supervisor detuvo el candidato sin activar alias, y la inspección posterior confirmó cero socket, VMM, procesos y cgroups hijos | el código público colapsa varios fallos de sistema a `unavailable`; la ausencia persistente de `runs` acota el fallo a `preflightKVM`, pero falta su subetapa exacta | unidad `89435f…`, supervisor `51a91f…`, launcher `d70f14…` y assets `4b0f8a…` conservados; un probe transient con las mismas restricciones systemd pasó `openat2`, metadata e `ioctl` con KVM API 12, y el cleanup físico quedó sin residuos | probar el arranque sin carga de la unidad exacta, añadir diagnóstico seguro/reintento solo si conserva fail-closed y repetir `1 + 16` sin relanzar a ciegas |
