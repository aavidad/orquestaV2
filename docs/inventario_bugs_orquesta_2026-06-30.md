...
| BUG-ORQ-20260724-403 | cerrado en candidato V22; pendiente sello/E2E real | V22/Codex/reinicio y secreto MCP | al adoptar tras reinicio un proceso Codex vivo, el adaptador podía observar salida sin reconstruir la guarda de sesión y aceptar material con bearer MCP efímero | la reanudación adoptaba journal antes de restaurar guarda desde autoridad durable; la prioridad entre conflicto de replay, limpieza y publicación terminal no estaba cerrada | `673e41a008` recupera la guarda desde `CredentialStore`/sesión antes de adoptar y rechaza/sanea sin persistir material. `TestRecoveryFailureQuarantinesLiveProcessBeforeFinalScrub` cubre replay conflictivo que no mata, cuarentena/limpieza y replay terminal; `TestObserveRecoveryAuthorityFailureScrubsAndRejectsOutput` cubre busy/mismatch sin terminal; `TestRecoveryScrubFailureOutranksAuthorityAndPublishesNoTerminal` fija prioridad limpieza sobre terminal | conservar prioridad cleanup/terminal y fallo cerrado; nunca persistir bearer ni sustituto reversible |
...
| BUG-ORQ-20260724-408 | cerrado en candidato V22; pendiente sello/E2E real | V22/compactación de pruebas de autoridad | compactación retiró denegación explícita de artefacto hermano y recuperación post-restart de admisión por identidad de servicio | se recortaron combinaciones causales completas para ganar presupuesto; segunda auditoría detectó que C/D aún necesitaban restauración exacta, fence de mailbox antes/después de restart y terminal sin contradicción | `c07c8506ec` restituye C/D robustos en `TestV22RealCodexFourGoalsSelectiveStopCrashRestartAndCloseThroughMCP`: restore exacto, aislamiento delivered/sibling y autoridad execution-bound tras restart y terminal; `TestV22RealCodexNoTerminalContradictionAndNoOwnedProcess` mantiene cierre sin contradicción/procesos propios | conservar bordes C/D y seguridad causal; recuperar tamaño solo con helpers/equivalencia |
