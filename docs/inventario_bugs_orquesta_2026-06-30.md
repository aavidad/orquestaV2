...
| BUG-ORQ-20260725-467 | abierto, diferido | arquitectura/runtime general de agentes y almacenamiento efímero | todavía no se ha decidido ni acreditado migrar agentes a Firecracker, ejecutar workspaces/rootfs/caches de agentes en RAM/tmpfs, introducir checkpoints frecuentes o red privada de agentes | una microVM de `TestAttestor` de V23 no demuestra que el runtime completo tenga contratos de aislamiento, coste, persistencia, recovery y egress adecuados | deuda explícita de la decisión `docs/decision_atestacion_bubblewrap_microvm_2026-07-25.md`; aceptación futura obligatoria: host filesystem/LAN inaccesibles, red privada, sin east-west/direct peer, colaboración solo por gateway Orquesta + mailbox causal + CAS/artefactos autorizados con identidad Goal/tarea/parent-child; cada VM solo gateway/proxy, egress sin inbound, bloqueos host loopback/RFC1918/ULA/link-local/metadata/SSRF/puertos y NAT nunca abierto, solo netns+nftables; no hay estado implementado | no abrir antes de la cadena obligatoria `TestAttestor Firecracker -> recuperar V23 -> Orquesta autoprogramable`; entonces evaluar capability separada, contratos de checkpoint/restart/egress y pruebas de composición; TestAttestor actual sigue sin red |
