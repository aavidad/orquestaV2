...
| BUG-ORQ-20260806-679 | abierto; bloquea el Codex KVM con red | PFC-06, composición de egreso controlado | `application.Dependencies.EgressPolicies` no se inyecta en bootstrap y el Goal real de smoke no solicita `egress_policy_ref`; por ello el compilador omite `controlled_egress_proxy` y Codex no puede alcanzar OpenAI desde una microVM sin NIC | los contratos PFC-05 y sus pruebas cerraron antes que la composición productiva de su catálogo/autoridad | búsqueda exacta confirma el puerto y la persistencia, pero no un resolver productivo en `buildOrchestratorDependencies`; no se ha fabricado verde físico | componer un resolver canónico y hacer que el smoke pida una ref explícita; mantener la microVM sin NAT/NIC y exponer solo el proxy loopback firmado |
| BUG-ORQ-20260806-680 | abierto; tooling operativo acotado | PFC-06, preparación de perfil y credenciales | no existe descriptor físico publicado ni superficie operativa que provisione la privada Ed25519 y `auth.json`; solo hay helpers de tests | PFC-01 aporta hashes de kernel/initramfs/perfil, pero no vCPU, memoria ni los cinco hechos de cada servicio; generar desde Orquesta o flags manuales fabricaría autoridad que no posee | la minitarea Orquesta se detuvo sin código; la matriz confirma que `agente_microvm` debe combinar candidato verificado, recursos canónicos y servicios efectivos PFC-07, emitir descriptor+digest+receipt y Orquesta limitarse a verificarlo | crear en el sibling la publicación física y en Orquesta una herramienta separada de credenciales; no copiar helpers `_test`, no incluir material en descriptor/receipts y no inferir hechos ausentes |
