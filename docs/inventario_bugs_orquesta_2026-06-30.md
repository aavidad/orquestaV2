...
| BUG-ORQ-20260630-025 | abierto | OPES/audio | `generate_audio_asset` podia lanzar agente con ruta de herramienta audio inexistente o runner sin red/control de stop | la capacidad `speech_synthesis` ya existe como contrato neutral, pero la composicion OPES aun necesita preflight ejecutable de tool-path/runner antes del launch real | `docs/incidencia_opes_asg_audio_tool_path_2026-06-24.md`; `modulos/orquesta-server/docs/tareas.md` SRV-TASK-023; `docs/incidencias/opes_orquesta_backlog_operativo_2026-06-28.md` ORQ-OPES-004 | Pendiente: preflight opt-in de `scripts/opes_audio_app.py --help`, runner/red/timeout declarados y bloqueo `opes_audio_tool_missing` antes de lanzar agente inutil |
| BUG-ORQ-20260630-026 | abierto | OPES/visuales | SVG heredados o bocetos podian quedar como visual final publicable en HTML/paquete | el contrato historico de `visual_asset` aceptaba SVG seguro como artefacto, pero el cierre editorial OPES necesita distinguir insumo/borrador de arte final profesional | `docs/incidencia_opes_svg_visual_final_no_bloqueado_2026-06-24.md`; `docs/opes_flujo_temario_operativo_2026-06-02.md`; `modulos/orquesta-server/docs/tareas.md` SRV-TASK-019 relacionado | Pendiente: cierre OPES debe invocar validadores visuales o politica equivalente y bloquear estados finales si hay SVG/maquetas como arte final sin rework |
| BUG-ORQ-20260630-027 | abierto | OPES/supuestos | olas de supuestos practicos podian parar procesos con cobertura parcial y esquemas no importables | Orquesta no modela aun contrato de salida por wave para carpetas esperadas, esquema JSON/JSONL y cobertura real de artefactos | `docs/tarea_opes_supuestos_validacion_schema_y_cobertura_2026-06-23.md`; scripts OPES locales `normalizar_supuestos_tcae.py`, `validar_supuestos_tcae.py`, `consolidar_supuestos_tcae.py` | Pendiente: adaptador/diagnostico opt-in que compute `delivery_status=partial`, faltantes y `schema_repairable/schema_invalid`, sin confundir `process_status=stopped` con cierre |
