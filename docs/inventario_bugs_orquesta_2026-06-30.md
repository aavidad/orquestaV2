...
| BUG-ORQ-20260711-208AH | mitigado | Tooling/higiene de disco/reapertura de subagente | al reabrir un subagente fallo `No space left on device`; `/home` estaba al 100% con 0 disponible, aunque `/` tenia 134G y `/tmp` 44G | medir solo `/` o `/tmp` no detecta agotamiento del mount real de `$HOME`; faltan preflight por mounts, presupuesto de cache y recibo de cleanup. No se atribuye todo el uso a Orquesta ni se autoriza borrar caches ajenas | `go clean -cache`, tras verificar ausencia de procesos Go, libero 17G; `/home` quedo con 17G libres/99%. Detalle en la [incidencia 208AH](incidencias/incidencia_orquesta_home_lleno_reapertura_subagente_2026-07-11.md), indice [vivo](inventario_bugs_estado_vivo.md), antecedente [BUG-ORQ-20260704-172](#bug-orq-20260704-172) | prevención pendiente. Cerrar solo con preflight `df` por mounts (`$HOME`, `/`, `/tmp` y trabajo) y `cache budget/receipt` en reapertura equivalente |
...
BUG nuevo `BUG-ORQ-20260711-208AG` (abierto, falso verde de criterios/checklist
