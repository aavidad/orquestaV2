...
| BUG-ORQ-20260811-709 | cerrado en código y pruebas en esta entrega; revalidación privada pendiente del binario integrado | V25/runtime Codex, controlador de cuota por `app-server` | tras desacoplar el arranque, la unidad privada quedaba disponible pero creaba y destruía un `codex app-server` aproximadamente cada segundo, sin una sola observación de cuota y con los tres launches fail-closed | Codex 0.147 emite la notificación informativa oficial `configWarning` después de `initialized`; `QuotaSession.Accept` solo permitía `account/rateLimits/updated`, clasificaba el aviso como `appserver.method_not_allowed` y reiniciaba la sesión antes de recibir la respuesta correlacionada de `account/rateLimits/read` | diagnóstico privado registró únicamente kind/method/tamaño, sin payload de cuenta. La sesión permite ahora `configWarning` como advisory exacto en estado inicializado, conserva el rechazo de notificaciones ajenas y el helper E2E la intercala antes de la cuota; focal ×20, `-race` ×3 y `vet` verdes | reconstruir el binario privado desde el commit integrado y exigir observación durable real antes de cualquier launch; no ampliar la allowlist sin evidencia de protocolo ni convertir el advisory en disponibilidad |
