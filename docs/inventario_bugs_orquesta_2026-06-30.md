...
| BUG-ORQ-20260804-618 | abierto; detectado antes del puente B10.0c | Agente MicroVM, handshake anfitrión→huésped vsock | el helper de conexión al servicio huésped acepta cualquier respuesta `OK <puerto>` después de solicitar `CONNECT 10000`, por lo que podría tratar como correcto un enlace confirmado para otro puerto | la validación comprueba el prefijo/formato general del ACK pero no la igualdad exacta con el puerto solicitado; B08/B09 usaban otros sentidos y servicios, y no ejercitaban esta frontera de sesión | diseño read-only B10.0c localiza el defecto en `src/adaptadores/firecracker/motor.rs`; no se arrancó KVM y el puente aún no consume el helper como capacidad de agente | exigir byte a byte `OK 10000\n` —o derivarlo del puerto solicitado sin aceptar otro—, añadir negativos de puerto/espacios/trailing data y cerrar el caso antes de integrar el canal de sesiones |
