...
| BUG-ORQ-20260724-402 | abierto; pendiente post-V22 | V22/selección de razonamiento por WorkItem | `AgentLaunchRequest` conserva `reasoning_effort` por tarea, pero el adaptador Codex construye el proceso con el esfuerzo global de composición; una tarea simple no puede aún rebajar de forma efectiva el modelo/esfuerzo ni una delicada elevarlo por contrato individual | la metadata de gobierno y la selección física del proceso pertenecen a capas distintas y no existe aún un puente tipado por launch | inspección de `internal/application/processing.go` y `internal/adapters/agent/codex/process.go`: el request lleva el campo, mientras el argv consume `adapter.config.ReasoningEffort`; no se declara cierre ni evidencia real de selección por tarea | diseñar un puente explícito en adaptador/composición con recibo de modelo/esfuerzo efectivo, política coste/calidad y pruebas; no simular selección dinámica solo en metadata |
...
| BUG-ORQ-20260726-503 | cerrado en runtime V2; UX manual separada | tooling/perfiles y adaptador Codex nuevo, aprobación interactiva | un agente arrancado manualmente con `codex-perfil` pidió confirmación repetida; al revisar el runtime V2 se comprobó además que su argv productivo no fijaba explícitamente la política no interactiva | `codex-perfil` solo aísla `HOME`/`CODEX_HOME`; en V2 `internal/adapters/agent/codex` construía `codex exec` con sandbox pero sin `--ask-for-approval never`, mientras la afirmación anterior procedía del stack legacy congelado | `d39ad261` fuerza el prefijo global `--ask-for-approval never` en rutas directa, supervisor, sesión y preflight; el paquete Codex, race, bootstrap y una invocación del CLI Codex real quedaron verdes tras revisión xhigh independiente | conservar el negativo que exige una única política explícita; la TUI manual sigue requiriendo flags elegidos por el operador y Firecracker será otra frontera de seguridad, no sustituto de esta política |
| BUG-ORQ-20260726-504 | abierto, flaky preexistente | tests launcher Firecracker, contabilidad global de descriptores | una ejecución barajada `-shuffle -count=10` observó una variación del contador global de FDs, mientras el focal aislado `-race -count=100` y los gates de BUG-500 quedaron verdes | el test compara estado global del proceso y puede interferir con otros casos concurrentes; no hay evidencia que lo relacione con el cambio de rutas UDS | incidencia separada por la revisión xhigh de `d7bc3ac9`; no se modificó producción ni se ocultó el rojo | reproducir con barreras y ownership de descriptores antes de corregir; no usar un contador global inestable como falsa atribución de fuga |
| BUG-ORQ-20260726-505 | abierto, deuda preexistente no bloqueante | runtime Codex y harness bootstrap, staticcheck | la verificación de BUG-459 encontró `SA4006` en `process.go` y cuatro `U1000` en helpers históricos del supervisor/bootstrap | `git blame` sitúa las líneas antes del cambio de aprobación y esfuerzo; no comparten causa ni deben mezclarse con el cutover V6→V7 | paquete Codex normal/race, bootstrap, ports/application y vet focales quedaron verdes; staticcheck conserva los diagnósticos exactos como deuda visible | limpiar en write-set separado con tests de comportamiento; no ampliar BUG-459 ni ocultar staticcheck para fabricar un verde |
