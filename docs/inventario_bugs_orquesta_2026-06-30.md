...
| BUG-ORQ-20260725-458 | diagnosticado; reintento pendiente | V23/workspace Git local | el primer Goal V23 quedó en cuarentena `application.effect_unknown_applied` al preparar el workspace; no se creó worktree, rama ni proceso Codex | el seed llamado V2 seguía siendo un worktree del repositorio legacy: heredaba `merge.ours.driver=true` y metadata Git escribible por grupo, ambas rechazadas por el adaptador seguro; después de abrir el intento físico, el error concreto se conserva solo como cuarentena genérica | Goal `goal:c0347fc47ed4f0757bbb02fc97f78646`, acción `action:prepare-workspace:execution:e18e1836078940bf8ee0a74f20853ef4`; SQLite registra `application.effect_unknown_applied` sin workspace binding ni recibo. El Goal se canceló por la API y se creó `/home/alberto/Trabajo/orquestaV2` como clon exclusivo de `github.com/aavidad/orquestaV2` | endurecer el clon, relanzar el Goal y cerrar solo cuando Orquesta prepare, lance y persista el workspace; estudiar una evidencia diagnóstica segura que no relaje la cuarentena ni autorice replay ambiguo |
