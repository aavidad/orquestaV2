...
| BUG-ORQ-20260726-510 | cerrado en código; E2E Codex real pendiente de repetición | V15/V22, presupuesto de Goal y reemplazo automático de ejecuciones | el E2E Codex real dejó el intento 1 en `codex.process_failed`, liquidó tokens y dinero desconocidos de forma conservadora y creó un intento 2 cuya demanda de tiempo ya no cabía en el envelope del Goal; SQLite lo clasificó como `budget.temporarily_unavailable` cada cinco minutos y mantuvo Goal, WorkItem y launch en cola sin frontera posible | el escritor materializaba el reemplazo antes de distinguir cargos acumulados irreversibles de reservas activas recuperables; el claim solo conocía «no cabe ahora» y aplicaba correctamente la semántica temporal, aunque ninguna liberación futura podía hacer caber la demanda completa | `03dd5f5c` calcula en aplicación el mínimo irreversible `settlements charged + settlement causal actual + demanda siguiente` contra el envelope histórico del Goal antes de crear el reemplazo. Si no cabe, consume una sola vez el observe, conserva `codex.process_failed`, deja una única Execution fallida e interrumpe el WorkItem para replan sin falso cierre del Goal. La prueba SQLite usa uso desconocido, `max_execution_attempts=3`, envelope de un intento y dos reinicios; verifica un settlement, cero launch/reemplazo nuevo, un receipt de observe y replay ocioso. Focal normal/race, paquetes completos de aplicación/SQLite, recuperación y vet quedan verdes | no ampliar envelopes ni inventar uso; reservas activas o slots ocupados siguen siendo indisponibilidad temporal y el fallo definitivamente no aplicado con settlement exacto cero conserva el retry. El estado real observado es evidencia de diagnóstico, no fue mutado ni se declara E2E reparado hasta repetirlo desde un Goal nuevo |
