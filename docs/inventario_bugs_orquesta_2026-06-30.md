...
| BUG-ORQ-20260806-699 | abierto; bloquea el cierre físico honesto | B11/B12, lifecycle del agente microVM y cierre de Goal | la ruta B10 lanza y ejecuta Codex dentro de la microVM, pero una observación terminal intenta cerrar la ejecución como éxito aunque el adaptador exige preservación; SQLite rechaza después el terminal porque no existe recibo de preservación y el Goal no puede cerrarse limpiamente | el lanzamiento y la sesión se conectaron antes que la cadena física de éxito `quiesce -> preserve -> close`; `RegistrarPreservacionEntornoAgente` existe, pero no tiene caller productivo, y el wrapper microVM no aporta un lifecycle durable compuesto | auditoría causal de `ProcessNext/Claim` hasta sesión Codex confirma B10; `2d95fe49` liga cierre y receipt al `ApplicationReceiptRef` durable y cubre ausencia o invención de hechos de workspace, pero aún faltan caller productivo, persistencia/replay y workflow físico; no se ejecutó el smoke | completar recibos durables, persistencia y workflow de `quiesce -> preserve -> close`, ligar el manifiesto exacto y solo después cerrar Execution, WorkItem y Goal; acreditar reinicio, replay y cero residuos físicos |
...
| BUG-ORQ-20260806-702 | cerrado en `a6804d0d`; migración B12 pendiente | V38/B12, persistencia del manifiesto físico preservado | el comprobante de preservación podía incorporar la ref y el digest del manifiesto físico, pero el writer SQLite no tenía columnas para ambos y habría informado éxito durable perdiendo el enlace al reiniciar | contrato y persistencia avanzaron en cortes separados; aceptar el nuevo par antes de la migración 034 creaba un falso verde de cierre físico | el par es opcional-atómico por compatibilidad y el writer SQLite lo rechaza antes de abrir transacción con código estable; pruebas cubren cero filas, reinicio, repetición y compatibilidad histórica; revisión independiente P0/P1=0 y commit publicado | la migración 034 debe persistir y releer el par exacto antes de retirar la guarda; endurecer formato y límites de la ref física en ese mismo corte |
| BUG-ORQ-20260806-703 | abierto; refactor y revisión en curso | V38/B11-B12, reconciliación lifecycle tras resultado físico ambiguo | el primer modelo pretendía guardar tokens y resoluciones `pending` declaradas por el propio snapshot, aunque no existía evidencia independiente que autorizase esa resolución; además podía consumir una revisión antes de conocer el resultado físico | se modeló la ambigüedad como nuevo estado durable en vez de conservar el mismo intento físico y reconciliarlo mediante un puerto de solo lectura | el diseño corregido conserva el snapshot en `attempted`, no ejecuta un segundo efecto y solo permite reconciliar el request original; el terminal directo o reconciliado debe hacer un único CAS y ligar un `ActionConsumptionReceipt` cuyo `ConsumedAt` sea la frontera causal | no publicar el lifecycle hasta cubrir reinicio Q/P/C, respuesta perdida, todavía pendiente, receipt ausente/duplicado/manipulado y orden temporal; el reconciliador nunca puede ejecutar ni inventar autoridad |
