...
| BUG-ORQ-20260806-680 | abierto; tooling operativo acotado | PFC-06, preparación de perfil y credenciales | existe fuente segura de `auth.json` (`1bc90fcd`) y constructor puro de descriptor (`agente_microvm@b67815c`), pero no publicación de descriptor+digest+receipt ni CLI para provisión Ed25519/`auth.json` | PFC-01 aporta hashes de kernel/initramfs/perfil, pero no vCPU, memoria ni los cinco hechos de cada servicio; generar desde Orquesta o flags manuales fabricaría autoridad que no posee | los hechos nuevos son solo offline: fuente sin material y descriptor puro; no hay servicio host, KVM ni Codex físico acreditados | publicar en el sibling y emitir receipt; añadir CLI separada de credenciales, sin copiar helpers `_test`, incluir material en descriptor/receipts ni inferir hechos ausentes |
