...
| BUG-ORQ-20260726-508 | abierto; corrección candidata pendiente de auditoría independiente | runtime Codex, migración V3–V6 a V7 y esfuerzo por ejecución | el primer cierre de esfuerzo por request podía recuperar una ejecución V3–V6 usando el esfuerzo global configurado en el reinicio y publicar con ello un binding V7; sus pruebas daban verde porque repetían esa inferencia, aunque el journal legacy nunca persistió el esfuerzo físico | se confundió una configuración actual con evidencia histórica inmutable y, en V3, se podía completar Goal/Work/generaciones desde la petición de replay pese a que el hash antiguo no los acreditaba | `d80e3fff` elimina la creación productiva del binding legacy→V7. Los candidatos `eb1c7392` y `b71ec104` añaden una intención privada durable ligada a request, runtime, supervisor y cgroup exactos; el supervisor la drena antes del scrub/terminal y las pruebas propias cubren reinicio, escalado, carrera entre adaptadores, descendiente `setsid`, ausencia de salida tardía y `Stop` V4–V6 con proof/receipt público. Esta evidencia es candidata y no autoriza cerrar la fila | auditoría independiente debe confirmar replay/idempotencia, cero PID/cgroup/salida tardía, terminal único y preservación de `Stop`; no reconstruir campos ausentes desde defaults o replay |
