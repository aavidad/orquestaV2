...
| BUG-ORQ-20260725-436 | abierto en revisión; bloquea candidato V22 | V22/Codex/supervisor y cierre de descriptores | si fallaba `acquireOwnerLock` después de preparar el supervisor, la rama cerraba el gate pero dejaba abiertos el memfd sellado y los dos extremos del canal diagnóstico | la limpieza de error se amplió desde el lanzamiento legacy sin centralizar todos los recursos nuevos; el memfd contiene prompt, entorno y clave privada Ed25519 | contrarrevisión premium read-only sobre `process.go`; solución exigida: cierre exhaustivo e idempotente de gate, memfd y canal en toda salida anterior al start, con prueba de la ruta de lock fallido | ningún error de preparación puede conservar secretos o descriptores; verificar leak de FDs y no limitar el test a ausencia de proceso |
...
| BUG-ORQ-20260725-440 | abierto; bloquea E V22 | V22/Codex/supervisor y drenaje de árbol sandbox | la captura de BUG439 demostró que Codex lanza herramientas con `bwrap --new-session`; el supervisor, stop, timeout y censo V22 operan sobre `kill(-PGID)` y consideran el árbol ausente cuando desaparece el líder schema 2, aunque un descendiente con PGID/SID propio pueda seguir vivo | el contrato equiparó grupo de procesos con árbol completo. `Pdeathsig` solo liga al worker directo y `--die-with-parent` es comportamiento del proveedor, no autoridad de Orquesta; aun así el supervisor podía firmar `tree_gone=true` sin una frontera que contuviese todos los descendientes | auditoría premium read-only sobre `supervisor_linux.go`, `process_control.go`, `adapter.go` y `completion_proof.go`; sonda acotada confirmó que matar el PGID padre dejó vivo un `sleep` descendiente tras `setsid`, que fue recogido al finalizar la sonda. Los tests existentes solo cubren hijos en el mismo PGID y la unidad systemd puede ocultar la fuga al recoger el cgroup completo | corrección exigida antes de E: aislamiento Linux exclusivo por ejecución —cgroup v2 o frontera equivalente— ligado a `process.json` y completion proof; supervisor entra antes de abrir el gate, stop/timeout/shutdown usan el cierre de esa frontera y solo publican `tree_gone` tras `populated=0`. Cubrir natural, timeout, cooperative, forced y crash/restart con descendiente `setsid`, y sibling externo que no debe morir |
