...
| BUG-ORQ-20260801-613 | abierto; autoridad corregida y código pendiente | `ORC-28`/A05.2/A05.4, capacidad física previa al claim | el plan y el handoff declararon A05.4 cerrada y habilitaron A04.2 aunque el runtime no produce ni entrega candidatos físicos al reclamo real | se contó la existencia del observador estático, la ordenación neutral y el arranque de controladores de cuota como si compusieran toda la cadena; `staticcapacity` no tiene consumidor productivo, `Orchestrator` no recibe fuente o candidatos y el `ClaimRequest` conserva una observación singular que `processing` no rellena | auditoría de `1382753a`: la ordenación solo aparece en pruebas y SQLite ignora capacidad al reclamar; esta fila y la corrección del plan restauran el estado parcial sin arrancar Orquesta ni alterar los tres Goals preservados | completar y presupuestar catálogo/observadores vivos de A05.2 e inyección de candidatos de A05.4; añadir una guarda arquitectónica que falle si no existe consumidor productivo y mantener A04.2 bloqueada hasta entonces |
