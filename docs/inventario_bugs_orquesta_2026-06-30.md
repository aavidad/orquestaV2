...
| BUG-ORQ-20260726-499 | abierto; diagnóstico físico pendiente | V23/launcher Firecracker, arranque bajo unidad endurecida | el tercer intento superó instalación y `ExecStartPre`, pero el launcher content-addressed salió 96 ms después de que systemd declarase activa la unidad con `code=test_attestor.firecracker_launcher_unavailable`; el supervisor detuvo el candidato sin activar alias, y la inspección posterior confirmó cero socket, VMM, procesos y cgroups hijos | el código público colapsa varios fallos de sistema a `unavailable`; falta correlacionar el código de etapa del E2E root con el preflight exacto que falla dentro de la unidad endurecida | unidad `89435f…`, supervisor `51a91f…`, launcher `d70f14…` y assets `4b0f8a…` conservados; host con KVM API 12 accesible, 110 GiB disponibles y cleanup físico observado sin residuos | leer el log root acotado del intento, discriminar preflight/Serve bajo el sandbox systemd, corregir con prueba negativa y repetir `1 + 16` sin relanzar a ciegas |
