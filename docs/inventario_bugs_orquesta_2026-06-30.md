...
| BUG-ORQ-20260726-547 | abierto; candidato WIZ20 no integrable | V23/comando público de intake revisado | después de la primera revisión de una pregunta, `orquesta.intakes.get` serializa `history[].questions_revised`, pero el schema v1 cerrado no declara ese campo y el dispatcher responde `commands.output_contract_invalid` | el modelo append-only evolucionó sin regenerar o versionar de forma coherente la proyección pública cerrada | auditoría xhigh independiente sobre `7b409be7`: contraejemplo revisión privada exitosa seguido de consulta pública `internal`; los paquetes existentes quedaban verdes porque no cubrían el round-trip posterior a revisión | añadir contraejemplo público y cerrar schema/versionado antes de integrar WIZ20; una mutación durable nueva debe poder consultarse por el mismo contrato que la expone |
| BUG-ORQ-20260726-548 | abierto; candidato WIZ20 no integrable | V23/intake, identidad causal de revisiones | una pregunta creada con derivación A podía revisarse presentando cualquier derivación B válida y no vacía; Apply, SQLite, restart y replay aceptaban el cambio | la validación acreditaba solo la forma de `DerivationIdentity`, no su igualdad con la identidad que produjo la versión activa | auditoría xhigh independiente sobre `7b409be7`; el único negativo existente probaba derivación cero y no sustitución por otra identidad válida | ligar cada revisión a la identidad causal exacta de su versión activa, con negativos de sustitución y persistencia/replay |
| BUG-ORQ-20260726-549 | abierto; candidato WIZ20 no integrable | V23/Wizard, alcance causal de reconciliación | si se reabría una sola decisión, el preflight consideraba reconciliables todas las preguntas y permitía reversionar otra pregunta por un cambio simultáneo de hechos sin arista causal con la reapertura | una señal global de “hay reapertura” sustituyó el cálculo de inputs y descendientes afectados | auditoría xhigh independiente sobre `7b409be7`; contraejemplo U1 reabre U3 mientras cambia `Facts.Surface` y reversiona U2 ajena | calcular el conjunto causal afectado por dependencias/inputs exactos; un cambio concurrente no relacionado sigue siendo conflicto |
| BUG-ORQ-20260726-550 | abierto; candidato WIZ20 no integrable | V23/Wizard, retiro append-only de preguntas derivadas | al cambiar U1 de `team` a `personal`, T1 deja de emitirse y la reconciliación devuelve `StateConflict` aunque su decisión ya se hubiera reabierto correctamente | el protocolo solo añade o revisa preguntas presentes y carece de retiro/tombstone durable para una proyección que deja de ser aplicable | auditoría xhigh independiente sobre `7b409be7`, con flujo real U1→T1 | incorporar retiro append-only, replay y restart exactos dentro del único Intake; no borrar historia ni crear un segundo lifecycle/store |
| BUG-ORQ-20260726-551 | abierto; candidato WIZ20 no integrable | V23/intake, reapertura transitiva tras revisar pregunta | revisar una pregunta padre reabría únicamente su decisión; una hija respondida podía conservar `CurrentDecision` aunque dependiera causalmente de la versión sustituida | la invalidación por revisión no recorría el grafo de dependencias que ya usa la creación inicial | auditoría xhigh independiente sobre `7b409be7`; contraejemplo padre e hija respondidos, revisión del padre y decisión hija todavía corriente | reabrir transitivamente decisiones descendientes con orden determinista, conservando motivos y estado válido tras SQLite restart/replay |
