...
| BUG-ORQ-20260725-445 | cerrado localmente; obliga a nuevo P/S | V22/evidencia Codex real genérica | el primer intento E sobre S `c5bb260309` se detuvo en el preflight porque `real_codex_mcp_e2e.json` conservaba el digest del candidato anterior (`56083b…`) frente al source vigente (`8b830e…`) | el supervisor/cgroup, bootstrap y configuración cambiaron después del último smoke genérico; el ratchet invalidó correctamente el PASS heredado antes de ejecutar los E2E V22 | se abortó el comando E sin continuar tras el preflight rojo y se reejecutó `TestRealCodexAdapterClosesGoalThroughProductionMCPServer` con composición real, cgroup privado y cero residuos: PASS `6.23s`, marker `ORQUESTA_CODEX_E2E_OK_6cc10badf96a35ad0f8aa804b8e6956e`; el receipt liga `sha256:8b830e1b6d94537bc654a6d341b148cfcaceb5cb89c14e4e0ba711f93fe5b34c` | el receipt genérico renovado debe entrar en un P nuevo; S y E se recrean después, sin reutilizar el sello anterior ni falsear su parentesco |
| BUG-ORQ-20260725-446 | solución premium en curso; bloquea E V22 | V22/bootstrap/restart real, sesión y adopción segura | la E exacta sobre S `348a2d3341290680447fe14164a7d6e665d90cad` dejó verde la suite normal, el bloque focal `-race` y `TestV22RealCodexNoTerminalContradictionAndNoOwnedProcess`, pero `TestV22RealCodexFourGoalsSelectiveStopCrashRestartAndCloseThroughMCP` falló tras el `SIGKILL`: el servidor reiniciado no publicó MCP en 30 s y su log solo expuso `No se pudo completar la operación. code=internal`; la limpieza encontró seis identidades schema 3 todavía vivas | `openBuildRepository` liga `RuntimeScope` y descubre/adopta procesos antes de que bootstrap construya y ligue `SessionResolver`; esa adopción llena `adapter.executions` y el binding posterior falla cerrado con `codex.session_invalid`. Un simple reorder sería inseguro: la discovery eager tampoco reconstruye las guardas de sesión/credencial antes de adoptar y podría reabrir BUG403 | salida E exacta retenida en `/tmp/orquesta-v22-evidence-348a2d3341/execution.output.txt` (18 459 bytes, SHA-256 `984ff93332b8a1d3e6ebe21281e73c2f0a7688a63f623c08a266f61af3dc15f9`). Reproducción focal independiente: mismo fallo determinista en `129.67s`; ambas unidades fueron recogidas y el censo confirmó cero procesos/servicios residuales | separar apertura/provisión de repositorio del binding runtime; ligar primero el resolver durable, después scope/discovery con contexto y restauración de guards previa a adopción; preservar cuarentena/scrub fail-closed y prohibición de reemplazo. Añadir regresión bootstrap con ejecución session-bound persistida, repetir focal real grande y después crear P/S nuevos para E exacta |
