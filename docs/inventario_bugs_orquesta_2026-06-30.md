...
| BUG-ORQ-20260806-703 | abierto; servicio y puerto Store completos, implementación SQLite y composición pendientes | V38/B11-B12, reconciliación lifecycle tras resultado físico ambiguo | el primer modelo pretendía guardar tokens y resoluciones `pending` declaradas por el propio snapshot, aunque no existía evidencia independiente que autorizase esa resolución; además podía consumir una revisión antes de conocer el resultado físico | se modeló la ambigüedad como nuevo estado durable en vez de conservar el mismo intento físico y reconciliarlo mediante un puerto de solo lectura | el modelo conserva el snapshot `attempted`, prohíbe un segundo efecto y reconstruye el request original; el nuevo servicio relee ganadores CAS, tras pending/error solo hace `Inspect` y reconciliación histórica, y almacena recibo, consumo y siguiente acción en el CAS terminal. Los tests cubren Q-P-C, replay, respuesta perdida, pending, CAS loser con reloj distinto y finalización solo en `closed`, incluidos focal y `-race` verdes | implementar `AgentEnvironmentLifecycleStore` en SQLite y componer lifecycle/reconciliador; sin ese CAS durable real, restart y smoke físico el frente permanece abierto |
