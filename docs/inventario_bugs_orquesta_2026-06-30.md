...
| BUG-ORQ-20260725-470 | abierto, en corrección | V23/guest Firecracker, identidad y capacidad real | la auditoría previa a integración encontró scratch root `0700` inaccesible para UID 65534, `NoSetGroups` invertido, explosión de directorios no contabilizada, alias `--json=false`, toolchain no acreditado para `nobody` y tmpfs sin dimensionado explícito | los fakes cubrían contratos internos pero no el cambio real de credenciales, traversal completo, modos del initramfs ni presión conjunta snapshot/workspace/cache | auditoría independiente 2026-07-25; protocolo raw-drive, checksum/OID, traversal, header-last y PID namespace quedaron limpios, pero el guest no se versiona hasta corregir y repetir `race`, ejecución real como UID 65534 y builder reproducible | corregir P0/P1, añadir preflight de capacidad y E2E focal del proceso real; cerrar con commit/pruebas, sin declarar aún boot Firecracker completo |
| BUG-ORQ-20260725-471 | abierto, en corrección | V23/configuración microVM y perfil host 128 GiB | defaults de guest 1536 MiB/cgroup 2 GiB no garantizan holgura para toolchain (~206 MiB), snapshot y workspace del repo (~202 MiB cada uno), caches y procesos; activar 16 sobre Bubblewrap sería además una desviación | las cotas se validaban individualmente, sin presupuesto cruzado guest/snapshot/output/reserva ni perfil de despliegue por host | medición 2026-07-25: host ~123 GiB total/~111 GiB disponible; objetivo solicitado 16 microVMs, guest 4 GiB y cgroup 5 GiB, presupuesto nominal 80 GiB más reserva del host | mantener 2 portable; acreditar validación cruzada y perfil de 16, pero aplicarlo atómicamente solo al activar `provider=microvm`; no aumentar el Bubblewrap vivo |
