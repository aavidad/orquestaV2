...
| BUG-ORQ-20260804-619 | abierto; primer draft B10.0b2 no commiteado | V38/B10.0b2, ledger durable y replay de sesión | la suite del draft estaba verde en memoria, pero un replay tras reinicio era imposible: confirmar exigía recrear el `Cambio` privado y no preservaba la intención de entrada | faltaban reserva durable de entrada/intención, consulta de pendientes y CAS atómico contra la `revision_trabajo` canónica de ejecuciones; el diseño confundía verificación en memoria con recuperación durable | auditoría del primer draft B10.0b2 antes de commit; no se integró ni se atribuye evidencia SQLite | rediseñar el ledger antes de SQLite para persistir la reserva, recuperar pendientes y confirmar atómicamente contra la revisión canónica sin recrear tipos privados |
