...
| BUG-ORQ-20260726-545 | deuda de compatibilidad previa al sello | V23/comando `orquesta.intakes.get` v1 | la salida añade `history[].derivation` opcional bajo un schema con `additionalProperties:false`; un cliente antiguo que valide estrictamente objetos de history puede rechazar la propiedad nueva | el contrato V23 aún no está sellado ni acreditado, pero el número de versión público ya existe en el registro candidato | `24cb4114` mantiene JSON histórico idéntico cuando derivation es cero y la proyección pública impide inyectarla; la compatibilidad de consumidores estrictos debe decidirse antes de sellar | antes de `AC-V23-WIZARD`, congelar el schema candidato o publicar una versión nueva si ya existe consumidor externo acreditado; no declarar compatibilidad total por ser un campo opcional |
