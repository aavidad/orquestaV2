...
| BUG-ORQ-20260806-703 | abierto; modelo puro revisado, writer y composición pendientes | V38/B11-B12, reconciliación lifecycle tras resultado físico ambiguo | el primer modelo pretendía guardar tokens y resoluciones `pending` declaradas por el propio snapshot, aunque no existía evidencia independiente que autorizase esa resolución; además podía consumir una revisión antes de conocer el resultado físico | se modeló la ambigüedad como nuevo estado durable en vez de conservar el mismo intento físico y reconciliarlo mediante un puerto de solo lectura | el modelo de esta entrega conserva el snapshot `attempted`, prohíbe un segundo efecto, reconstruye exclusivamente el request original y exige recibo físico terminal, inspección concordante y un único `ActionConsumptionReceipt`; los tests cubren secuencia Q/P/C, replay, respuesta perdida, pendiente, duplicados, manipulación y orden temporal con race verde | implementar el `AgentEnvironmentLifecycleTerminalWriter` real y componer lifecycle/reconciliador; sin ese CAS durable y sin smoke físico el frente permanece abierto |
