...
| BUG-ORQ-20260723-395 | deuda aceptada por operador; no bloquea V22 | V22/simplicidad/tamaño | el delta formateado alcanzó 2.937 LOC de producto y 4.113 LOC de tests/harness frente al objetivo inicial 2.800/3.800 | el cierre causal, los negativos de seguridad y el E2E real necesitaron más evidencia que la estimación roja; recortar antes de ejecutar podía degradar legibilidad o cobertura | decisión del operador 2026-07-23: conservar ficheros grandes si funcionan, registrar deuda y usar techo explícito 3.000/4.200; el gate sigue midiendo físicamente y no se elimina | adelgazar después de V22 solo con equivalencia de tests; no convertir el objetivo original en falso verde ni sacrificar escenarios |
...
| BUG-ORQ-20260724-403 | abierto; bloquea P V22 | V22/Codex/reinicio y secreto MCP | al adoptar tras reinicio un proceso Codex vivo, el adaptador no conserva ni reconstruye `sessionGuard`; el terminal puede aceptar diagnóstico o artefacto que contenga el bearer MCP efímero | la reanudación adopta el journal de proceso antes de trasladar la guarda resuelta y `Observe(execution_ref)` no dispone de una vía tipada para rematerializarla desde la autoridad durable | auditoría independiente sobre `resumeLaunchRecordLocked`, `Observe` y `gateCredentialTerminalLocked`; falta prueba de regresión que escriba el bearer desde un proceso adoptado y exija `codex.secret_leak` sin persistir material | reconstruir la guarda solo mediante `CredentialStore`/autoridad de sesión, ligarla al proceso adoptado antes de observar salida y fallar cerrado si no puede recuperarse; nunca persistir el bearer ni un sustituto reversible |
| BUG-ORQ-20260724-404 | abierto; bloquea P V22 | V19/sello histórico frente a configuración posterior | `TestV19PSELifecycleIsExact` compara la identidad efectiva sellada de V19 con `config.Resolve` del HEAD V22; una clave canónica legítima de V22 vuelve rojo un sello histórico sin que su commit, manifest ni receipt hayan cambiado | el verificador mezcla la fuente sellada con el registro vivo posterior en vez de resolver o comprobar la configuración desde el OID de producto V19 | gate literal V22 en `0afdf3bda9`: `v19 seal effective config identity differs`; V22 no debe reescribir el manifest ni fabricar otra evidencia V19 | validar el blob/configuración del commit sellado V19 y su binding ya acreditado, no la configuración viva; añadir negativo de alteración del blob sellado y conservar intactos manifest, receipt y OID históricos |
| BUG-ORQ-20260724-405 | abierto; bloquea P V22 | V22/revocación tras cuarentena de handoff | si `admit_mailbox` termina en cuarentena, `CompleteAction` retorna antes de agendar `revoke_execution_session`; la autoridad lógica cae, pero la credencial física de una ejecución terminal queda utilizable indefinidamente | la excepción que conserva el bearer hasta resolver el handoff solo agenda revocación en la rama completada y omite el terminal negativo | auditoría independiente de `claim.go` y `processPostArtifactMailboxAdmission`; falta caso terminal con cuarentena, restart, retry e idempotencia de receipt | agendar la misma acción causal de revocación también antes del retorno quarantined; probar una única revocación durable y física tras restart, sin revocar antes de resolver el handoff |
| BUG-ORQ-20260724-406 | abierto; bloquea recovery V22 | V22/recovery de revocación | recovery acepta acciones `revoke_execution_session` semánticamente imposibles —por ejemplo sobre ejecución running— y no exige la acción tras un terminal que ya resolvió su handoff | la migración admite el nuevo kind, pero los validadores de recovery solo cubren semántica histórica de launch/observe/deliver/admit | auditoría independiente de `recovery_validation*.go`; faltan negativos de revoke prematuro, ausencia post-terminal y receipt incongruente, más el positivo de admit aún pendiente | añadir validación V22 de tuple, generaciones, terminalidad, receipt exacto y totalidad; permitir ausencia únicamente mientras el handoff terminal conserve un `admit_mailbox` activo |
| BUG-ORQ-20260724-407 | abierto; bloquea E V22 | V22/presupuesto de implementación frente a evidencia | el presupuesto suma `product/evidence/*.json` como LOC de producto; el receipt V3 real y la promoción del roadmap desbordan el techo aunque el código congelado no cambie | el clasificador mezcla implementación candidata con evidencia post-sello que por contrato nace fuera de P/S | auditoría de `v22_simplicity_budget_test.go`: producto 2979/3000 antes de E y un receipt normal supera el margen de 21 líneas | excluir explícitamente `product/evidence/` del delta de implementación, mantener el roadmap como producto y volver a medir en E; no minificar evidencia para engañar la métrica |
| BUG-ORQ-20260724-408 | abierto; bloquea cobertura exacta P V22 | V22/compactación de pruebas de autoridad | la compactación eliminó la denegación explícita de un artefacto hermano no entregado y la recuperación post-restart de una admisión completada por la identidad de servicio | se eliminaron escenarios completos para ganar presupuesto aunque las coberturas genéricas no prueban esas combinaciones causales exactas | revisión independiente del delta `9f9d16b897`; el E2E real cubre parte del recorrido, pero no ambos negativos/restart focales | reponer assertions compactas en pruebas existentes y recuperar margen mediante helpers o duplicación real, no retirando bordes de seguridad |
| BUG-ORQ-20260724-409 | abierto; pendiente antes de E V22 | V22/cadena P-S-E | el gate del receipt exige que S sea hijo exacto de P, pero permite que el source ejecutado sea cualquier descendiente de S; commits ajenos podrían entrar entre el sello y la ejecución | la validación usa ancestralidad donde el contrato operativo describe checkout exacto detached-clean de S | revisión independiente de `v22AssertExactSeal`; aún no existe E que explote el hueco | exigir que el `sealed_source.git_commit_oid` ejecutado sea exactamente S y que el checkout acreditado esté detached-clean; E solo puede añadir receipt/output y promoción fuera del candidato |
