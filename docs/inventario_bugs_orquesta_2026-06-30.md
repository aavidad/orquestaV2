...
| BUG-ORQ-20260724-410 | cerrado en candidato V22; pendiente sello/E2E real | V22/gate de write-set | `scripts/check_rebuild_write_set.sh` omitía siete rutas V22 exactas y rechazaba un delta legítimo | la allowlist V22 conservaba el corte anterior y no enumeraba las dos pruebas V19 ni cinco rutas Codex/SQLite nuevas; no era lícito abrir un patrón amplio | `7b0a1679bf` añade solo `acceptance/v19_council_seal_manifest_test.go`, `acceptance/v19_council_source_test.go`, `internal/adapters/agent/codex/{control.go,control_persistence.go,recovery_test.go,upgrade_v3_test.go}` e `internal/adapters/state/sqlite/recovery_validation_v21_test.go`; no añade prefijos. `scripts/check_rebuild_write_set.sh 665ef446a32a7f0512255640fa99b2e7edd9f29d` prueba la base V21 | cada corte declara rutas exactas por base; un rechazo de delta legítimo se corrige con allowlist mínima, nunca con glob/patrón amplio |
