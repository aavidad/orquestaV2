...
| BUG-ORQ-20260805-642 | abierto; diseño causal delimitado | B10.5d, receipt recuperado y claim de reconciliación | un lanzamiento recuperado pertenece al `EffectAttempt` histórico, pero la confirmación consume un claim nuevo con cerca superior; el esquema actual exige una única `action_fence` tanto al receipt físico como al comprobante de consumo | `effect_receipts.action_fence` y `action_consumption_receipts.fence` representan hoy dos autoridades que coinciden en el camino normal pero divergen legítimamente tras reinicio | inspección de `effectReceipt`, `insertEffectReceipt`, `completeClaimWithEffect` y de los triggers `effect_receipts_causal_guard`/`action_consumption_effect_receipt_guard`: fabricar otro intento en la cerca nueva falsearía un segundo efecto físico y reutilizar la cerca vieja rompería el CAS del claim nuevo | separar explícitamente la cerca del intento histórico de la cerca de confirmación/consumo en una migración acotada; probar receipt perdido, reinicio, replay, claim obsoleto y carrera sin crear otro `EffectAttempt` ni relanzar la microVM |
...
| BUG-ORQ-20260805-644 | cerrado operativamente por la reejecución integrada en `c322e156` | delegación B10.5d2, agente SQLite | el agente devolvió como cierre instrucciones antiguas para arrancar `codex6`, ajenas a la minitarea SQLite asignada, aunque había dejado un WIP parcial en el write-set correcto | contaminación de contexto o cierre no correlacionado al reutilizar un hilo de agente; el texto final no puede tomarse como evidencia de la tarea actual | se conservó y revisó el diff, no se acreditó desde aquella respuesta y la tarea se reemitió corta; el cierre posterior citó write-set y gates actuales y pasó revisión xhigh | exigir que todo ACK/final cite tarea, ficheros y tests actuales; descartar respuestas no correlacionadas y no acreditar trabajo desde ellas |
