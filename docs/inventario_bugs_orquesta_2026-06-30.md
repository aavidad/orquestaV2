...
| BUG-ORQ-20260725-427 | en investigación; rerun controlado pendiente | V22/E2E/arranque y diagnóstico | el primer smoke limpio sobre `b90c7a2671` no llegó a crear Goals: `v22Start` agotó 30 s intentando conectar y el proceso `orquesta serve` solo escribió `No se pudo completar la operación. code=internal` (SHA-256 exacto con LF `6335cb127f47bd48d297c49c9dc440a481e2070d75a5417baf7465eb54922f5a`); la unidad delegada `orquesta-v22-33c01739bc61c36cf4c27a02.service` terminó y fue recogida con 41,019 s de runtime, 59,667 s de CPU y 1001,6 MiB pico, sin procesos residuales | causa aún no cerrada; el fallo ocurrió al arrancar el servidor después de compilar dentro de una sesión con carga externa y el primer comando no fijó los límites operativos previos `GOMAXPROCS=8`/`GOFLAGS=-p=1`; además, la salida CLI `internal` no conserva una causa útil en el log del harness | repetir exactamente el smoke desde estado nuevo con concurrencia Go acotada y cachés aisladas; si recurre, preservar root/log/SQLite antes del cleanup y corregir el diagnóstico o la causa de producto con evidencia | no atribuirlo a BUG426 ni a las ventanas A/B/C/D: falló antes de la primera operación MCP. La unidad y el temp interno quedaron recogidos |
