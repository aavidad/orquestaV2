...
| BUG-ORQ-20260725-495 | abierto, reproducido en host físico | V23/instalador Firecracker, primitivas de red | `--apply` instaló los artefactos content-addressed, pero abortó antes del E2E con `error=primitive_netns_metadata`: `ip netns add` dejó `/run/netns/orquesta-firecracker-attestor-empty` como montaje `nsfs` `root:root 0444` aunque el helper intentó `chmod 0600` y exigía después el modo imposible `0600` | el contrato confundió permisos de un inode regular con la metadata visible de un bind mount `nsfs`; el acceso al namespace depende además de capacidades y de su identidad de montaje, no de poder cambiar ese modo aparente | host Ubuntu/kernel 7.0.0-28: unidad candidata `disabled/inactive`, sin alias, evidencia, receipt, microVM, cgroup ni VMM residual; quedaron el runtime bind y el netns vacío creados por la transacción parcial | validar de forma portable dueño, tipo no symlink, link count, filesystem `nsfs`, identidad de montaje y red completamente vacía; repetir `apply/check` idempotentes y el E2E físico `1 + 16` antes de activar |
