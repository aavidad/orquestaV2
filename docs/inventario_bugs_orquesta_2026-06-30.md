...
| BUG-ORQ-20260726-507 | cerrado en código; composición física pendiente | runtime general de agentes Firecracker, autenticación de red | el primer corte describía anti-replay aunque solo había framing del proof y validación de receipts; además alteró `AGT-01.source_proposal` para alojar una decisión de implementación sin esquema propio | no existía un verificador host ni consumo atómico del challenge y la decisión nueva se introdujo en un campo de procedencia histórica en vez de modelarla explícitamente | `2d9abc8a` restaura la procedencia y registra `implementation_decisions.agent_microvm_network` sin crear una capability ficticia; `95a48981` usa atestación exacta, callback de `CredentialStore`, HMAC-SHA256 con `hmac.Equal` y challenge acotado de consumo único; `b430a17f` liga documento y digest. Negativos, concurrencia `-race -count=20`, `vet`, `staticcheck` y guardas raíz verdes | conservar `planned_not_applied`: todavía faltan wiring del atestador/verificador, reserva vsock por VM, broker/proxy físicos y E2E multi-microVM; no presentar contratos o unitarios como acreditación de red |
