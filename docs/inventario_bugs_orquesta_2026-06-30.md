...
| BUG-ORQ-20260729-607 | abierto; causa confirmada | scheduler SQLite/bootstrap, observación terminal Codex | terminal.json válido quedó sin observar 11m52s y 4m52s mientras launches/reintentos de 30–67s seguían entrando | `ClaimNextAction` ordena `launch_agent` (2) antes de `observe_agent` (6); el único scheduler llama `ProcessNext` síncrono y, tras el pseudo-límite de dos launches, reinicia con delay cero y vuelve a reclamar launch | `internal/adapters/state/sqlite/claim.go` CASE/ORDER BY y `internal/bootstrap/scheduler.go`; el test SQLite existente solo cubre launches no admitibles por presupuesto y el test bootstrap usa secuencia artificial sin backlog | priorizar observes ejecutables antes de launches (mantener stop urgente), cubrir backlog admitible + observe terminal y scheduler con launches continuos; medir que una observación no espera vaciado de backlog. Separar carril/concurrencia de observación si el SLA exige no esperar ni un launch síncrono |
