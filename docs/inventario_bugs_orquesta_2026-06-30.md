...
| BUG-ORQ-20260805-659 | abierto; candidato PFC-07a rechazado antes de commit | instalador root de Agente MicroVM | el candidato usaba alias/versionado ilegal de slice y ruta cgroup imposible, podía dejar servicio anterior y nuevo activos durante upgrade y confiaba en `TMPDIR` bajo root; rollback/status también perdían estado o activos críticos | el fake de systemd simplificó en exceso la semántica real y el diseño content-addressed se aplicó a una unidad slice que no admite aliases | revisión independiente NO-GO; no se ejecutó sudo, systemd ni KVM; corrección exige slice estable, upgrade de escritor único, temporal root-trusted, receipt de estados exactos y negativos realistas | repetir revisión independiente y arnés tras la corrección; PFC-07b físico debe acreditar systemd/cgroup/UDS antes de PFC-06 |
