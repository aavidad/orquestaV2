...
| BUG-ORQ-20260711-222 | abierto | Goal-first/consumo/control de proceso | un goal estrecho materializo 65 lineas y test verde, pero supero 1,1 M tokens sin result durable; el observer pidio stop por alto consumo y el proceso siguio hasta control forzado | la politica automatica persiste peticion cooperativa sin demostrar actuacion y ausencia del proceso; progreso/checkpoint no corta repeticion interna | goal externo `019f4fa2-e2d1-7e62-9f4c-b3be100a7833`, estado/runtime `state-2`/`runtime-2`; [incidencia 221-225](incidencias/incidencia_orquesta_autoprogramacion_local_preflight_consumo_y_scope_2026-07-11.md) | cablear corte automatico a actuador confirmado y probar proceso ausente, recibo terminal e idempotencia |
| BUG-ORQ-20260711-223 | abierto | Goal-first/observacion HTTP concurrente | `POST /api/v0/autoprogramming/goal/observe` devolvio 500 generico mientras el observer residente observaba el mismo goal con ticks `ok` | la contencion o lease concurrente se proyecta como error interno en vez de estado recuperable con accion publica | HTTP audit en `state-2`, run `run-run-codex-preflight-20260711-001`; [incidencia 221-225](incidencias/incidencia_orquesta_autoprogramacion_local_preflight_consumo_y_scope_2026-07-11.md) | reproducir concurrencia y devolver retry/observe recuperable sin perder causalidad |
| BUG-ORQ-20260711-224 | abierto | Autoprogramacion/write-set/promocion | el rework declaro dos ficheros pero modifico tambien `daemon.go`; la integracion manual tuvo que retirar el cambio | el write-set congelado informa al agente, pero no impide ni bloquea automaticamente escrituras fuera de alcance antes del cierre | run `run-run-preflight-rework-20260711-001`, runtime `runtime-3`; [incidencia 221-225](incidencias/incidencia_orquesta_autoprogramacion_local_preflight_consumo_y_scope_2026-07-11.md) | atestar snapshot/diff contra write-set y exigir rework causal para ampliar alcance |
| BUG-ORQ-20260711-225 | cerrado localmente | Tooling/perfil aislado/umask | la suite aislada produjo falsos rojos en pruebas que crean permisos inseguros deliberados; fuera del perfil ambas pasan | `orquesta_use_isolated_test_env` dejaba `umask 077` activo en el shell llamador y transformaba `0770/0777` en `0700`, invalidando el fixture de seguridad | prueba de exito/error y modos privados en `test_orquesta_test_batches.sh`; focales de secreto y tool store dentro del perfil; [incidencia 221-225](incidencias/incidencia_orquesta_autoprogramacion_local_preflight_consumo_y_scope_2026-07-11.md) | cierre: setup conserva `0700/0600`, restaura exactamente el umask en exito/error y no contamina la suite |
