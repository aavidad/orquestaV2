...
| --- | --- | --- | --- | --- | --- | --- |
| BUG-ORQ-20260705-191 | abierto | Remoto/autoprogramacion goal-first integracion Git | en `srv1651826`, el goal G5 `request-ref-remoto-wizard-bot-llm-20260705-001` termino con `orquesta_goal_result.v0 status=complete`, tests declarados verdes y cambios de codigo bajo `pilot-remoto-1`, pero el worktree principal/GitHub quedo en `885e76b0` sin los ficheros G5; el result no trae `commit_sha`; `/api/v0/autoprogramming/status` muestra cola `count=0`; ademas el worktree principal tiene `origin=/tmp/orquesta-self.bundle`, no GitHub, por lo que `git ls-remote origin trabajo/plataforma-agentes` falla | Orquesta confunde entrega terminal del agente con integracion operacional del cambio; la promocion/revision Git queda fuera del contrato durable y si el revisor no integra, el sistema declara cola vacia aunque quedan cambios recuperables sin commit; el remoto tampoco valida que su remote Git canonico pueda hacer fetch/push antes de declarar sincronizacion | incidencia `docs/incidencias/incidencia_orquesta_remoto_g5_complete_sin_integracion_git_2026-07-05.md`; evidencia: `modulos/orquesta-web/docs/orquesta_goal_result_goal-ref-task-autoprogramming-0cc9487d6778-g01.json` en `/srv/orquesta-self/worktrees/pilot-remoto-1` con `status=complete` y `commit_sha=null`; `git -C /srv/orquesta-self/worktrees/pilot-remoto-1 status` muestra 5 modificados + 5 untracked; `git -C /srv/orquesta-self/worktrees/orquesta status` limpio en `885e76b0`; `git -C /srv/orquesta-self/worktrees/orquesta remote -v` apunta a `/tmp/orquesta-self.bundle`; nightly 20260705 verde solo en preflight, no valida G5 integrado | Pendiente: anadir estado/contrato de `integration_receipt` para goals de codigo (`commit_sha`, branch, remote, push_state, reviewer/ref); si falta, proyectar `pending_integration` o `blocked_push`, no cola vacia cerrada; validar remote Git canonico en servidor remoto; crear test/smoke que reproduzca result complete sin commit y exija accion de integracion |
