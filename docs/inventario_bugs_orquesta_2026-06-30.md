...
| BUG-ORQ-20260806-700 | abierto; contrato sibling publicado, ledger pendiente | Agente MicroVM/B11, detención física y reconciliación | la API sibling exponía una única detención sin modo y devolvía solo la ejecución actualizada; Orquesta no podía distinguir cooperativa de forzada ni obtener `receipt_ref`, instante físico confirmado y replay causal exacto | el motor ya producía un comprobante de retirada de recursos, pero el servicio y el ledger lo descartaban; la idempotencia conservaba resultado lógico, no el recibo físico | `agente_microvm@daf8469` define y valida modo solicitado/efectivo, `receipt_ref` y timestamp en el comprobante público; aún no acredita persistencia/ledger, replay tras restart, reconciliación de respuesta perdida ni efecto físico | no anunciar capacidad B11 hasta demostrar que cooperativa no escala por sí sola, forzada sí puede hacerlo, respuesta perdida reconcilia el mismo recibo y restart no fabrica timestamps ni autoridad |
