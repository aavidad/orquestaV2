...
## Incidencias detectadas al recuperar V23.3

| ID | Estado | Superficie | Síntoma | Causa | Evidencia/corrección | Criterio pendiente |
| --- | --- | --- | --- | --- | --- | --- |
| BUG-ORQ-20260729-597 | abierto; candidato preservado e integración manual acotada | V23.3/Council requerido tras rework | el WorkItem del codec conservaba atestación PASS, dos reviews `approve`, autor `awaiting_integration` y `council_policy=required`, pero `orquesta.council.round.open` devolvió `conflict` incluso tras reclamar un lease nuevo y válido; `orquesta.changes.integrate` quedó correctamente bloqueado por falta de resolución Council | por aislar en la escritura SQLite de la ronda o en su validación de frontera; la API pública solo proyectó `conflict` y no permite distinguir qué invariante durable falló | Goal `goal:cea02be00047f6e695b75b5ba99ba5a6`, change-set `change-set:803b22899643488d7382cff226f7178d`, lease fence 6 válido hasta `2026-07-29T08:32:45Z` y requests `request:root-v23-council-open-20260729-2` / `request:root-v23-codec-integrate-20260729-1`; el mismo árbol se aplicó limpio sobre HEAD, pasó `internal/wizard/gaps -race`, aplicación y SQLite completo, y se integró manualmente en `1e396b98` sin fabricar receipt ni mutar la base | reproducir la ronda sobre retry/rework en application y SQLite, conservar la causa tipada hasta commands y demostrar apertura, tres contribuciones, decisión e integración tras restart; la excepción manual no cierra el bug |
| BUG-ORQ-20260729-598 | abierto; Goal nuevo evita el camino roto | V23.3/workspace Git de rework y atestación | el rework SQLite declaró `parent_change_ref=f5bc53f6…`, pero su changeset físico `7401f0bb…` volvió a basarse en `d326a04b…`; perdió la migración 022 y la constante V21, y el required test falló al compilar con `undefined: recoverySchemaV23WizardInputs` | `parent_change_ref` se conservó como metadata causal sin materializar ni verificar que el árbol Git del workspace incluyera el padre declarado; la prueba local usó una composición temporal distinta del subject realmente atestado | execution `execution:8f355afa7ed869094196ca5da9278ad2`, required test `required-test:v23-3-sqlite-schema21-rework`, change-set `change-set:0dc14acedf59cc43649a3c382278fdbc`; la recuperación usa un Goal nuevo sin parent lógico y exige `WorkspaceBinding.base_oid == git HEAD` antes de editar | ligar rework/parent a una base física verificable o rechazar la preparación; añadir negativo donde metadata padre y árbol divergen y exigir que atestación pruebe exactamente el mismo subject que el autor |
| BUG-ORQ-20260729-599 | cerrado en código `ff16353d` | V21/i18n y aceptación acumulativa | `TestTypedExtractorFindsRegistryCatalogAndPublicDocsKeys` fallaba en el HEAD limpio porque el extractor tipado ya descubría las claves Wizard de dossier/gaps/catalog, pero la lista `actual` del ratchet V21 seguía limitada al catálogo previo | las superficies i18n de V23 crecieron sin actualizar atómicamente el inventario acumulativo que V21 usa como autoridad negativa | `ff16353d` acumula 813 claves Wizard desde APIs tipadas, conserva los negativos de literales y claves huérfanas y limita el techo histórico a superficies V21; focal normal y `-race` verdes, y aceptación completa verde sobre `6830e54e` | conservar la extracción desde fuentes tipadas y no volver a usar una lista manual parcial para superficies acumulativas |
| BUG-ORQ-20260729-600 | cerrado en código `a7a5768b`; despliegue runtime pendiente | Goals/cancelación temprana, lectura posterior y sesión de ejecución | un Goal SQLite recién lanzado se canceló correctamente por una precondición SHA errónea: Goal, WorkItem y execution quedaron `canceled` y el control `confirmed`, pero `orquesta.goals.get` devolvió HTTP 500 inmediatamente después; la acción durable de revocar la sesión siguió reintentándose con `state.invalid` | la cancelación ocurrió tras un `commit_change` de efecto externo incierto: quedó artefacto staged y un receipt `quarantined + application.effect_unknown_applied`, sin `ChangeSet`; el validador solo preservaba el caso retirado antes del efecto y volvía ilegible el agregado antes de que la revocación alcanzara el broker | Goal `goal:b7161aa4919e0560e19c741f8ab4a8ca`, execution `execution:47954e80c5abab8a177a4e5ad12f9dea`, control `control:170dbdd53d5913e06dcf26eceb7fc3c9`; a las 11:14 la acción `action:revoke-execution-session:execution:47954e80c5abab8a177a4e5ad12f9dea` llevaba 17 intentos/fence 40. `a7a5768b` conserva la evidencia ambigua sin declararla aplicada y acredita lectura inmediata, restart, recovery, primer fallo de broker y revocación idempotente con un único receipt; suite SQLite completa y compilación global verdes | desplegar el binario corregido cuando termine el Goal V23 activo, comprobar que la acción histórica completa la revocación sin editar filas y que `goals.get` vuelve a leer el agregado cancelado |
| BUG-ORQ-20260729-601 | cerrado en código; descubierto al integrar | V02/aceptación sparse y transporte `git cat-file --batch` | tres ejecuciones del focal V02 quedaron vivas sin CPU útil: el test enviaba todos los OID por `StdinPipe` antes de leer stdout, por lo que `cat-file` llenaba su pipe de salida, dejaba de consumir entrada y ambos extremos esperaban indefinidamente; además, una raíz parcialmente visible como `cmd/` omitía blobs `skip-worktree` | el helper trató entrada y salida de un protocolo bidireccional como dos fases secuenciales y solo usó el índice cuando la raíz física completa no existía | se terminaron cooperativamente los tres focales propios, se pasó la entrada como `io.Reader` para que `os/exec` la copie mientras se drena stdout y se detectan entradas `S` aun con raíz parcial; focal V02 normal y `-race` pasan en segundos | conservar prueba con volumen superior al pipe y una raíz mixta visible/skip-worktree; cualquier helper `cat-file --batch` futuro debe drenar mientras alimenta y no declarar verde con procesos de test aún vivos |
| BUG-ORQ-20260729-602 | cerrado en código `70920ea2` | SQLite/V18 y coste patológico del fixture de migración | el paquete SQLite del Goal V23.3 y el focal V18 podían agotar sus timeouts durante la síntesis de una base legacy, antes de alcanzar la migración productiva que pretendían probar | los helpers abrían la fixture con durabilidad SQLite síncrona y hacían migraciones y copias mediante autocommits; con `/dev/sdb` bajo presión de I/O generaron 1.241 `fsync`, 71,9 segundos acumulados y máximos de 2,7 segundos, mientras el mismo caso en `tmpfs` tardaba unos 0,78 segundos por subcaso | `70920ea2` limita `synchronous=OFF` a la construcción test-only de la fixture y afirma el pragma; el `Open` productivo sigue usando `FULL`. En ext4 pasan focal por diez, migraciones V18 más ratchet `FULL` y focal `-race`; no se tocaron VMs ni procesos ajenos | conservar separadas la velocidad de síntesis y la durabilidad del subject: ninguna optimización de fixtures puede cambiar el perfil `FULL` del `Open` productivo ni relajar la negativa de migración sin drenar |
| BUG-ORQ-20260729-603 | abierto; primer candidato rechazado | comandos públicos/efectos pendientes | los `prepare_workspace` y `agent_launch` sensibles quedaban correctamente aparcados por falta de aprobación, pero `orquesta.goals.get` no mostraba al operador su `intent_ref` y digest; los agentes parecían atascados en `queued` aunque Orquesta esperaba una decisión explícita | la proyección de comandos duplicaba parcialmente la política de admisión y calculaba «aprobación actual» sin la misma foto temporal, membresías vigentes, retry de cero-release ni decisión denegada que usa el scheduler | reproducción del 2026-07-29: los Goals Firecracker y BUG595 arrancaron inmediatamente al decidir los intents recuperados desde SQLite; la auditoría premium rechazó el candidato `a018e700` porque también ocultaba retries válidos y podía preferir una aprobación vieja frente a una denegación concurrente | crear una consulta/DTO seguro en application que reutilice el evaluador real de admisión y una foto coherente; exponer pending/denied sin `reason`, cubrir revocación de membresía, cero-release, separación crítica y comprobar fugas sobre el JSON crudo |
| BUG-ORQ-20260729-604 | abierto; contrato diagnosticado | runtime Codex/disposición de entrega | una salida válida `blocked` podía recorrer el mismo camino que un artefacto `produced` y preparar un `commit_change`, convirtiendo un bloqueo explícito del agente en candidato de código | el adaptador reducía dos disposiciones semánticas a la presencia de un documento y el journal no conservaba una decisión tipada antes de crear efectos Git | contrato propuesto y aún no integrado: disposición cerrada `produced|blocked`, journal durable schema 8, `agent_diagnostic` para el bloqueo y negativos de restart/replay/race que prueben cero commit ante `blocked` | implementar después de BUG595 por el write-set compartido de application/SQLite; conservar el trabajo bloqueado como diagnóstico/rework y no clasificarlo como basura ni éxito |
| BUG-ORQ-20260729-605 | abierto; causa reproducida y dos Goals de corrección activos | TestAttestor/Firecracker, capacidades Go y diagnóstico seguro | una required test `go test -race` pasó en host, pero el attestor microVM devolvió inmediatamente `exit_code=2` y solo conservó `output_digest`; la barrera impidió integrar, pero la API no explicó la incompatibilidad | el guest fija deliberadamente `CGO_ENABLED=0`, mientras la admisión acepta `-race`; Go exige CGO para el race detector. El contrato durable preserva digest para no filtrar stdout/stderr, pero no anuncia ni valida esa capacidad antes de lanzar | reproducción exacta: `CGO_ENABLED=0 GOTOOLCHAIN=local go test -race -run '^$' ./acceptance` devuelve 2 con `go: -race requires cgo`; BUG580 se reacredita sin `-race` en microVM y conserva la evidencia host `-race`; Goal `goal:e6b6b169612303915f0bc5a45e56a674` corrige el contrato de capacidad | declarar capacidad tipada CGO/race del asset/adaptador, rechazar fail-fast antes de snapshot/launcher con código estable y cero efecto parcial, aceptar `-race=false`, conservar salida bruta fuera de la proyección pública y dejar soporte CGO real como corte independiente |
| BUG-ORQ-20260729-606 | abierto; causa reproducida y reacreditación focal activa | TestAttestor/Firecracker, dependencias de herramientas de required tests | `go test -mod=vendor -count=1 ./acceptance` era formalmente una required test `tool:go`, pero varias pruebas históricas V21/V22 ejecutan `git`; el guest solo expone `PATH=/toolchain/bin`, por lo que la microVM devolvió `exit_code=1` aunque las pruebas V23 del cambio sí pasaron | el contrato declara únicamente el ejecutable superior y no inventaría las herramientas hijas que el binario de test puede lanzar; la atestación tampoco proyecta la causa segura y solo conserva el digest de salida | attestation `attestation:required-tests:execution:9405a785cb3efd8bcf70be785637e4a1`, digest `6eadc3a5…`; reproducción con el entorno mínimo del guest mostró `exec: "git": executable file not found in $PATH` en V21/V22 y verde en las cinco pruebas V23; Goal focal `goal:dfe8f7a967c3e649c7b1d12ba98eec54` conserva suite completa y `-race` como evidencia host | modelar dependencias auxiliares/capacidades de herramienta de la required test o separar suites herméticas de suites host; no añadir Git al guest ni ampliar su superficie de ataque sin contrato, asset, digest y revisión propios; reacreditar BUG580 en Firecracker con el scope V23 exacto |
| BUG-ORQ-20260729-607 | abierto; causa confirmada y alcance ampliado | scheduler SQLite/bootstrap, progreso durable frente a launches Codex | `terminal.json` válido quedó sin observar 11m52s y 4m52s; después, un `commit_change` ya aprobado siguió en `awaiting_commit` mientras launches de reviewers sin capacidad se reintentaban | `ClaimNextAction` ordena `launch_agent` (2) antes de `commit_change` (3), `attest_test` (4), `integrate_change` (5) y `observe_agent` (6); el único scheduler llama `ProcessNext` síncrono y, tras el pseudo-límite de dos launches, reinicia con delay cero y vuelve a reclamar launch | `internal/adapters/state/sqlite/claim.go` CASE/keyset/ORDER BY y `internal/bootstrap/scheduler.go`; el test SQLite existente solo cubre launches no admitibles por presupuesto y el test bootstrap usa secuencia artificial sin backlog; reproducción adicional con cuatro perfiles ocupados y dos reviews aprobadas dejó detrás el commit aprobado de BUG481 | priorizar acciones ejecutables de progreso durable antes de nuevos launches, manteniendo `stop_agent` urgente y `prepare_workspace` causal; cubrir backlog admitible frente a commit/attest/integrate/observe y scheduler con launches continuos. Un launch ya dentro de `ProcessNext` puede seguir añadiendo su latencia; un carril/worker separado queda como segunda fase si el SLA exige eliminarla |
| BUG-ORQ-20260729-608 | abierto; fallo aislado, causa interna guest pendiente | TestAttestor Firecracker y regresión shutdown Codex | el parche BUG481 pasó host, `-race`, `vet` y revisión premium, pero su required test combinado falló en microVM; al dividirlo, `blocked-ready` y `deadline-descriptors` pasaron y solo `stop-observe-ready` devolvió exit 1 | la división descarta fallo general de toolchain, selección o lifecycle básico; queda una diferencia guest en el caso que combina persistencia de launch, Stop/Observe concurrentes y ausencia de stop request después de Shutdown, ejecutado como UID/GID 65534, sin grupos/capacidades y sobre scratch tmpfs | attestation `attestation:required-tests:execution:0a6ad1c01e2a0aa1d6e7b4f234ff518f`: outcomes 0/0/1; el mismo tercer test pasó 500 rondas host con `CGO_ENABLED=0`, `GOPROXY=off` y `GOMAXPROCS=2`; Firecracker conserva solo `output_digest`, por lo que todavía no identifica la aserción exacta | no integrar ni debilitar el rail; separar el tercer escenario en required tests causales (`unavailable`, drenaje, no efecto durable), probar count 1/2/10 en procesos guest distintos y mejorar diagnóstico seguro enlazado con BUG606; conservar el guard ya acreditado hasta demostrar defecto de producción |
| BUG-ORQ-20260729-609 | workaround operativo cerrado; preflight y diagnóstico pendientes | workspace Git local y selección de semilla | el primer Goal de la tanda V23 quedó en `queued` con `prepare_workspace` en cuarentena `application.effect_unknown_applied`; no creó workspace ni lanzó Codex | el clon principal había activado `extensions.worktreeConfig=true` para sostener configuración distinta entre sus numerosos worktrees; el adaptador Git lo rechaza correctamente, pero la comprobación ocurrió después de abrir el intento físico y la causa segura quedó reducida a efecto ambiguo | Goal `goal:0f6aae64ce7e067b53f3a12f35977819`, acción `action:prepare-workspace:execution:11197d7fccca5c82a19568fdfb7fd85f`, cero workspace y cancelación pública confirmada. Se creó un clon semilla privado independiente, sin `extensions.worktreeConfig`, y el sucesor `goal:bf6e22b110354db1ad95a1c23a51ea3e` preparó workspace y lanzó Codex12 sin cuarentena | validar los controles del seed antes de crear el intento durable y exponer un código seguro de preflight; no relajar la prohibición ni modificar el clon principal, porque rompería sus worktrees. Mantener el clon semilla independiente como composición operativa |
| BUG-ORQ-20260729-610 | abierto; candidato y atestación preservados | V15/V18, presupuesto de procesos para autor y revisores | V23-01 produjo un change-set y pasó su required test en Firecracker, pero los dos revisores quedaron indefinidamente en `queued` con `budget.temporarily_unavailable` aunque la composición se reinició como pool de tres perfiles | el Goal nació con `process_slots=1`; la reserva del autor se conserva mientras está `awaiting_integration`, pero V18 exige dos ejecuciones revisoras antes de integrar. Los envelopes son inmutables y reiniciar el adaptador con más concurrencia no puede corregir esa contradicción causal | Goal `goal:bf6e22b110354db1ad95a1c23a51ea3e`, autor `execution:ed0826a325bfadb23d1394a5ab3948db`, change-set `change-set:c1593e475483017b781fa813c9df0c5e`, atestación `attestation:required-tests:execution:ed0826a325bfadb23d1394a5ab3948db` PASS y revisores `execution:review:2e4c039515f1bdaa528845d80f9cc6f108956a73627c1929a9ce363cc8fb7439` / `execution:review:2447c92ef343cd02d87e70598bb035003509156a77e16e16ae8bc926cb6e1bb4` sin reserva. No se modificó SQLite; la composición pool queda válida para Goals nuevos | admission debe rechazar o elevar de forma explícita la demanda mínima del plan cuando la política requiere revisores, o liquidar el slot físico del autor sin perder su reserva de tokens/dinero; cubrir autor terminado + dos reviews, restart, varios HOME y cierre sin ampliar envelopes históricos |
| BUG-ORQ-20260730-611 | abierto; rework focal activo | runtime de revisores y contrato del artefacto de evaluación | dos revisores produjeron findings útiles, pero sus ejecuciones terminaron `review.assessment_invalid` porque `evidence_ref` superó el límite interno de 512 bytes que el prompt de lanzamiento no publicaba; el primer arreglo preservó el contrato correcto pero nombró el test de forma distinta a la required test y Firecracker lo rechazó con código 254 | validación y prompt tenían autoridades desalineadas; además, la autoría no cotejó el nombre exacto del test requerido antes de entregar | candidato `change-set:60b53618de802cfe892cadc6c46f1e39`; Goal de rework `goal:5a18a5071ae6bc9b0e13dc5c35f6eeda`, ejecución `execution:95f3efc68a5f2494ab06ca80bd5f3e0c`; el rework conserva límites reales y renombra el focal a `TestReviewerLaunchRequestPublishesReviewPayloadLimits` | obtener atestación, doble review e integración; después desplegar binario y demostrar que un revisor recibe todos los límites y no repite por payload recuperable |
| BUG-ORQ-20260730-612 | corregido localmente; acreditación e integración pendientes | `GOV-11`/`EVD-05`/`STG-06`, lanzamiento del Consejo con capacidad temporal agotada | el crítico del Consejo del Goal `goal:5a18a5071ae6bc9b0e13dc5c35f6eeda` agotó sus tres intentos como `agent.launch_failed`, aunque ninguno obtuvo proveedor, referencia externa ni recibo de lanzamiento; defensor y árbitro sí arrancaron al liberarse perfiles | `dispatchLaunchEffect` trataba toda ejecución del Consejo antes de comprobar si el error era temporal y estaba acreditado como definitivamente no aplicado; `codex.capacity_unavailable` consumía un intento y creaba un sustituto en vez de conservar la misma ejecución en espera | los intentos del crítico `execution:council:7054…`, `b63b…` y `59e3…` carecen de efecto físico; el arreglo prioriza cancelación, después indisponibilidad temporal y solo entonces el reemplazo por fallo real del Consejo; `TestCouncilTemporaryCapacityKeepsTheSameExecutionsUntilSlotsRecover` exige tres liberaciones exactas, cero sustitutos y arranque posterior de las mismas tres ejecuciones | ejecutar focal normal y con carrera, paquete de aplicación y persistencia tras reinicio; desplegar antes de someter el conjunto `change-set:b3fe43c194156e13d524ded6cdd9e426` a un Consejo nuevo, sin reutilizar ni puentear la ronda agotada |
| BUG-ORQ-20260801-613 | cerrado en código; acreditación V38 pendiente | `ORC-28`/A04/A05, capacidad física previa al claim | el plan y el handoff declararon A05.4 cerrada y habilitaron A04.2 aunque el runtime no producía ni entregaba candidatos físicos al reclamo real | se contó la existencia del observador estático, la ordenación neutral y el arranque de controladores de cuota como si compusieran toda la cadena; `staticcapacity` no tenía consumidor productivo, `Orchestrator` no recibía fuentes o candidatos y el `ClaimRequest` conservaba una observación singular que `processing` no rellenaba | `e7f9e60e`+`993e3d03` completaron el productor y `a6c326d7` materializa/revalida la observación, reserva y liga una única colocación dentro del mismo claim; suites completas de SQLite, Codex, bootstrap, `internal/**`, `cmd/orquesta`, `vet` y focales `-race` verdes, sin arrancar Orquesta ni alterar Goals | A04/A05 quedan ejercitadas como parte de la compuerta A; no acreditar `ORC-28` ni V38 hasta completar A06–A08 y las compuertas B/C sobre el mismo candidato |
| BUG-ORQ-20260802-614 | abierto, intermitente no reproducido | `ORC-28`/bootstrap, E2E de entorno Go con cuota y colocación | una ejecución de la suite completa dejó `TestCodexProductionProcessReceivesPinnedGoEnvironment` en `dispatching` durante los 15 s del observador, con un intento y cero receipts; el mismo focal pasó tres veces seguidas entre 4,3 y 6,2 s y la suite completa posterior pasó en 31,4 s | no determinada; la evidencia solo prueba latencia aislada durante `Launch`, no fuga, error de cuota, pérdida de colocación ni deadlock | se preservó el límite de 15 s para no ocultar bloqueos; Codex completo, bootstrap completo posterior, suite transversal y cero procesos residuales quedaron verdes | reproducir con una barrera causal o trazas seguras antes de cambiar timeouts o producción; si reaparece, conservar estado de supervisor, cgroup y proceso exactos sin registrar entorno sensible |
| BUG-ORQ-20260804-615 | cerrado en `agente_microvm` `df6469c`; acreditación física B12 pendiente | V38/B09, huésped Firecracker y artefactos musl | las 221 pruebas Rust y clippy pasaban en host, pero el primer rebuild real `x86_64-unknown-linux-musl` falló con `E0308` en las tres llamadas `SIOCGIFFLAGS`/`SIOCSIFFLAGS`; el candidato B09 no podía producir su initramfs aunque figuraba ejercitado | el tipo de request de `libc::ioctl` es `Ioctl` específico del target; el build GNU admitía las constantes y ocultaba que musl requería conversión explícita | `df6469c` tipa las operaciones y hace obligatorio el build musl locked; dos builds aislados dieron huésped `44941753…`, initramfs `837a38ed…` y dos perfiles ext4 `04954ea4…` idénticos; `bb90346` actualiza SBOM/contrato y `cc4ef47` conserva la evidencia, con suite completa y cero KVM arrancado | mantener la compilación cruzada en la verificación mínima del huésped; no confundir reproducibilidad con acreditación física y validar ioctl, AF_VSOCK, CONNECT, límites y limpieza en B12 |
| BUG-ORQ-20260804-616 | abierto; plan corregido antes de programar | V38/B10, contrato entre ciclo físico y trabajo del agente | el plan pedía implementar `AgentLauncher/Observer` sobre el cliente v1 aunque `Lanzar` solo crea la microVM y `Observar` solo informa estado físico; mapear `disponible` a `AgentRunning` habría producido otro falso verde | faltan una sesión/resultado de trabajo públicos, comprobante causal, descriptor físico negociado y hechos durables de autorización/efecto/cerca para repetir la concesión byte a byte tras reinicio | `docs/reconstruccion/adr_v38_b10_frontera_trabajo_agente_2026-08-04.md` separa `B10.0`, autoridad, dependencia, conector, composición y recovery; mantiene Codex físico real en B12.4 y prohíbe inventar refs o reinterpretar estados | completar primero la API hermana neutral y fijar después su módulo Go; B10 solo queda `wired/exercised_without_kvm` hasta que B12 acredite la microVM real |
| BUG-ORQ-20260804-617 | cerrado en `agente_microvm` `a8d19fe`; ledger B10.0b2 pendiente | V38/B10.0b, autoridad de eventos de sesión | el modelo durable incrementaba `revision_sesion` únicamente al cambiar de estado, mientras el contrato público exige una revisión de sesión estrictamente creciente por cada evento `stdout`, `stderr` o truncación; el primer borrador del puerto habría asignado secuencia en el repositorio | contrato, agregado y ledger se diseñaron en cortes distintos sin fijar primero quién posee la secuencia de eventos; dejarla al adaptador crearía una segunda autoridad y permitiría que snapshot y stream discrepasen | `a8d19fe` hace al agregado autoridad de secuencia, revisiones, entradas sin evento, paso ambiguo, truncación, trabajo reservado y snapshots antes/después; cuatro revisiones premium cerraron P0/P1/P2 y pasaron 212 pruebas, Clippy y build musl, sin KVM | el puerto y SQLite deben persistir `CambioSesionTrabajo` exacto y contrastar receipts/eventos; no pueden recalcular secuencia, revisiones, resultado ni historia |
| BUG-ORQ-20260804-618 | cerrado en `agente_microvm` `ad14ad6`; puente B10.0c pendiente | Agente MicroVM, handshake anfitrión→huésped vsock | el helper de conexión al servicio huésped aceptaba cualquier respuesta `OK <puerto>` después de solicitar `CONNECT 10000`, por lo que podía tratar como correcto un enlace confirmado para otro puerto | la validación comprobaba el prefijo/formato general del ACK pero no la igualdad exacta con el puerto solicitado; B08/B09 usaban otros sentidos y servicios, y no ejercitaban esta frontera de sesión | `ad14ad6` exige byte a byte `OK {puerto_solicitado}\n` y cubre puerto distinto, espacios, CRLF, formato incompleto y trailing data; focal y motor completos verdes, dos smokes KVM omitidos, sin arrancar KVM | conservar el negativo exacto al integrar el canal B10.0c; el cierre del helper no acredita aún sesiones, puente ni agente físico |
| BUG-ORQ-20260804-619 | cerrado en `agente_microvm` `3fc86eb`; SQLite B10.0b3 pendiente | V38/B10.0b2, ledger durable y replay de sesión | la suite del primer draft estaba verde en memoria, pero un replay tras reinicio era imposible: confirmar exigía recrear el `Cambio` privado y no preservaba la intención de entrada | faltaban reserva durable de entrada/intención, consulta de pendientes y CAS atómico contra la `revision_trabajo` canónica de ejecuciones; el diseño confundía verificación en memoria con recuperación durable | `3fc86eb` conserva intención exacta y fases reservada/intentada, reconstruye receipts sin fabricar el cambio consumido, fija CAS/efecto atómico y rechaza colisiones de clave, intención, identidad o resolución; auditoría final GO, 218 pruebas activas, Clippy estricto y build musl verdes | B10.0b3 debe demostrar estas invariantes en SQLite v15 con restart, replay, rollback y cuarentena; este cierre no acredita persistencia ni un agente real |
| BUG-ORQ-20260804-620 | cerrado en `agente_microvm` `50df8e6` | B10.0b2, restauración de sesión y paso ambiguo | la restauración aceptaba `Finalizada`/`Interrumpida` con secuencia 1 sin paso por ambigüedad, estado imposible por el camino vivo | el validador de restauración no distinguía el mínimo causal de una secuencia terminal directa del caso con transición ambigua | `50df8e6` exige secuencia >=2 sin ambigüedad y admite 1 con ambigüedad; prueba simétrica, focal y Clippy verdes | conservar la simetría entre restauración y camino vivo; el cierre no acredita aún el ledger SQLite B10.0b2 |
| BUG-ORQ-20260804-621 | cerrado en `agente_microvm` `56f1426` | V38/B10.0b3a, recuperación de entradas de sesión | el primer esquema v15 permitía varias entradas `reservada`/`intentada` para una sesión, pero el contrato de recuperación solo representa una entrada pendiente | se comprobó tabla por tabla sin contrastar la cardinalidad global del read-model `SesionTrabajoRecuperable` | `56f1426` añade índice parcial único para una entrada pendiente por sesión y prueba el rechazo de una segunda reserva/intento | mantener serialización por sesión en B10.0b3b; no convertir entradas resueltas históricas en pendientes ni crear efectos activos propios para ellas |
| BUG-ORQ-20260804-622 | cerrado en `agente_microvm` `eecd42a` | V38/B10.0b3b1, lectura y paginación del ledger de sesiones | el primer lector SQLite pasaba la suite aunque podía reconstruir otra intención de inicio, aceptar eventos de otra ejecución, ocultar huecos y anunciar terminal sin acreditar el evento final y su resultado; el primer arreglo también rechazaba el polling vacío desde el cursor terminal | las FK y el estado agregado se trataron como prueba suficiente sin contrastar la cadena causal completa ni los replays de cursor | `eecd42a` liga la intención a `clave_inicio`, valida ejecución/cerca/continuidad, revalida el último evento y resultado mediante el receipt de dominio y cubre terminal multipágina y replay vacío; auditoría final GO, 224 pruebas activas, Clippy y musl verdes | todo read-model durable nuevo debe probar corrupción cruzada, huecos, última página y consulta repetida después del terminal; un productor no determinista nunca convierte por sí solo un snapshot parcial en verdad |
| BUG-ORQ-20260804-623 | cerrado en `agente_microvm` `6169073` | V38/B10.0b3b2, receipts y recovery conservador | el primer lector de receipts era exacto y sus focales estaban verdes, pero recovery ignoraba un receipt ya existente si la operación seguía marcada `reservada` o `intentada`, por lo que podía presentar como relanzable un efecto contradictorio | se validó el receipt al consultarlo directamente, no la coherencia conjunta entre fase, receipt y decisión de recuperación | `6169073` exige receipt válido para toda operación `resuelta`, ausencia para toda pendiente y falla cerrada ante pendiente+receipt tanto en inicio como en entrada; auditoría final GO y suite completa verde | las futuras escrituras deben cambiar fase, snapshot y receipt en una sola transacción; nunca inferir que una operación es segura de repetir solo porque su fase siga pendiente |
| BUG-ORQ-20260804-624 | cerrado operacionalmente; prevención permanente documentada | tooling de agentes e higiene de búsquedas | un subagente lanzó `find /home/alberto/Trabajo` para localizar un único archivo Rust ya conocido y el proceso quedó en estado `D`, consumiendo CPU sin producir resultado útil; terminar el shell padre no pudo retirar inmediatamente el hijo bloqueado | se usó una búsqueda global por filesystem en vez de la ruta exacta y `rg` acotado exigidos por el contrato operativo; una espera de E/S no interrumpible tampoco responde a `SIGTERM` hasta volver del kernel | PID `2563395` y un segundo `find .. -name AGENTS.md` fueron censados antes de enviar `SIGTERM`; ambos desaparecieron al volver del kernel, el subagente recibió orden de usar rutas exactas y el censo posterior dejó cero búsquedas propias residuales | los prompts de programación deben proporcionar rutas exactas y prohibir también `find` global; antes de cerrar la sesión hay que recensar los PIDs, sin matar por patrón ni tocar procesos ajenos |
| BUG-ORQ-20260805-625 | cerrado en `agente_microvm` `d4f252d` | arnés Rust/API Unix y temporales concurrentes | la suite completa falló de forma intermitente en `rechaza_directorio_accesible_por_otros` con `AlreadyExists` antes de probar la guarda de permisos | varias pruebas del mismo proceso construían el directorio temporal solo con PID y reloj; la resolución efectiva del reloj permitió dos nombres iguales bajo ejecución concurrente | `d4f252d` añade una secuencia atómica local al nombre; focal y suite completa posterior pasan con 236 pruebas de librería, 38 de binarios/integración y dos smokes KVM ignorados explícitamente | toda factoría temporal concurrente debe combinar PID, reloj y secuencia o usar creación exclusiva con reintento; un fallo de preparación no acredita la conducta bajo prueba |
| BUG-ORQ-20260805-626 | cerrado en `agente_microvm` `cbfc672` | V38/B10.0b3b, recovery causal SQLite de sesiones | varios focales verdes aceptaban estados imposibles: inicio resuelto sin receipt/evento coherente, entrada resuelta sin receipt, contador de entradas mayor que su ledger, receipts que revertían historia, avance desde ambigüedad y `ultima_secuencia` con eventos ausentes | lectura, escritura y recovery validaban tablas o snapshots por separado; ninguna comprobación reproducía en orden la cadena conjunta de inicio, entradas y observaciones mediante la autoridad del agregado | `cbfc672` reconstruye cada receipt en orden, exige snapshot final exacto, cuenta eventos y entradas, usa dedupe indexado, serializa entradas pendientes y añade negativos para cada corrupción; auditorías premium dieron GO sin P0 y la suite cerró con 257 pruebas de librería, 38 de binarios/integración, Clippy y build musl verdes | todo ledger de agentes debe validarse como una cadena causal única, no como filas localmente válidas; el puente B10 debe demostrar que una entrada pendiente se reconcilia antes de reintentar el evento retenido por el huésped |
| BUG-ORQ-20260805-627 | abierto | V38/B10, contrato de executor neutral y sesión de agente | la documentación y el contrato presentan un executor/perfil neutral, pero `agente_microvm/src/adaptadores/firecracker/motor.rs` fija `/perfil/bin/agente` como `app-server`; además, `EnviarEntradaSesion` escribe stdin crudo, por lo que un prompt plano no es el protocolo de `app-server` | el adaptador Firecracker mezcla un perfil de ejecución concreto con el contrato neutral de B10 y trata la entrada de sesión como si stdin fuera una interfaz Codex negociada | contraste directo entre contrato/documentación B10 y el launcher Firecracker: ruta y binario hardcodeados; la senda de entrada carece de framing, handshake o protocolo Codex `app-server` | separar el perfil/ejecutor concreto detrás del contrato y definir o adaptar el protocolo de sesión antes de atribuir a B10 ejecución Codex real; no usar prompts planos como evidencia de `app-server` |
| BUG-ORQ-20260805-628 | cerrado en código/offline por `8e1d865d` | V38/B10.3, receipt de lanzamiento | el primer conector podía emitir receipt tras el lanzamiento físico, sin acreditar inicio ni entrega de la sesión de trabajo | se confundió creación de microVM con efecto de trabajo aceptado | `TestAdapterLaunchBuildsPacketBeforeMutationAndDeliversExactInput` y `TestAdapterLaunchReconciliationResolvedAndPendingNeverResend` fijan que el receipt sigue a `WorkPacket` e inicio/entrega de sesión; revisión premium, sin KVM | conservar la frontera: no usar el receipt offline como evidencia de microVM física ni Codex real |
| BUG-ORQ-20260805-629 | cerrado en código/offline por `b770cac6` | executor Codex, cancelación | un FIFO bloqueado podía impedir que `SIGTERM` interrumpiera el executor y dejar la parada cooperativa sin progreso | el `read(2)` del pipe/FIFO no responde de forma portable a la cancelación | `TestRunnerInitialDeadlineInterruptsRealFIFO` y `TestSIGTERMInterruptsRealStdinPipe` cubren deadline y señal sobre I/O real | B11 conserva la parada física exacta; este cierre no la acredita |
| BUG-ORQ-20260805-630 | cerrado en código/offline por `b770cac6` | executor Codex, entorno hijo | el executor podía heredar entorno del anfitrión, incluidos secretos o `HOME` ajenos | el proceso hijo no se construía desde una allowlist sellada | `TestRunnerChildReceivesOnlySealedEnvironment` prueba el entorno mínimo y ausencia de herencia | no convertir el aislamiento de entorno en evidencia de aislamiento físico |
| BUG-ORQ-20260805-631 | cerrado en código/offline por `806b4d78` | B10.3, compilación/request/perfil | compilación, request y perfil podían cruzarse entre sí y producir un encargo causal distinto del autorizado | faltaba un paquete sellado que ligase los tres objetos por identidad y digest | `TestBuildWorkPacketV1RejectsForeignOrMutatedCompilation` y `TestBuildWorkPacketV1SealPreservesExactProfileServiceOrder` fijan el enlace exacto | B10.5 debe recuperar ese `EffectAttempt` tras reinicio del host; hoy no es recuperable |
| BUG-ORQ-20260805-632 | cerrado en código/offline por `3838dbe0`, `2b21911e`, `8e1d865d` y `bd30c246` | B10.3/B10.4, capacidades/observación remotas | el adaptador podía operar contra una contraparte que no anunciase todas las operaciones necesarias o interpretar estado físico como trabajo | la negociación y la observación de sesión estaban incompletas y mezclaban autoridades | `TestAdapterRejectsIncompleteRemoteCapabilitiesBeforeLaunch`, `TestObserveAgentKeepsMissingAndAmbiguousSessionNonterminal` y el bootstrap microVM exigen capacidades completas antes del estado durable y separan observación física/sesión | no acredita UDS/KVM físico; B10.5 conserva recuperación durable |
| BUG-ORQ-20260805-633 | cerrado en código/offline por `8e1d865d` | B10.3, `409` de sesión pendiente | el `409` tipado `sesiones.operacion_pendiente` se clasificaba como rechazo permanente y bloqueaba la reconciliación causal | todo conflicto HTTP se trataba como rechazo sin cotejar el código remoto tipado y exacto | `TestAdapterLaunchMapsOnlyTypedExactRemotePending` y `TestAdapterLaunchSecondReconcileToInputPendingRaceNeverResends` cubren el caso | conservar rechazo para conflictos distintos o sin código; B10.5 debe cubrir la recuperación tras restart |
| BUG-ORQ-20260805-634 | cerrado en código/offline por `8e1d865d` | B10.3, receipt de reconciliación stale | un receipt stale o cruzado podía aceptarse durante la reconciliación de sesión | no se cotejaban todos los bindings causales antes de reconocer el receipt | `TestAdapterLaunchRejectsStaleOrCrossReconciliationReceipt` y `TestAdapterLaunchRejectsAlteredSessionBindings` fijan el rechazo | no reutilizar receipts offline como comprobante físico; B12 conserva esa acreditación |
| BUG-ORQ-20260805-635 | cerrado en código/arnés por `agente_microvm@db23566` | ORC-28/B05, validación ELF del perfil | comprobar solo `PT_INTERP` aceptaba un ELF sin intérprete que aún declaraba `DT_NEEDED` | la verificación de ejecutable no inspeccionaba dependencias dinámicas | el constructor rechaza `PT_INTERP` y `DT_NEEDED`; el arnés reproduce el negativo con `libm` sin intérprete y la revisión premium dio GO | no acredita carga dentro de microVM, KVM ni sesión Codex real |
| BUG-ORQ-20260805-636 | cerrado en código/arnés por `agente_microvm@db23566` | ORC-28/B05, publicación atómica del perfil | `ln` podía seguir un directorio enlazado al publicar `.parcial` | el parcial público no quedaba ligado a un inode privado exclusivo | `.parcial` se crea como inode privado con hard-link exclusivo; el arnés prueba enlace malicioso y conserva intacta la víctima | no acredita desmontaje físico B11/B12 ni KVM |
| BUG-ORQ-20260805-637 | cerrado en código/arnés por `agente_microvm@db23566` | ORC-28/B05, limpieza ante señal | una señal podía dejar un output parcial publicado | el cleanup no demostraba cero parciales por cada caso del arnés | cleanup por señal y comprobación de cero `perfil.*/*.parcial` en cada caso; arnés/revisión premium GO | no demuestra recuperación tras caída del host, KVM ni socket físico |
| BUG-ORQ-20260805-638 | cerrado en código/arnés por `agente_microvm@db23566` | ORC-28/B05, integridad de bytes del perfil | copiar tras validar o volver a hashear el nombre público permitía sustituir los bytes durante la construcción/publicación | validación, imagen y comprobante no quedaban ligados al mismo inode privado | staging privado y movimiento de los mismos inodes; hash solo del inode privado y revalidación no-deref del inode público; el arnés reproduce la sustitución y la revisión premium dio GO | no acredita socket físico, KVM ni sesión Codex real |
| BUG-ORQ-20260805-639 | cerrado antes de commit por `bd30c246` | B10.4, orden de composición de capacidad | adelantar el catálogo para todas las rutas hacía que process o una `AgentFactory` custom pudieran consultarse antes de recibir workspace y runtime scope | el primer wiring no limitó la negociación temprana a la microVM productiva | `TestBuildCustomAgentComposesCapacityAfterHistoricalBindings` exige ambos binders antes del catálogo custom; doble revisión premium y focales `-race` dieron GO | mantener capacidad temprana solo para `AgentFactory == nil && isolation == microvm` |
| BUG-ORQ-20260805-640 | abierto | suite raíz V38/B10.4 y gates transversales históricos | `go test -mod=vendor -count=1 ./...` sobre clean HEAD `563f56eb` falla en múltiples gates preexistentes/no atribuibles a B10.4: arquitectura adapter→protocol/os env, ratchets/digests/receipts stale, dependencia B10 esperada antigua, acceptance históricas, expectativa stale de retained claim en recovery SQLite y script F3-R2 ausente | deuda acumulada de ratchets y gates cruzados que mezcla contratos históricos con el candidato actual; la suite raíz no puede usarse aún como gate atribuible de B10.4 | `internal/bootstrap` sí verde; focales B10.4 normal, `-race` y `vet` verdes. No se actualizaron hashes ni fixtures | separar los fallos por minitareas y asignar cada gate a su contrato antes de volver a usar la suite raíz como gate; no declarar resuelto ni alterar hashes/fixtures por esta incidencia |
| BUG-ORQ-20260805-641 | cerrado antes de commit por `5f6792b0` | B10.5b, selección de intento ambiguo | el primer selector omitía intentos con receipt y podía reconciliar otro intento ambiguo del mismo action/intent, abriendo riesgo de doble lanzamiento | el receipt se trató como candidato descartable en vez de veto global de un efecto ya aplicado | `TestAgentLaunchRecoverySelectorRejectsReceiptsAndAmbiguity` cubre receipt aceptado, receipt stale/cruzado y dos intentos ambiguos; revisión premium dio GO | mantener fail-closed cualquier receipt relacionado; B10.5d debe repetir la guarda dentro del CAS SQLite |
| BUG-ORQ-20260805-642 | cerrado en persistencia/writer por `c322e156` y `13879a05`; E2E pendiente en B10.5 | B10.5d, receipt recuperado y claim de reconciliación | un lanzamiento recuperado pertenece al `EffectAttempt` histórico, pero la confirmación consume un claim nuevo con cerca superior; el esquema exigía una identidad causal exacta para ambas autoridades | `effect_receipts.action_fence` y `action_consumption_receipts.fence` coinciden en el camino normal pero divergen legítimamente tras reinicio | `EffectReceipt.ActionFence` conserva la cerca física, `outbox.recovery_effect_attempt_ref` fija el intento dentro del CAS y `ActionConsumptionReceipt.Fence` conserva la cerca actual; el writer atómico rechaza cruces y repetición sin crear otro intento | mantener estas autoridades separadas al cablear `ReconcileLaunch`; cerrar receipt perdido, reinicio y carrera en el E2E B10.5 |
| BUG-ORQ-20260805-643 | cerrado por `60cdf4d6` | efectos, vencimiento del claim y validación en Go | validadores Go de receipt usaban `After` y admitían `ConfirmedAt == LeaseUntil`, mientras la autoridad SQLite rechazaba correctamente ese borde | la regla temporal semiabierta vivía estricta en los triggers SQL (`confirmed_at < claimed_until`), pero no estaba expresada igual en application ni en las guardas Go del adaptador | `TestEveryPhysicalEffectReceiptRejectsExclusiveAttemptLeaseBoundary`, `TestPhysicalReceiptGoGuardsRejectExclusiveLeaseBoundary` y `TestEffectReceiptLeaseBoundaryIsRejectedBeforeInsert` prueban todos los efectos físicos, launch/stop, rechazo previo al `INSERT` y cero mutación; focales normal/`-race`, `vet` y `diff-check` verdes | conservar `StartedAt <= ConfirmedAt < ClaimLeaseUntil` en application, adaptador y SQL; no relajar el borde ni omitir el lease histórico al separar las cercas en B10.5d |
| BUG-ORQ-20260805-644 | reabierto; recurrencia aislada sin cambios acreditados | delegación B10.5d2, agente SQLite | el mismo hilo reutilizado volvió a responder con instrucciones antiguas para arrancar `codex6`, ajenas a una nueva traza de `ProcessClaim`; la primera recurrencia se había mitigado reemitiendo la minitarea y cerró en `c322e156` | contaminación de contexto o cierre no correlacionado al reutilizar un hilo de agente; el texto final no puede tomarse como evidencia de la tarea actual | ambas respuestas no correlacionadas se descartaron, el segundo encargo era solo lectura y no produjo cambios; se retiró ese hilo del frente y las revisiones delicadas se lanzan en hilos nuevos | exigir que todo ACK/final cite tarea, ficheros y tests actuales; no reutilizar un hilo que ya devolvió contexto contaminado y no acreditar trabajo desde su final |
| BUG-ORQ-20260805-645 | cerrado por `c322e156` | SQLite V27, `effect_receipts_causal_guard` | el trigger vigente todavía comparaba `receipt.confirmed_at` con `outbox.claimed_until`, aunque desde V27 el lease físico histórico pertenece a `effect_attempts.claim_lease_until` | el trigger no se renovó al introducir la autoridad temporal durable del intento; camino normal ocultaba el error porque ambos leases coinciden | la migración 028 liga el receipt al intento y aplica `started_at <= confirmed_at < claim_lease_until`; consumo y claim conservan su tiempo/cerca actuales por separado, sin `ConfirmationFence` | mantener física y confirmación normalizadas; el writer recovery y el reconciler siguen siendo minitareas posteriores, no reabrir la autoridad temporal |
| BUG-ORQ-20260805-646 | cerrado por `c322e156` | SQLite V28, compatibilidad de versiones y gates | focales V28 verdes ocultaron cuatro fallos de paquete: consulta prematura de columna V28 sobre V14, clasificación V15 sustituida por error V28, fixture V27 ejecutado como latest y downgrade schema20 bloqueado por trigger V28 | las pruebas focales nacieron sobre el esquema latest y no ejercieron todos los prefijos/downgrades históricos; además se interrumpió una suite completa antes de su duración normal conocida | consultas read/write versionadas, códigos V15 preservados, fixture V27 real y downgrade schema20 con triggers canónicos; focales verdes, `-race` verde, `vet` verde y paquete SQLite completo verde en 188.647 s | al añadir esquemas, probar latest y prefijos reales; no cortar una suite silenciosa antes de su duración conocida ni inferir verde desde focales |
| BUG-ORQ-20260805-647 | cerrado antes de publicar por `f3c7bcb9` | B10.5d3, selector automático de recovery y regresiones históricas | el primer selector trató todo intento como recuperable: rompió el ratchet `ExcludeLaunch`, bloqueó retries con zero-release exacto, podía devolver recovery sin columna V28 y reclamó después de revocar la autoridad; una corrección intermedia validó también approvals de peers ya resueltos | el pegamento SQLite clasificó la mera presencia de filas antes de reutilizar la autoridad canónica de aplicación y los focales latest no ejercitaron contratos V15 ni revocación separada | preflight puro selecciona el único intento ambiguo, ignora peers definitivamente no aplicados y vuelve a revalidar tras la cerca real; tests públicos cubren receipt+rollback, esquema V27, `ExcludeLaunch`, retry normal, proposer activo con approver distinto revocado, paquete SQLite completo dos veces, `-race`, `vet` y revisión xhigh | todo recovery nuevo debe probar paquete histórico completo, seguridad de identidad y rollback; approval expirada al iniciar el intento es histórica, membership revocada sigue siendo corte fuerte |
| BUG-ORQ-20260805-648 | cerrado antes de publicar por `5c048bf4` | B10.5d4b1, extracción del cierre común de launch | el primer helper compartido podía usar el tiempo histórico del proveedor como `EffectReceipt.ConfirmedAt` también en el camino normal | al compartir cierre normal/recovery no se separaron inicialmente el reloj de lifecycle actual y el tiempo histórico del receipt recuperado | `completeAcceptedLaunch` recibe ambos tiempos de forma explícita; `TestProviderClockSkewCannotDriveLifecycle` ratchea que el launch normal conserva el reloj lógico y recovery el lease histórico | todo helper compartido entre ejecución normal y recovery debe probar por separado las semánticas temporales de ambos caminos |
| BUG-ORQ-20260805-649 | cerrado por `ce195888` y `5c048bf4` | B10.5d4, preservación de entorno tras crash | el launch MicroVM exigía preservación, pero `ExecutionDispatching` no guardaba el byte antes del efecto; tras reinicio recovery reconstruía `false` y firmaba un request divergente | el requisito se añadía al request después de `RecordLaunchPrepared` y el ratchet SQLite solo admitía fijarlo al pasar a `running` | aplicación lo fija antes de preparar; esquema 029/CAS lo persisten en `queued -> dispatching`; tests con reapertura y cambio posterior de capacidades prueban que recovery conserva el valor histórico | toda entrada que participe en firma/idempotencia debe ser durable antes de cruzar el adaptador físico |
| BUG-ORQ-20260805-650 | cerrado antes de publicar por `5c048bf4` | B10.5d4, guardas pre-efecto de recovery | un claim recovery expirado/reclamado, una autoridad revocada, un reconciler ausente o una reserva de capacidad ya transicionada podían alcanzar sesión o proveedor; una primera guarda SQLite tampoco comparaba estado/revisión completos de capacidad | se reutilizó el selector histórico sin una aserción final del outbox vivo y se ordenaron algunas capacidades externas antes de validar todas las autoridades actuales | puerto obligatorio read-only valida token/worker/delivery/fence/lease/disposición/ref, presupuesto único activo, capacity completa y placement; reconciler y autoridad se resuelven antes de `Ensure`; revocación aparca sin settlement; negativos prueban cero `Ensure`/`ReconcileLaunch`/`Launch` y revisión xhigh final da P0=0/P1=0 | mantener la guarda antes de cualquier broker/proveedor; un resultado ya físico no se descarta por revocación posterior y recovery nunca cae a `Launch` |
| BUG-ORQ-20260805-651 | cerrado por `0fb8ad36` y `f768716e` | B10.5d4b2, retry temporal de recovery | el trigger V28 hacía imposible soltar un claim recovery conservando su ref; una primera corrección tampoco podía volver a aparcar la acción ya `unclaimed` si se revocaba autoridad | V28 solo modelaba ref recovery durante un claim vivo y el park reescribía columnas de claim incluso cuando ya estaban nulas | esquema 030 conserva la ref exacta e inmutable en `claimed -> unclaimed`; el park distingue scheduling-only de lease expirado; writer revalida budget/capacity/placement y tests cubren restart, reclaim, revocación, CAS, cruces y texto no tipado | todo retry de un efecto ambiguo repite solo reconciliación con la misma autoridad; nunca borra la ref, crea otro intento ni cae a `Launch` |
| BUG-ORQ-20260805-652 | cerrado operacionalmente; prevención pendiente en wrapper | tooling de pruebas SQLite con `-race` | dos wrappers de revisión reportaron finalización mientras sus binarios `sqlite.test` seguían vivos al 100% de un núcleo | la capa que recogió la salida cerró antes de confirmar el proceso hijo real | el revisor comprobó PID, cwd y parentesco, terminó solo sus dos binarios huérfanos y dejó intacto el focal legítimo del implementador; este terminó con exit 0 en 140.569 s y el censo final quedó limpio | no acreditar ni abandonar una prueba por el estado del wrapper: comprobar siempre salida, exit y desaparición del hijo real antes de cerrar |
| BUG-ORQ-20260805-653 | abierto; PFC-01/PFC-02/PFC-03a/PFC-05a cerrados en código o autoridad durable | primer Codex físico en Firecracker | B10 lógico estaba cerca del cierre, pero no existía evidencia de un agente Codex real en microVM; el E2E histórico 1+16 solo acredita TestAttestor | faltaban activos actuales, egress por proxy preservado, credencial Codex sellada, broker host real, instalación y E2E; la frontera UDS root/no-root ya estaba resuelta sin acreditar todavía el servicio físico | `docs/reconstruccion/auditoria_primer_codex_firecracker_2026-08-05.md` separa B10 offline de la acreditación física; `agente_microvm@989272d` cierra PFC-02; `orquestaV2@5412c70e` cierra los activos reproducibles PFC-01; `ca78a490`, `db1ac680` y `8492a97c` cierran contrato neutral del broker y autoridad durable de egreso | cerrar PFC-03b, PFC-04, PFC-05b/c, PFC-06 y PFC-07 antes de afirmar primer Codex físico; sudo solo para instalar la frontera root y sus permisos auditables |
| BUG-ORQ-20260805-654 | cerrado por `5412c70e` | PFC-01, construcción reproducible de activos Codex | los gates baratos no habían ejecutado el constructor físico completo; el primer gate real descubrió toolchains Go/Rust incorrectos, resolución Cargo del workspace host, ausencia de checksums de directory source y metadatos Rust dependientes del temporal | se había inferido reproducibilidad desde validaciones estáticas sin construir dos veces el grafo exacto y sin modelar la procedencia Cargo como parte del activo | el constructor usa Go 1.25.11 y Rust 1.97.1 exactos, extrae 22 `.crate` verificados, deriva y publica el lock efectivo, remapea rutas, limita el wrapper a las dos crates locales y obtiene dos bundles byte a byte iguales con manifiesto `3bf07f6a…`; revisión independiente P0=0/P1=0 | B12 debe copiar y revalidar cada `.crate` en almacenamiento privado para cerrar la ventana TOCTOU y acreditar cadena de suministro hermética; PFC-01 solo afirma reproducibilidad en el mismo entorno |
| BUG-ORQ-20260805-655 | cerrado por `5412c70e` | PFC-01, cancelación y limpieza del constructor | una cancelación temprana podía dejar el directorio temporal aunque el trap del proceso padre estuviera instalado | `mktemp` se ejecutaba en una sustitución de comando: la señal podía matar el hijo después de crear el directorio y antes de que el padre recibiera su ruta | la ruta candidata se fija antes del `mkdir`, la creación difiere señales, se captura identidad del inode y el arnés cancela en la ventana temprana comprobando cero residuo; el gate físico completo quedó verde y no dejó procesos de build | conservar la prueba de cancelación en todo cambio del constructor y no declarar cierre desde el estado del wrapper sin comprobar proceso hijo y temporales reales |
| BUG-ORQ-20260805-656 | cerrado antes de publicar por `agente_microvm@bd1b088` | PFC-03b, códec de apertura del broker host | el primer códec Go aceptaba aliases de claves con casing distinto y podía transformar UTF-8 inválido o surrogates aislados que Serde rechaza; además su JSON no coincidía byte a byte con Rust para `<>&` y U+2028/U+2029 | `encoding/json` compara nombres de campo sin distinguir mayúsculas y aplica reglas de escape/normalización distintas de `serde_json`; el fixture inicial solo usaba ASCII | whitelist exacta de trece claves, detección de duplicados tras desescape, validación UTF-8/surrogates y encoder compatible; revisión independiente P0=0/P1=0/P2=0, tests Go normal/race/vet y fixtures cruzadas verdes | conservar los casos de alias, duplicado, UTF-8 inválido, surrogate alto/bajo aislado, par válido, separadores Unicode y literal escapado; no aceptar un fixture ASCII como prueba completa de interoperabilidad |
| BUG-ORQ-20260805-657 | cerrado por `fe7eefb2` y `a74bfc26` | PFC-05, transporte durable de autoridad de egreso | el primer transporte exigía UTF-8 aunque SQLite conservaba bytes opacos; después, el caso sin política reconstruía un slice vacío no nulo y rompía una comparación byte-a-byte tras restart | se estrechó el contrato histórico al añadir el puerto y se proyectó `[]byte("")` sin canonizar el cero | `AgentLaunchEgressAuthority` conserva `[]byte` exactos con digest y comparación explícita; la proyección devuelve el valor cero canónico cuando no hay política; focal de restart, puertos/aplicación, race, vet y gate amplio `./internal/... ./cmd/orquesta` verdes | mantener payload opaco en aplicación y parsearlo solo en el adaptador físico; probar siempre `nil`/vacío y bytes no UTF-8 en replay |
| BUG-ORQ-20260805-658 | cerrado antes de publicar por `agente_microvm@17175ae` y `orquestaV2@b6f0fafd` | PFC-05, JSON estricto y activación del proxy | el primer candidato duplicó parser/whitelist del sibling y `encoding/json` aceptaba `CONTROLLED_EGRESS_PROXY`, activando egreso con una clave de casing no exacto | dos autoridades JSON podían divergir y la decodificación de structs Go compara tags sin distinguir casing | un único helper sibling posee UTF-8, surrogates, forma, duplicados y trailing; schemas anti-drift; WorkPacket exige nombres exactos y prueba casing; doble revisión terminó GO P0/P1/P2=0 | el asset huésped y el pin deben reconstruirse juntos cuando el paquete con egreso añada campos; no relajar `DisallowUnknownFields` ni la allowlist exacta |
| BUG-ORQ-20260805-659 | cerrado offline por `agente_microvm@2acd300`; evidencia física pendiente | instalador root de Agente MicroVM | el candidato usaba alias/versionado ilegal de slice y ruta cgroup imposible, podía dejar servicio anterior y nuevo activos durante upgrade y confiaba en `TMPDIR` bajo root; rollback/status también perdían estado o activos críticos | el fake de systemd simplificó en exceso la semántica real y el diseño content-addressed se aplicó a una unidad slice que no admite aliases | slice estable y cgroup `/agente.slice/agente-microvm.slice`; writer único; temporal root-trusted; receipts con hashes/estados; marker por identidad renderizada; negativos de controladores, comandos, residuos y concurrencia; dos rondas de revisión independientes y arnés verde | PFC-07b debe acreditar systemd/cgroup/UDS reales antes de PFC-06; `launch_ready=false` impide confundir este cierre offline con instalación física |
| BUG-ORQ-20260805-660 | cerrado en código por `agente_microvm@2f00b3d`; prueba física pendiente | PFC-04, credencial Codex dentro del huésped | el primer C2 superó 30 tests, Clippy y build musl, pero podía bloquear PID 1 durante `connect`, emitir un ACK de purga desde `Drop`, ignorar una purga fallida y permitir que la ruta legacy `Ejecutar` leyera `auth.json` con el mismo UID 1000; una segunda revisión detectó además que ACK inicial o `spawn` fallidos podían dejar la microVM reutilizable | los seams cubrían protocolo y proyección lógica, pero no ownership completo del canal/montaje, estados ambiguos ni exclusión entre las dos superficies de ejecución del huésped | conexión no bloqueante con deadline, `Drop` sin ACK, purga explícita, EOF/ACK/spawn ambiguos fatales, ratchet legacy y tests de doble intento; revisión independiente final GO P0/P1/P2=0, 40/40 pruebas huésped, Clippy estricto y build MUSL release verdes | PFC-06 debe acreditar tmpfs, modos, proceso Codex y purga dentro de KVM real; el cierre offline no sustituye esa evidencia |
| BUG-ORQ-20260805-661 | cerrado antes de publicar por `agente_microvm@6d08deb` y pin `orquestaV2@cc7cec3e` | PFC-04, códec Go del protocolo de credencial | el primer C1b pasaba pruebas normales, race y vet, pero no fijaba por reflexión todos los DTO ni ejercía lecturas/escrituras fragmentadas | los tests iniciales validaban vectores completos sin ratchear deriva estructural ni short I/O real | anti-drift reflectivo de los tres contratos, reader fragmentado, writer fragmentado y compatibilidad Rust/Go; rerevisión independiente GO P0/P1/P2=0 | conservar un solo códec público vendorizado; no duplicar parser ni relajar JSON estricto al implementar el broker |
| BUG-ORQ-20260805-662 | cerrado por `5d00ede4` y `b96b3a2e` | PFC-04, consumo físico one-shot de credencial | `credentials.Store.Use` reproduce una petición idempotente ejecutando otra vez el callback, por lo que usarlo directamente desde el broker permitiría una segunda entrega del mismo secreto | el store actual modela uso autorizado repetible, no consumo físico de un solo uso; el protocolo nuevo necesita semántica más estrecha y durable | contrato separado `OneShotStore`; persistencia local acredita el claim antes del callback, replay exacto sin callback, error terminal también consumido, reapertura, carrera, rotación/revocación y failpoints; revisión independiente P0=0/P1=0 | conservar `Use` legacy sin alterar y usar exclusivamente `UseOnce` en la entrega física al huésped |
| BUG-ORQ-20260805-663 | cerrado antes de publicar por `4f48195c` | PFC-04, clave durable de autoridad física | el primer contrato de autoridad aceptaba `RunRef` con saltos de línea o tamaño sin límite, por lo que una clave opaca no era segura para persistencia, auditoría o framing | se reutilizó la validación nominal de `ExecutionRef` sin aplicar el límite y la forma estricta exigidos en la frontera física | revisión independiente detectó ambos negativos; la validación final limita a 512 bytes, rechaza NUL/CR/LF/espacios laterales y mantiene compatibilidad con refs históricas canónicas | toda ref que cruza persistencia o protocolo físico necesita forma y límite propios además de su constructor nominal |
| BUG-ORQ-20260805-664 | cerrado antes de publicar por `agente_microvm@928ef30` y pin `orquestaV2@5394e3bf` | PFC-04, extracción de servicios firmados | el primer helper aceptaba un plan sin `control_broker`, incluido proxy-only, y dos servicios con el mismo `servicio_ref` | el contrato general del plan tolera cero servicios para otros usos, pero la autoridad host del primer Codex exige exactamente un broker de control y refs no ambiguas | helper estrecho exige control primero, proxy opcional, roles/puertos/refs canónicos y sin duplicados; fixtures Go/Rust y revisión independiente quedaron verdes | no endurecer el plan general para resolver una precondición del adaptador host; derivar una vista estrecha validada |
| BUG-ORQ-20260805-665 | cerrado; wiring residente pendiente | PFC-04, versión exacta de credencial en autoridad de lanzamiento | la autoridad física inicial no fijaba la versión ni la tupla completa de credencial; describirla después de lanzar permitía cruzarla con una rotación o cuenta distinta | selección, consulta de versión y ejecución física no compartían una única identidad causal durable | `b25b1f58`, `7f04863d` y `02b864a3` añaden reader sin material, descripción autorizada y `OneShotClaim` completo ligado a run/fence/attempt/session; `5a8d4bb6` selecciona por placement exacto y `f02b811e`/`f6c07202` lo persisten antes del efecto | recovery y broker deben reutilizar esa versión durable y nunca volver a describirla |
| BUG-ORQ-20260805-666 | abierto; workaround acotado | tooling Go, resolución de módulos | `go get` dentro de OrquestaV2 falla porque `go.mod` conserva un `replace` hacia `./modulos/orquesta-estado-vivo/testdeps/rapid`, ruta inexistente en el árbol limpio | la limpieza/separación de legacy dejó una sustitución local sin su módulo objetivo | para fijar `agente_microvm@928ef30` se descargó el módulo desde un temporal externo con rewrite SSH y se sincronizó vendor/checksum; los gates `-mod=vendor` pasan | decidir si restaurar el módulo de test aprobado o retirar el `replace` con evidencia de todos sus consumidores; no recrear legacy a ciegas |
| BUG-ORQ-20260805-667 | cerrado antes de publicar por `5a8d4bb6` | PFC-04, resolver de claim por colocación | el primer resolver podía acuñar un `OneShotClaim` desde una petición con receipt, aprobación y tiempos de efecto vacíos porque validaba solo run/fence/attempt/session | el helper causal estrecho se confundió con acreditación de toda la autoridad durable de lanzamiento y el fixture también estaba incompleto | revisión independiente dio NO-GO; `Resolve` valida ahora `AgentLaunchRequest` y `AgentLaunchEffectAuthority` antes del reader, rechaza contexto typed-nil y el ratchet material-free recorre tipos anidados; focal, race y vet verdes y rerevisión GO | ninguna derivación de credenciales puede completar autoridad ausente; los fixtures deben partir de una petición de lanzamiento productivamente válida |
| BUG-ORQ-20260805-668 | cerrado antes de publicar por `130961aa` | PFC-04, constructor de autoridad física | una concesión firmada podía cruzarse con otro attempt/session del mismo run+fence, y un claim válido de otra colocación/cuenta podía entrar porque la identidad one-shot no incluía placement | el primer constructor comparaba solo los campos publicados por el extractor firmado y recibía un claim desnudo, sin conservar la prueba del resolver que seleccionó la cuenta | `ResolvedCredentialClaim` sella placement con campos privados; el constructor exige compilación/contexto/plan firmados exactos y cruces reales de attempt, session, placement, credential y versión; rerevisión independiente GO | conservar la evidencia de selección hasta la frontera de persistencia; no degradarla a una tupla válida pero descontextualizada |
| BUG-ORQ-20260805-669 | cerrado antes de publicar por `f02b811e` | PFC-04, migración SQLite v32 de autoridad física | el primer esquema admitía textos con NUL oculto, no acreditaba `agent_launch`/estado prelaunch/no-receipt y permitía insertar una autoridad ya bound | se confió en FK y GLOB como sustitutos de la validación semántica; SQLite puede cortar comparaciones ante NUL y la FK solo liga identidad parent, no lifecycle | checks NUL/forma/límites, trigger causal exacto, `external_ref IS NULL` obligatorio al INSERT, fixture derivado con el puerto canónico y round-trip prepared/bound; doble NO-GO independiente corregido antes del commit | una tabla inmutable debe impedir la fila inválida al nacer; la validación del registry no sustituye la defensa del esquema |
| BUG-ORQ-20260805-670 | cerrado antes de publicar por `f6c07202` | PFC-04, registry SQLite de autoridad física | el primer registry podía preparar después de existir un effect receipt y enviar uint64 no representable por INTEGER SQLite | la consulta de precondiciones omitía el hecho terminal del efecto y el contrato neutral no impone el límite concreto del adaptador SQL | `Prepare` exige ausencia de receipt y actor/proyecto/intento exactos; registry rechaza fence/version mayores que MaxInt64 antes de tocar DB; focal race 252,617 s, concurrencia de binds, replay/reopen y revisión independiente verdes | declarar y probar en el adaptador cualquier estrechamiento de representación; prepare-before-effect debe cortar también estados parciales/corruptos |
| BUG-ORQ-20260806-671 | cerrado antes de publicar por `01f22436` | PFC-04, sesión durable de autoridad física | una fila V32 podía conservar una `session_ref` vacía o cruzada, migrar a V33 con receipt verde y quedar resoluble tras restart | el trigger V32 no ligaba la sesión de la autoridad a `executions.execution_session_ref` y el validador de recovery era opcional | la migración 033 ejecuta un preflight SQL dentro de la transacción antes de tocar trigger, receipt o versión; una sesión inválida aborta y conserva V32 byte-equivalente, mientras `Prepare` y el trigger nuevo exigen sesión exacta no vacía; focales, vet y doble revisión independientes verdes | conservar el checksum V32 y acreditar rollback atómico de cualquier migración que endurezca hechos históricos |
| BUG-ORQ-20260806-672 | cerrado antes de publicar por `9c64e12b` | PFC-04, replay del adaptador de lanzamiento microVM | la reconciliación podía volver a resolver la credencial actual y confundir un `Prepare` fallido con efecto definitivamente no aplicado | el camino de replay reutilizaba selección mutable en vez de la autoridad histórica y clasificaba demasiado pronto el fallo durable anterior al proveedor | `Reconcile` resuelve primero la autoridad persistida, reconstruye la concesión exacta con su versión histórica y repite `Prepare` idempotente; un fallo de preparación ya no se declara definitivamente no aplicado; replay prepared/bound, race, vet y revisión independiente verdes | todo recovery debe partir de autoridad durable y conservar ambigüedad hasta reconciliar el efecto físico |
| BUG-ORQ-20260806-673 | cerrado antes de publicar por `9c64e12b` | PFC-04, composición productiva microVM | el adaptador endurecido exigía resolver claims y registrar autoridad, pero el bootstrap productivo no inyectaba esas dependencias ni separaba credencial Codex de credencial firmante | el contrato del adaptador avanzó antes que su raíz de composición y la configuración no expresaba la identidad de cuenta física | el bootstrap abre SQLite antes del cliente, inyecta resolver por colocación y registry durable, exige `runtime.codex.credential_ref` solo en microVM y la distingue de la credencial de firma; process/custom siguen verdes; normal, race, vet, diff-check y revisión independiente verdes | ningún puerto obligatorio se considera cerrado hasta estar compuesto desde configuración canónica y cubierto por las rutas alternativas |
| BUG-ORQ-20260806-674 | cerrado antes de publicar por `911357d9` | PFC-04, autenticación del broker de credenciales | el diseño inicial pretendía verificar el hash de `/proc/<pid-root>/exe` desde Orquesta no privilegiada, pero el kernel niega esa lectura incluso para procesos root legítimos | se intentó imponer autenticación simétrica sobre una frontera deliberadamente asimétrica: el daemon root sí puede verificar a Orquesta, pero Orquesta no puede inspeccionar de forma fiable el ejecutable root | el listener privado exige `SO_PEERCRED` con UID exacto y PID positivo; el daemon root conserva la verificación inversa de UID y hash del binario Orquesta antes de abrir el servicio | no reintroducir una comprobación imposible de `/proc`; la defensa completa combina socket privado, peer UID del kernel y autenticación inversa por el daemon privilegiado |
| BUG-ORQ-20260806-675 | cerrado antes de publicar por `911357d9` | PFC-04, publicación segura del UDS de credenciales | entre `bind` y la captura de identidad, una sustitución concurrente del pathname podía convertirse erróneamente en autoridad de limpieza y ser borrada al cerrar | identidad del pathname y descriptor escuchando se comparaban sin demostrar que el pathname resolvía al listener exacto | autoprobe por el pathname, `Accept4` sobre el FD exacto, `SO_PEERCRED` del propio proceso y recaptura estable de dispositivo/inodo antes de promover la identidad; dos ventanas de sustitución tienen tests deterministas | nunca hacer `unlink` antes de probar la asociación pathname-listener; ante fallo previo, dejar el residuo a una limpieza gobernada y no adivinar ownership |
| BUG-ORQ-20260806-676 | cerrado antes de publicar por `911357d9` | PFC-04, portabilidad del listener de credenciales | la primera implementación declaraba config y API solo bajo `linux`, por lo que el paquete dejaba de compilar en otros sistemas | el contrato común quedó mezclado con `SO_PEERCRED` y la publicación UDS específica de Linux | config, códigos y API residen en fuente común; el stub `!linux` mantiene la misma superficie y falla cerrado con `server_unavailable`; crosscompile Darwin/Windows verde | los adaptadores físicos no disponibles deben conservar contrato compilable y fallar cerrado, nunca desaparecer por build tags |
| BUG-ORQ-20260806-677 | cerrado antes de publicar por `4db7e8b6` | PFC-04, composición residente del broker de credenciales | el handler y el listener one-shot existían, pero el bootstrap no los construía ni poseía su lifecycle; un agente microVM productivo no podía entregar la credencial al huésped | la autoridad durable avanzó por cortes estrechos y faltaba completar la raíz de composición sin transferir ownership del store compartido | bootstrap inyecta por separado reader, `OneShotStore` y registry SQLite; publica el listener antes de devolver el agente y cierra broker, cliente, store y repositorio en ese orden; rollback, socket `0600`, ruta ajena, race, vet, suite y revisión xhigh P0/P1=0 verdes | cada recurso residente necesita un owner y un orden de cierre explícito; el wrapper nunca debe cerrar el store compartido del Runtime |
| BUG-ORQ-20260806-678 | abierto, P2 gobernado | PFC-04, salud y recuperación del listener de credenciales | una caída inesperada del accept loop solo aflora al cerrar y un socket stale tras crash hace fallar cerrado el siguiente arranque | el primer corte prioriza ownership seguro: no adivina que una ruta es huérfana ni dispone todavía de puerto de health para proyectar la muerte del listener | `911357d9` conserva pathname ajeno y `4db7e8b6` drena exactamente al cerrar; no existe falso fallback ni borrado ciego | acreditar el comportamiento en PFC-06 y añadir después health/recovery gobernado con prueba de PID/owner/inodo; no bloquear el primer smoke ni limpiar a ciegas |
| BUG-ORQ-20260806-679 | abierto; bloquea el Codex KVM con red | PFC-06, composición de egreso controlado | el resolver compuesto ya carga/compone la política sellada, pero el Goal real de smoke no solicita `egress_policy_ref`; sin esa ref el compilador omite `controlled_egress_proxy` y Codex no puede alcanzar OpenAI desde una microVM sin NIC | los contratos PFC-05 y el resolver `59113b5d`+`2c3ebe88` avanzaron antes que la petición causal del Goal/smoke | resolver compuesto publicado; no hay verde físico ni solicitud de ref acreditada | hacer que el Goal/smoke pida una ref explícita; mantener la microVM sin NAT/NIC y exponer solo el proxy loopback firmado |
| BUG-ORQ-20260806-680 | abierto; tooling operativo acotado | PFC-04/PFC-06/PFC-07, provisionado y publicación física | `f9b86e00` cierra provisionado puro/reanudable de firma Ed25519+auth Codex con `ProvisionStore`, callbacks fail-closed, cancelación y reapertura local de `Create` perdido; `b67815c` construye el descriptor y `7d3644a` proyecta JSON CLI canónico desde config efectiva+digest causal. Aún no hay publicación root-owned, receipt/installer ni CLI/composición operativa completa | PFC-01 aporta hashes de kernel/initramfs/perfil, pero no vCPU, memoria ni los cinco hechos de cada servicio; generar desde Orquesta o flags manuales fabricaría autoridad que no posee | cierres offline publicados; no hay servicio host, digest real de binario, KVM ni Codex físico acreditados | publicar desde el sibling con ownership root, receipt e installer; cerrar CLI/composición sin incluir material secreto ni inferir hechos ausentes |
| BUG-ORQ-20260806-681 | cerrado por `agente_microvm@a8d1e76` | PFC-06/PFC-07, perfil físico | un falso verde validaba el perfil físico sin que llegara al motor que arranca la microVM | config, motor e instalador no compartían el mismo perfil efectivo | `a8d1e76` conecta el perfil físico en config/motor/instalador | conservar la ruta única; no equivale a arranque KVM ni cierre físico |
| BUG-ORQ-20260806-682 | cerrado antes de commit por `2c3ebe88` | PFC-05, wiring de egreso | el resolver de egreso compuesto tenía colisiones de wiring detectadas antes de publicar | la composición local de fuentes de política no estaba cerrada como una única ruta | `2c3ebe88` cierra la composición; sigue pendiente que Goal/smoke solicite la ref | no reabre BUG-679 ni acredita egreso físico |
| BUG-ORQ-20260806-683 | cerrado por `agente_microvm@b67815c` | PFC-06, fixture de descriptor/UDS | un fixture AF_UNIX superaba el límite de 108 bytes | la ruta de fixture no respetaba el límite de pathname Unix | `b67815c` acota el fixture; constructor puro de descriptor disponible | conservar rutas acotadas; no acredita publicación, receipt, CLI ni ejecución física |
| BUG-ORQ-20260806-684 | abierto; P2 no bloqueante | PFC-06, descriptor puro | queda deuda de prefijo duplicado y de prueba de egreso/digest del descriptor | el constructor puro aún no tiene publicación/receipt/CLI y esos detalles no alteran el alcance de V23 | deuda registrada tras `agente_microvm@b67815c`; sin verde físico | resolver como calidad del descriptor sin inflar V23 ni bloquear el primer smoke físico |
| BUG-ORQ-20260806-685 | cerrado antes de `f9b86e00` | PFC-04, callbacks de provisionado | un callback podía hacer panic y dejar una provisión en estado ambiguo | el borde de callback no convertía el panic en fallo causal | `f9b86e00` recupera y falla cerrado; el store conserva resultado reanudable | mantener todo callback fuera de la transición durable y fail-closed |
| BUG-ORQ-20260806-686 | cerrado antes de `f9b86e00` | PFC-04, reentrancia de callbacks | un callback podía reentrar durante la provisión y duplicar o cruzar su efecto | callback y transición compartían una ventana sin aislamiento | `f9b86e00` acota la reentrancia y conserva una única autoridad durable | no ejecutar efectos reentrantes dentro de una transición de store |
| BUG-ORQ-20260806-687 | cerrado antes de `f9b86e00` | PFC-04, orden pre-efecto | un efecto podía arrancar antes de validar toda la provisión | la validación final estaba después de la frontera de efecto | `f9b86e00` valida antes de cualquier efecto y falla cerrado | conservar la validación completa antes del efecto |
| BUG-ORQ-20260806-688 | cerrado antes de `f9b86e00` | PFC-04, autoridad `ProvisionStore`/reader | store y reader podían aportar autoridades partidas para la misma provisión | la lectura de estado no quedaba ligada al escritor durable | `f9b86e00` liga `ProvisionStore` y reader a la misma autoridad causal | no recomponer provisiones desde fuentes independientes |
| BUG-ORQ-20260806-689 | cerrado antes de `f9b86e00` | PFC-04, cancelación terminal | cancelar al final podía dejar un terminal contradictorio o reanudable como éxito | la carrera cancelación/cierre no tenía precedencia terminal explícita | `f9b86e00` hace terminal la cancelación y conserva reapertura segura | conservar cancelación como frontera terminal causal |
| BUG-ORQ-20260806-690 | cerrado antes de `f9b86e00` | PFC-04, metadata de `Create` | `Create` podía persistir metadata no inicial o incompleta | el alta no fijaba todos los hechos iniciales antes del store | `f9b86e00` inicializa la metadata y el adaptador local reabre una respuesta `Create` perdida | fijar metadata inicial antes de publicar/reintentar |
| BUG-ORQ-20260806-691 | cerrado por `agente_microvm@7d3644a` | PFC-06, proyección CLI JSON | un flush podía informar éxito sin haber confirmado la salida CLI | la proyección no comprobaba el fallo de flush | `7d3644a` propaga el error y prueba el falso éxito | no declarar receipt/salida desde un flush no confirmado |
| BUG-ORQ-20260806-692 | cerrado por `agente_microvm@7d3644a` | PFC-06, ayuda y golden CLI | ayuda y JSON canónico carecían de golden que fijara contrato visible | la superficie CLI no tenía ratchet de presentación | `7d3644a` añade ayuda/golden para config efectiva+digest causal | mantener los goldens al cambiar el contrato CLI |
| BUG-ORQ-20260806-693 | cerrado antes de publicar; auditoría independiente GO | arquitectura reconstruida | el gate completo `TestRebuildArchitecture` fallaba por imports adapter -> `internal/adapters/protocol/codexwork` en `agentmicrovm/codexwork` y por `os.Environ` en `runner_linux_test.go` | el protocolo puro compartido por adaptadores hermanos vivía dentro de `internal/adapters`; el helper Linux observaba el entorno sellado mediante una API global prohibida; la incidencia no se atribuye a PFC-04/PFC-06/PFC-07 | protocolo completo movido sin cambiar package/wire/goldens a `internal/agentprotocol/codexwork`; imports y builder actualizados; allowlist exacta con mutantes negativos y guarda stdlib-only robusta basada en `go/build`; helper observa `/proc/self/environ`; verdes protocolo/consumidores normal y `-race`, compilación `./...`, `TestRebuildArchitecture`, `go vet`, `bash -n`, `git diff --check` y auditoría independiente P0/P1/P2=0 | no ampliar la excepción exacta, no reabrir Firecracker/config y no convertir este cierre en acreditación física |
| BUG-ORQ-20260806-694 | cerrado antes de publicar; auditoría independiente GO | PFC-06, confianza del descriptor físico | el loader productivo exigía que el descriptor perteneciera al EUID de Orquesta, pero el instalador privilegiado debe publicarlo como `root`; un despliegue correcto quedaría rechazado antes de crear el agente | la frontera de lectura mezclaba owner del consumidor con publisher confiable y solo contemplaba el fixture de desarrollo | el owner real queda limitado a `root` o al EUID consumidor; ruta canónica, digest esperado, regular, `nlink=1`, tamaño, lectura estable, ausencia de symlink/setid/sticky y no escritura por grupo/otros siguen fail-closed; focal normal y `-race`, arquitectura, vet y auditoría independiente P0/P1/P2=0 verdes | no aceptar UIDs terceros, autoridad por grupo ni convertir este cierre en publicación root o evidencia KVM |
| BUG-ORQ-20260806-695 | cerrado antes de publicar por `a3c7df6b`; auditoría independiente GO | PFC-04, salida operativa del provisionado | la CLI devolvía fallo si `stdout` rechazaba el JSON, pero ningún test demostraba que no declarase éxito ni filtrase material después de hacer durables las credenciales | el primer corte probó creación, reanudación y parciales, pero omitió el fallo normal del writer; repetía el riesgo histórico de BUG691 | tabla success/partial fuerza `(0,error)`, exige cierre único del store antes de escribir, exit 1, `cli.output_invalid` y ausencia de auth, rutas y codificaciones; el `panic` de `Close` también queda recuperado; focal normal y `-race`, formato y rerevisión P0/P1/P2=0 verdes | toda salida que actúe como comprobante debe acreditar el fallo normal del writer; el retry conserva la autoridad durable y no vuelve a leer auth ni generar la firma |
| BUG-ORQ-20260806-696 | cerrado en la misma sesión; primer análisis descartado | coordinación de agentes y separación V2/legacy | un subagente encargado de acotar BUG679 en `/home/alberto/Trabajo/orquestaV2` devolvió primero símbolos y rutas de `/home/alberto/Trabajo/orquesta`, mezclando la aplicación limpia con legacy | la tarea declaraba el repo, pero no exigía evidencia inicial del directorio efectivo y el agente heredó contexto histórico amplio | el resultado no se usó; la repetición exigió `pwd`, top-level, branch y presencia de `internal`, y devolvió la ruta causal real V2 desde commands hasta autoridad durable y compilación microVM | toda minitarea entre repos hermanos debe comenzar acreditando top-level/branch y debe rechazarse si cita superficies ausentes del árbol asignado |
| BUG-ORQ-20260806-697 | cerrado por `5f77747c` | Git, preparación de commits pequeños | la regla inicial `orquesta` de `.gitignore` coincidía también con el directorio versionado `cmd/orquesta`; `git add` preparó cambios tracked pero terminó con error por ruta ignorada, dejando una operación parcialmente exitosa y confusa | el ignore destinado al binario raíz no estaba anclado | la regla queda como `/orquesta`; `git check-ignore --no-index` confirma que solo el artefacto raíz coincide y que `cmd/orquesta/main.go` deja de estar ignorado | anclar ignores de artefactos a su ubicación exacta y verificar el índice tras cualquier `git add` que devuelva error |
| BUG-ORQ-20260806-698 | cerrado por `c5b9e681` | PFC-04/PFC-06, bootstrap de credenciales microVM | el comando que debía provisionar la credencial exigía resolver antes el descriptor físico y su digest, pero ese descriptor solo podía publicarse después del propio provisionado; el primer arranque quedaba en un círculo sin orden válido | el resolver runtime completo se reutilizó en una operación de bootstrap que todavía no puede poseer dos hechos futuros | `ResolveForCredentialProvisioning` conserva todas las validaciones canónicas y solo admite que ruta y digest del descriptor estén ambos ausentes; `Resolve` sigue estricto y un preflight inválido no abre el store, no lee `auth.json` ni genera clave; focales normal/`-race`, `vet` y revisión independiente quedaron verdes | ningún perfil de bootstrap puede relajar hechos que ya existan o admitir solo uno de la pareja ruta/digest; el servidor productivo siempre usa `Resolve` |
| BUG-ORQ-20260806-699 | abierto; workflow y Store SQLite Q-P-C listos, adaptador físico y caller productivo pendientes | B11/B12, lifecycle del agente microVM y cierre de Goal | la ruta B10 compone de forma offline el encargo Codex y la sesión sobre el adaptador microVM, pero aún no acredita un Codex dentro de KVM; cuando exista una observación terminal física con preservación requerida, cerrar directamente la ejecución como éxito fallaría porque todavía no habría receipt causal de preservación | lanzamiento/sesión y la cadena física de éxito `quiesce -> preserve -> close` avanzaron en cortes separados; `RegistrarPreservacionEntornoAgente` existe, pero no tiene caller productivo, y el wrapper microVM no aporta un lifecycle durable compuesto | la auditoría causal de `ProcessNext/Claim` confirma B10 application+SQLite sin KVM; la auditoría PFC mantiene explícitamente pendiente el primer Codex físico. `2d95fe49` liga cierre y receipt al `ApplicationReceiptRef`; el servicio de aplicación gobierna Q-P-C y el candidato de migración 035 implementa su Store SQLite con CAS, restart, replay, lease expirada, re-claim y preservación `workspace_absent`. Focal normal, `-race`, paquete SQLite completo y `vet` verdes; todavía no existe wiring bootstrap ni smoke físico | componer lifecycle/reconciliador solo cuando el protocolo hermano publique `Inspect/Quiesce/Preserve/Close` reales; después acreditar PFC-06/PFC-07b con Codex real en KVM y solo entonces cerrar Execution, WorkItem y Goal con reinicio y cero residuos físicos |
| BUG-ORQ-20260806-700 | abierto; contrato, modelo, señal, esquema y lectura durable sibling publicados; escrituras productivas pendientes | Agente MicroVM/B11, detención física y reconciliación | la API sibling exponía una única detención sin modo y devolvía solo la ejecución actualizada; Orquesta no podía distinguir cooperativa de forzada ni obtener `receipt_ref`, instante físico confirmado y replay causal exacto | el motor ya producía un comprobante de retirada de recursos, pero el servicio y el ledger lo descartaban; la idempotencia conservaba resultado lógico, no el recibo físico | `agente_microvm@daf8469` fija el contrato público; `agente_microvm@db1b167` modela el ledger causal; `agente_microvm@767f9e8` añade señal `TERM`/`KILL` cercada sin esperar, escalar ni retirar recursos; `agente_microvm@63d86c9` añade el esquema SQLite `STRICT` v17; `agente_microvm@078b794` publica enumeración paginada y `agente_microvm@8b890e7` recupera pendientes, receipts y sustituciones por scope exacto, con guardia v17 y vínculos bilaterales. Revisión premium P0/P1=0 y gate completo verde con 361 pruebas, dos KVM ignoradas, Clippy estricto y build musl; todavía faltan reserva/marca/confirmación transaccionales, servicio/recovery, API/clientes y replay físico | no anunciar capacidad B11 hasta demostrar que cooperativa no escala por sí sola, forzada sí puede hacerlo, respuesta perdida reconcilia el mismo recibo y restart no fabrica timestamps ni autoridad |
| BUG-ORQ-20260806-701 | desviación detenida sin cambios publicados | coordinación de agentes y aceptación B12.4 | un primer intento de smoke añadió más de 400 líneas de test y pretendía forzar microVM dentro del E2E histórico del proveedor `process`, lo que habría eliminado cobertura válida y mezclado dos contratos | la tarea de aceptación era demasiado amplia y buscó reutilizar una fixture histórica en vez de abrir un escenario físico nuevo y aislado | el cambio se redujo, se revisó y finalmente se revirtió completo antes de commit; el worktree volvió a limpio y B12.4 quedó separado como aceptación física propia | las aceptaciones físicas nuevas deben declarar proveedor, assets, credencial y residuos en una fixture dedicada; no modificar una prueba histórica de otro adaptador para obtener un verde aparente |
| BUG-ORQ-20260806-702 | cerrado en código por `a6804d0d` y esta entrega | V38/B12, persistencia del manifiesto físico preservado | el comprobante de preservación podía incorporar la ref y el digest del manifiesto físico, pero el writer SQLite no tenía columnas para ambos y habría informado éxito durable perdiendo el enlace al reiniciar | contrato y persistencia avanzaron en cortes separados; aceptar el nuevo par antes de la migración 034 creaba un falso verde de cierre físico | el par sigue opcional-atómico por compatibilidad; la migración 034 añade ambas columnas con checks, el writer y reader conservan el par exacto, recovery valida V25/V34 y los tests cubren upgrade V33, reinicio, replay e inputs adversarios | no confundir persistencia del manifiesto con composición del lifecycle ni con evidencia física B12 |
| BUG-ORQ-20260806-703 | abierto; servicio, puerto y Store SQLite completos, composición física pendiente | V38/B11-B12, reconciliación lifecycle tras resultado físico ambiguo | el primer modelo pretendía guardar tokens y resoluciones `pending` declaradas por el propio snapshot, aunque no existía evidencia independiente que autorizase esa resolución; además podía consumir una revisión antes de conocer el resultado físico | se modeló la ambigüedad como nuevo estado durable en vez de conservar el mismo intento físico y reconciliarlo mediante un puerto de solo lectura | el modelo conserva el snapshot `attempted`, prohíbe un segundo efecto y reconstruye el request original; el servicio relee ganadores CAS, tras pending/error solo hace `Inspect` y reconciliación histórica, y almacena recibo, consumo y siguiente acción en el CAS terminal. La implementación SQLite candidata añade migración 035, read-model, CAS inicial/pre-efecto/terminal, recuperación exacta tras restart y lease expirada y pruebas de re-claim sin segundo intento; focal normal, `-race`, paquete SQLite completo y `vet` verdes | componer lifecycle/reconciliador con operaciones físicas reales del protocolo hermano y acreditar restart/smoke físico; el Store aislado no autoriza stubs ni una segunda autoridad de lifecycle |
| BUG-ORQ-20260811-704 | cerrado en harness en esta entrega | V14/Codex, stop cooperativo bajo presión de procesos | `TestCodexSelectiveStopAdoptsAfterCrashAndRejectsReusedPID` exigía retorno en menos de un segundo, pero la inspección real única de `/proc` tardó 1,251–1,259 s en el host cargado y produjo falsos rojos repetibles aunque devolvía `pending`, el árbol seguía vivo y no esperaba settlement | el test confundía la latencia variable de una inspección exacta de identidad con espera del proveedor; la semántica ya prohíbe la segunda inspección y el settlement en modo cooperativo | el presupuesto del arnés queda en 2 s, todavía muy inferior al proceso de 30 s y al timeout configurado de 10 s; conserva árbol vivo, señal única, replay, escalado forzado y desaparición exacta. El focal se repite antes del gate completo | no ampliar el timeout de producto ni retirar la inspección exacta; si supera 2 s, perfilar `/proc` y carga del host antes de relajar otra vez |
| BUG-ORQ-20260811-705 | cerrado en aceptación en esta entrega | V38/B10, pin del conector publicado | el contrato de aceptación exigía `agente_microvm` en `c1161cc5a57c`, aunque `go.mod` y `vendor/modules.txt` ya fijaban coherentemente la revisión posterior `928ef309a173` | el ratchet de versión no se actualizó al publicar y vendorizar el conector que añadió la extracción estricta de servicios firmados | la constante de aceptación se liga a la versión exacta realmente declarada y vendorizada; sigue prohibido cualquier `replace` local | actualizar el ratchet en el mismo cambio que el pin y verificar módulo, vendor y ausencia de reemplazo local |
| BUG-ORQ-20260811-706 | cerrado en aceptación en esta entrega | V38/B04, evidencia candidata no acreditable | la guarda de V38 planificada rechazaba cualquier ruta o contenido con `v38`, incluida `product/evidence/candidates/v38/b04_compatibility_v1.json`, aunque otra aceptación exige ese candidato `exercised_without_kvm` y su propia limitación `no_v38_promotion` | la ausencia de acreditación final y la existencia de resultados candidatos quedaron representadas por una única heurística de texto demasiado amplia | la excepción solo admite JSON regular bajo `candidates/v38/`, schema `orquesta.v38.*`, estado exacto `exercised_without_kvm` y limitación explícita `no_v38_promotion`; mutaciones a acreditado o sin la limitación siguen fallando | conservar separados candidatos y recibos finales; no ampliar estados permitidos ni promover V38 hasta A+B+C real sobre el mismo candidato |
| BUG-ORQ-20260811-707 | cerrado en código y pruebas en esta entrega | V38/runtime de proceso, capacidad y cuota de proveedor | tres Goals con workspaces aislados, acciones aprobadas y autoridad vigente permanecieron sin reclamar. Sin perfil, el selector falla cerrado correctamente por ausencia de capacidad/cuota; con un perfil Codex privado válido, el controlador esperaba `account/rateLimits/read`, `app-server` no publicaba una observación inicial antes del plazo y el servidor completo terminaba con `context deadline exceeded` en vez de quedar disponible y no listo para launches | el arranque del servidor estaba acoplado a la primera observación de cuota del `app-server`; un fallo o timeout transitorio cerraba controladores que ya tenían activo su bucle de reconexión | `abrirControladoresCuota` conserva los controladores y deja arrancar el servidor si falta la observación inicial, pero sigue recogiendo todo ante cancelación real del Build. La prueba de composición acredita cero launches sin cuota durable, observación real posterior, un único launch terminal y cierre exacto; focal bootstrap y focal `-race` verdes. La instancia privada conservó DB, Goals y receipts y nunca usó bypass legacy ni tocó `agente_microvm` | la admisión sigue fail-closed y no existe candidato sintético. Una señal tipada adicional de readiness puede mejorar el diagnóstico operativo, pero no es necesaria para preservar seguridad ni progreso |
| BUG-ORQ-20260811-708 | cerrado para filesystem local; acreditación V31 pendiente | V31/ArtifactStore filesystem y paridad S3 | el adapter filesystem usaba un espacio global por digest, no recibía `ProjectRef` y no persistía `MediaType`; dos proyectos compartían el objeto físico y un replay de los mismos bytes con otro tipo podía declararse válido, mientras S3 aislaba por proyecto y rechazaba el conflicto | el contrato content-addressed histórico del filesystem solo modelaba bytes/digest y la composición local de un proyecto; no tenía namespace opaco ni metadata causal durable para aplicar las invariantes multihost de V31 | `OpenForProject` deriva un namespace SHA-256 de la ref opaca y publica sidecar JSON canónico, privado, no reemplazable y durable; `Build` lo compone desde `project.default` tanto con identidad local como OIDC, sin leer ni migrar el layout legacy. Una suite común y el E2E de bootstrap acreditan CAS, replay exacto/conflicto de media, tamper, lectura acotada, aislamiento entre proyectos, restart y concurrencia con ganador único; focales normal/`-race`, aceptación V31 y `vet` verdes | OPS-13 sigue sin acreditarse hasta backend S3 compatible real, receipt de ese backend, E2E PostgreSQL+S3 multihost y candidato sellado; mantener `Open` solo como compatibilidad explícita de instalaciones legacy, nunca como fallback silencioso de `Build` |
| BUG-ORQ-20260811-709 | cerrado en código, pruebas y revalidación privada | V25/runtime Codex, controlador de cuota por `app-server` | tras desacoplar el arranque, la unidad privada quedaba disponible pero creaba y destruía un `codex app-server` aproximadamente cada segundo, sin una sola observación de cuota y con los tres launches fail-closed | Codex 0.147 emite notificaciones informativas como `configWarning` y `remoteControl/status/changed` después de `initialized`; `QuotaSession.Accept` solo permitía `account/rateLimits/updated`, clasificaba cada advisory como `appserver.method_not_allowed` y reiniciaba la sesión antes de recibir la respuesta correlacionada de `account/rateLimits/read` | diagnóstico privado registró únicamente kind/method/tamaño, sin payload de cuenta. La sesión ignora ahora sin inspeccionar payload toda notificación ajena a cuota, procesa exclusivamente `account/rateLimits/updated` y conserva el rechazo de requests y respuestas no correlacionadas; el helper E2E intercala ambos advisories antes de la cuota. Focal ×20, `-race` ×3 y `vet` verdes. El binario privado integrado `dcf4d99d` publicó observaciones durables de cuota y capacidad `available`, mantuvo estable `app-server` y lanzó una ejecución Codex real sin bypass | no convertir notificaciones advisory en disponibilidad ni aceptar requests o respuestas no correlacionadas del servidor en esta sesión; conservar la revalidación privada y sus Goals/receipts mientras haya trabajo activo |
| BUG-ORQ-20260811-710 | revisión de convergencia abierta; ambos backends son producto válido | Agente MicroVM/backends Firecracker y Docker, lifecycle físico compartido | la arquitectura del propietario exige Firecracker en equipos con KVM y Docker en equipos sin KVM; la rama Docker implementó correctamente el segundo backend y un canary real, pero además del adapter específico mantiene un puerto paralelo de 17 operaciones, servicio de lifecycle/recovery, API y persistencia SQLite propios mientras `main` evolucionó el ejecutor físico neutral y B12 | los backends se desarrollaron en ramas disjuntas desde una base común sin una integración posterior que demuestre qué lifecycle, recovery y persistencia son autoridad neutral compartida y qué piezas pertenecen exclusivamente a cada aislamiento; la existencia de ambos backends no es la incidencia | la rama limpia y publicada `agente_microvm@2ead546` tiene 41 commits propios desde `74cb1d3`; `main@963d145` tiene 30 propios y trabajo físico activo. El canary Docker 29.5.2 acredita launch, replay, sesión terminal y cleanup. La comparación identifica `MotorContenedorV1` paralelo a `MotorMicrovm`, `ServicioContenedoresV1` y repositorios propios dentro de unas 12.900 líneas añadidas, pero no invalida Docker como backend para hosts sin KVM | preservar ambos backends; seleccionar por capacidad/configuración del host sin degradación silenciosa durante una ejecución. Cuando `main` alcance frontera segura, auditar y extraer una sola autoridad neutral de lifecycle/recovery/persistencia, manteniendo únicamente Engine/aislamiento/identidad/traducción específicos y los smokes físicos de Docker y Firecracker |
| BUG-ORQ-20260811-711 | cerrado antes de publicar en el candidato de migración 035 | V38/B11-B12, terminal lifecycle tras lease expirada y re-claim | después de persistir el intento físico Q y reiniciar, el scheduler podía volver a reclamar la acción expirada; la terminalización del intento original guardaba su receipt histórico pero dejaba en outbox la autoridad del claim nuevo, haciendo divergir action, consumo y recovery | el writer terminal reutilizaba el outbox vivo sin restaurar dentro de la misma transacción los campos exactos del claim que autorizó el `EffectAttempt` histórico | el CAS terminal repone token, worker, delivery attempt, fence y lease históricos antes de completar la acción y escribir receipt/consumo; el test `TestAgentEnvironmentLifecycleStoreQPCSurvivesRestartAndExpiredLease` realiza restart, lease expirada y re-claim real y demuestra cero segundo attempt/efecto. Focal normal, `-race`, paquete SQLite completo y `vet` verdes | mantener separadas autoridad física histórica y reclamaciones posteriores; cualquier reconciliación nueva debe repetir restart, re-claim y recovery sin fabricar receipt ni ejecutar de nuevo el efecto |
| BUG-ORQ-20260811-712 | cerrado antes de publicar; gate de arquitectura y trazabilidad V31 verdes | V31/ArtifactStore, suite contractual compartida | filesystem y S3 importaban la suite común desde `internal/adapters/artifact/contracttest`; aunque era código de test, el gate correcto rechazó que un adapter dependiera de otro paquete bajo la capa de adapters | el helper reusable se ubicó junto a las implementaciones que probaba y los focales V31 no ejecutaron inicialmente `TestRebuildArchitecture`; además el fixture solo comparaba una ruta textual sin comprobar que existiera | la suite se movió sin cambio semántico a `internal/testsupport/artifactcontract`; la excepción arquitectónica permite solo ese import exacto desde `_test.go` y rechaza producción y rutas mutantes. El fixture/aceptación V31 apuntan al destino real y exigen `suite.go`; FS/S3, `-race`, arquitectura, aceptación, `vet` y diff-check quedan verdes | todo helper compartido por adapters hermanos vive fuera de `internal/adapters`; ejecutar arquitectura además de focales y comprobar materialización de las rutas declaradas por fixtures |
| BUG-ORQ-20260811-713 | cerrado antes de publicar en migración 035 y recovery | V38/B11-B12, gobierno de acciones Q-P-C | el primer esquema 035 permitía insertar `quiesce_agent`, `preserve_agent_environment` o `close_agent_environment` con `governance_version=0` y sin `effect_intent`; el scheduler podía reclamarlas, pero aplicación nunca podía preparar el efecto, creando un loop durable imposible que recovery aceptaba | Q-P-C se añadieron a los enums y al selector de claims sin ratchet simultáneo que exigiera su gobierno e intent exactos en tabla, trigger y recuperación | CHECK y trigger exigen gobierno, intent y frontera lifecycle causal; claim no devuelve la corrupción inyectada y recovery la rechaza. El negativo reabre la DB alterada, demuestra cero claim y falla cerrado en validación; focal, `-race`, paquete SQLite y `vet` verdes | al añadir un nuevo `ActionKind` físico, actualizar en el mismo cambio enum, admisión, selector, DDL, triggers, recovery y negativo de corrupción; no inferir autoridad por la mera presencia del kind |
| BUG-ORQ-20260811-714 | cerrado antes de publicar en el Store SQLite lifecycle | V38/B11-B12, éxito durable no recuperable | `RecordAgentEnvironmentLifecycleInitial` podía aceptar un snapshot estructural no ligado al launch persistido y el CAS terminal no cotejaba `PhysicalReceipt` con `EffectReceipt.ExternalRef` ni la preservación con su receipt y manifiesto exactos; el writer devolvía éxito aunque recovery posterior rechazase la historia | la validación de aplicación cubría forma y secuencia, mientras el adapter solo reconstruía la causalidad completa durante recovery y no antes del commit | initial y terminal ejecutan ahora `ReplayAgentEnvironmentLifecycleSnapshot` contra el `GoalRecord` y los hechos recién materializados dentro de la misma transacción. Negativos cruzan launch, receipt físico, manifest ref/digest y fallo intermedio; todos hacen rollback sin receipt, preservación, consumo ni next-action parcial, reabren verdes y continúan el happy path. Upgrade V34 poblado y suites normal/`-race`/SQLite/`vet` verdes | todo writer que recovery pueda rechazar debe ejecutar la misma validación causal antes del commit; mantener rollback y reapertura como negativos obligatorios al ampliar snapshot o receipt |
| BUG-ORQ-20260811-715 | cerrado en harness de aceptación | V06/V09/V10, fixtures tras capacidad V38 | los escenarios de outbox, backup e identidad esperaban ejecutar un launch, pero `ProcessNext` devolvía `idle` antes de invocar al agente | las fixtures históricas no publicaban candidato ni cuota física después de que la admisión V38 pasase a ser fail-closed; el rojo se confundió inicialmente con la preservación Q-P-C recién añadida | el helper V06 aporta capacidad y cuota válidas, V09 reclama mediante `Orchestrator.ClaimNextAction` y el fake declara explícitamente que no exige preservación física. V06/V09/V10 pasan juntos, incluido `-race` en 266 s, sin cambiar producción ni relajar lifecycle | toda fixture que espere un efecto físico debe satisfacer la admisión vigente por la misma superficie que producción; un resultado `idle` se diagnostica antes de atribuirlo al adapter |
| BUG-ORQ-20260811-716 | cerrado en ratchets V02/V13/V14 | V02, superficie congelada y autoridad de claim | V02 contaba 548 ficheros `cmd` frente a los 546 congelados y marcaba `bootstrap/scheduler.go` como writer ajeno al Orchestrator; V13/V14 heredaban el falso rojo | el nuevo ejecutor Codex de producto se mezcló en el hash legacy y el scanner no distinguía una delegación exacta `scheduler -> Orchestrator` de un acceso directo al repositorio | el ejecutor queda declarado como `product_root` y excluido únicamente del hash legacy, que conserva count/digest originales. El scanner admite solo `bootstrap.scheduler -> application.Orchestrator`, mantiene `Orchestrator -> repository` como writer y rechaza caller desconocido o bypass; V13 reutiliza el constructor V06/V09 canónico. Focales V02/V13/V14 verdes | congelar legado no impide añadir producto en raíces declaradas; los guards de autoridad deben modelar delegación tipada y seguir rechazando el salto directo al store |
| BUG-ORQ-20260811-717 | cerrado en aceptación V20/V22 | bindings de comandos y adapters futuros | V20 prohibía cualquier import de application en todo `interfaces/httpapi`, incluido un endpoint posterior no perteneciente al registro V20, y V22 exigía que `internal/adapters/state` contuviera físicamente solo SQLite, rechazando la foundation Postgres V31 no compuesta | dos ratchets históricos confundían su scope acreditado con una prohibición global sobre toda evolución posterior | V20 audita una lista exacta de bindings HTTP/MCP/CLI del registro y conserva el veto a imports/calls lifecycle en ellos; V22 comprueba que bootstrap compone SQLite y no Postgres, sin prohibir adapters futuros inertes. Los grupos V20 y V22 completos quedan verdes | los ratchets de una vertical validan su autoridad y wiring, no reservan para siempre un directorio entero ni juzgan endpoints que no pertenecen a su contrato |
| BUG-ORQ-20260811-718 | cerrado en extractor de aceptación V21 | i18n tipado y catálogo vivo | V21 declaraba 55 claves válidas como ausentes y después las denunciaba como catálogo sin uso | el extractor AST ignoraba preguntas Wizard no `domains.*`, valores de mapas `*MessageKeys`, asignaciones locales `messageKey` y el helper de diagnósticos de credenciales | el extractor reconoce Help/Example y options de preguntas no-domain según el manifest, sigue mapas tipados y asignaciones de message key y cubre diagnósticos de credenciales; no se borró ninguna clave ni se alteró producto. Grupo V21 completo y aceptación global verdes | un extractor de catálogo debe seguir las formas tipadas reales de la composición; no reparar un falso `unused` borrando mensajes públicos válidos |
| BUG-ORQ-20260811-719 | cerrado operativamente en esta integración | disciplina de gates del agente integrador | el primer commit del Store se publicó tras focales, raíz e `internal/...`, pero sin ejecutar el `go test ./...` exacto exigido; la aceptación completa aún contenía los rojos 715-718 | el integrador trató compilación global más suites parciales como equivalente al gate transversal y produjo un falso «verde» | se detuvo el siguiente push, se clasificaron y corrigieron todos los fallos, y la aceptación completa pasó en 47,794 s antes de renovar el recibo real y repetir el gate global exacto | para cambios transversales ejecutar literalmente `go test -mod=vendor -count=1 ./...`; una compilación `-run '^$'` o una selección de verticales no lo sustituye |
| BUG-ORQ-20260811-720 | cerrado en guard F3-R2 | scripts históricos bajo sparse checkout | el gate global falló `TestF3R2ShellSyntax` porque `orquesta_server_drain.sh` y su test están versionados pero excluidos intencionadamente por el sparse checkout; `bash -n` recibió rutas inexistentes | el guard suponía que todo fichero tracked estaba materializado y no distinguía una exclusión `skip-worktree` declarada de una eliminación real | el test usa el fichero del worktree cuando existe; si falta, exige marca exacta `S` en el índice, lee `HEAD:ruta`, lo materializa solo en `t.TempDir` y valida la misma sintaxis. Una ausencia no-sparse sigue fallando; focal y paquete `scripts` verdes | los gates de contenido en repos sparse deben validar el objeto Git excluido sin mutar el sparse checkout ni convertir una desaparición normal en skip |
