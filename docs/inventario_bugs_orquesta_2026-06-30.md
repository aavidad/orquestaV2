...
| BUG-ORQ-20260725-467 | abierto, primer corte autorizado | arquitectura/runtime general de agentes y almacenamiento efímero | todavía no se ha acreditado migrar agentes a Firecracker, ejecutar workspaces/rootfs/caches de agentes en RAM/tmpfs, introducir checkpoints frecuentes ni el transporte acotado de agentes | una microVM de `TestAttestor` V23 no demuestra que el runtime completo tenga contratos de aislamiento, coste, persistencia, recovery y egress adecuados | decisión actualizada el 2026-07-26 en `product/roadmap.json` y `docs/arquitectura_comunicacion_agentes_firecracker_2026-07-25.md`: sin NIC/TAP/NAT, vsock exclusivo a broker Orquesta y proxy controlado, identidad de lanzamiento ligada a proyecto/Goal/WorkItem/ejecución/agente, sin east-west ni inbound; TestAttestor conserva red y vsock ausentes | implementar primero contrato, validación y render determinista con negativos; después cablear proxy guest loopback→vsock y broker host-side; no ejecutar root ni acreditar hasta E2E multi-microVM con policy/plan/receipt digests y cleanup; almacenamiento RAM/checkpoints sigue pendiente separado |
