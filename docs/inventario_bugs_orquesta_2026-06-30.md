...
| BUG-ORQ-20260804-624 | abierto; orden de terminación emitida, proceso aún en espera de E/S del kernel | tooling de agentes e higiene de búsquedas | un subagente lanzó `find /home/alberto/Trabajo` para localizar un único archivo Rust ya conocido y el proceso quedó en estado `D`, consumiendo CPU sin producir resultado útil; terminar el shell padre no pudo retirar inmediatamente el hijo bloqueado | se usó una búsqueda global por filesystem en vez de la ruta exacta y `rg` acotado exigidos por el contrato operativo; una espera de E/S no interrumpible tampoco responde a `SIGTERM` hasta volver del kernel | PID observado `2563395`, comando y parentesco censados antes de enviar `SIGTERM`; el subagente recibió orden explícita de no esperar ni repetir la búsqueda y continuar solo dentro de `agente_microvm`; las pruebas Rust propias terminaron sin procesos residuales | los prompts de programación deben proporcionar rutas exactas y prohibir también `find` global; antes de cerrar la sesión hay que recensar el PID y confirmar su desaparición, sin matar por patrón ni tocar procesos ajenos |
