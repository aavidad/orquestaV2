...
| BUG-ORQ-20260806-699 | abierto; workflow de aplicación Q-P-C listo, adaptador durable y caller productivo pendientes | B11/B12, lifecycle del agente microVM y cierre de Goal | la ruta B10 lanza y ejecuta Codex dentro de la microVM, pero una observación terminal intenta cerrar la ejecución como éxito aunque el adaptador exige preservación; SQLite rechaza después el terminal porque no existe recibo de preservación y el Goal no puede cerrarse limpiamente | el lanzamiento y la sesión se conectaron antes que la cadena física de éxito `quiesce -> preserve -> close`; `RegistrarPreservacionEntornoAgente` existe, pero no tiene caller productivo, y el wrapper microVM no aporta un lifecycle durable compuesto | auditoría causal de `ProcessNext/Claim` hasta sesión Codex confirma B10; `2d95fe49` liga cierre y receipt al `ApplicationReceiptRef`; el servicio de aplicación ya gobierna Q-P-C, persiste Attempt antes del efecto, genera la continuación siguiente y solo permite finalizar tras `closed`. Focal de aplicación y `-race` verdes, pero aún no existe implementación SQLite del nuevo Store ni wiring bootstrap; no se ejecutó el smoke | implementar el Store CAS durable y componer el servicio con lifecycle/reconciliador físicos; solo después cerrar Execution, WorkItem y Goal y acreditar reinicio y cero residuos físicos |
