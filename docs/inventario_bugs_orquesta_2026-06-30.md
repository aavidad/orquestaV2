...
| BUG-ORQ-20260806-680 | abierto; tooling operativo acotado | PFC-06, preparación de perfil y credenciales | no existe ejecutable productivo que transforme `candidate-manifest.json` en un descriptor sellado ni superficie operativa que provisione la privada Ed25519 y `auth.json`; solo hay helpers de tests | los builders producen activos reproducibles, pero la promoción a configuración consumible por bootstrap quedó manual/no codificada | los contratos públicos de descriptor y `CredentialStore` ya existen; una minitarea independiente está construyendo el generador sin secretos | crear herramientas estrechas, atómicas y auditables; no copiar helpers `_test`, no incluir material en descriptor/receipts y no pedir sudo fuera del staging/servicio |
