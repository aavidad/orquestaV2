...
| B10.5 | Reinicio del host, concesión byte a byte, cerca, duplicado, receipt/socket perdidos, cursor durable, concurrencia y `-race`. | Sigue abierta: el intento histórico ya es durable y recuperable, pero faltan separar la cerca física de la cerca nueva de confirmación, reclamar/CAS, cablear el reconciler y probar reinicio/carreras extremo a extremo. | Se mide al cerrar. |
