...
| B10.5 | Reinicio del host, concesión byte a byte, cerca, duplicado, receipt/socket perdidos, cursor durable, concurrencia y `-race`. | Sigue abierta: selección, intento, lease, referencia CAS, writer, wrapper y `ProcessClaim -> ReconcileLaunch` ya son durables; faltan el reencolado temporal conservador y el reinicio/carrera extremo a extremo. | Se mide al cerrar. |
