...
| B10.5 | Reinicio del host, concesión byte a byte, cerca, duplicado, receipt/socket perdidos, cursor durable, concurrencia y `-race`. | Sigue abierta: selección, intento, lease, referencia CAS, writer y wrapper ya son durables; falta el caso de uso `ProcessClaim -> ReconcileLaunch` y probar reinicio/carreras extremo a extremo. | Se mide al cerrar. |
