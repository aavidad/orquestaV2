...
| B10.5 | Reinicio del host, concesión byte a byte, cerca, duplicado, receipt/socket perdidos, cursor durable, concurrencia y `-race`. | Sigue abierta: intento, lease y referencia CAS ya son durables; faltan selección automática, writer del receipt histórico/consumo actual, cablear el reconciler y probar reinicio/carreras extremo a extremo. | Se mide al cerrar. |
