...
| Hijo | Write-set y resultado | Orden | Presupuesto |
|---|---|---|---:|
| A08.1 | `e6352ec2` más la revalidación del candidato: aceptación neutral con SQLite y almacén de artefactos reales, agente sustituto contractual y cohortes lógicas `1/16/70/500`. Reclama los escalones de capacidad neutral `1/5/10/16/20`, conserva las reservas al reabrir y vuelve a ofrecer el mismo trabajo tras refrescar una observación obsoleta. | `exercised`; cierra la comprobación neutral y abre B03 sin arrancar KVM, Firecracker ni Agente MicroVM. | Consumido `P=0,V=349` |
| A08.2 | La misma prueba exige por nombre los contratos ejecutables de orden y ACK de buzón, parada, recuperación en fronteras causales, reserva exacta, preservación y despacho; además guarda que el núcleo neutral no contenga dependencias físicas. | Tres rondas normales, contratos focales y `-race` verdes. No acredita por sí sola `ORC-28` ni V38. | Incluido en `V=349` |
...
### B09 — `P=2683, V=1179` regularizado
...
Las cifras de las cinco filas son la estimación previa y no se usan ya como
ledger. El corte reproducible `7c9ef00..bb90346` consume en bruto
`P=+2683/-117,V=+1179/-69`; documentación añade `D=+255/-17`. El
[ADR de regularización B09](adr_v38_regularizacion_b09_2026-08-04.md) fija el
método, prohíbe reutilizar B08 y reserva a B10 la traducción productiva de
receipts en Orquesta. B09 queda `exercised` sin KVM y no acredita B12.

### B10 — techo histórico `P=280, V=430`; regularización pendiente

La inspección del 2026-08-04 demostró que el cliente v1 existente gobierna el
ciclo físico y órdenes síncronas, pero no una sesión de agente observable. El
[ADR de frontera de trabajo](adr_v38_b10_frontera_trabajo_agente_2026-08-04.md)
es autoridad para este tramo. La descomposición anterior suponía esa API y
queda sustituida por las minitareas siguientes; no se implementa un adaptador
sobre estados físicos aproximados.
...
| B10.0b1 | `agente_microvm@c8fcd3c` + `a8d19fe`: modelo durable de sesión con siete estados, cuarentena explícita `Ambigua`, resultado terminal y autoridad única sobre secuencia, revisiones y cambios de eventos/entradas. Conserva revisión de trabajo reservada, contenido por referencia opaca, truncación única y snapshots antes/después; todavía no incorpora ledger. | Cerrada tras cuatro revisiones premium, suite completa, Clippy estricto y build musl; sin KVM. | Se mide al cerrar B10.0 |
...
| B10.5b | `5f6792b0`: el modelo conserva `ClaimLeaseUntil`, selecciona un único intento ambiguo sin receipt y reconstruye la autoridad histórica byte a byte bajo un claim de recuperación con fence superior. Un launcher genérico no entra: debe implementar `AgentLaunchReconciler`. | Cierra solo selector/puerto offline con focales normal/`-race` y revisión premium. | Sus dependencias de reconciler y productor cerraron después en B10.5c/d1a; migración, CAS y wiring siguen abiertos. |
| B10.5c | `09527249`: `agentmicrovm.Adapter.ReconcileLaunch` reutiliza el pipeline íntegro de `Launch`; firma, payload, idempotencia, sesión y receipt no tienen una segunda implementación. | Cerrada offline con replay exacto, divergencia `409`, cancelación, paquete y revisión premium. | No acredita socket/KVM ni CAS del host. |
| B10.5d1a | `42f28e32`: todo intento nuevo copia el vencimiento exacto del claim y la validación rechaza cero, `<= StartedAt` o mutación antes de autorizar el efecto. | Cerrada en aplicación, normal/`-race` y revisión premium. | Persistencia/migración SQLite sigue abierta. |
| B10.5d1b | `d961e43f`, `1f7acfa6`, `e20d1dff` y `60cdf4d6`: el esquema 027 persiste y recupera el lease causal exacto; la migración solo reconstruye desde una identidad histórica inequívoca, los ratchets distinguen latest de prefijos antiguos y todas las guardas aplican `StartedAt <= ConfirmedAt < ClaimLeaseUntil`. | Cerrada con migración/rollback/copia, focales normal/`-race`, paquete SQLite completo, `vet` y revisión premium. | No cierra todavía la cerca nueva de confirmación/CAS. |
| B10.5d2a | `14b9d9e0` y `c322e156`: `EffectReceipt.ActionFence` conserva la cerca física histórica y `outbox.recovery_effect_attempt_ref` fija, dentro del CAS del claim, el intento exacto que se reconciliará. No existe `ConfirmationFence`: `ActionConsumptionReceipt.Fence` sigue siendo la autoridad normalizada del consumo actual. `requireClaim` reejecuta el selector canónico de aplicación. | Cerrada en persistencia/CAS, con revisión xhigh, paquete SQLite completo, focales normal/`-race`, `vet` y compatibilidad de prefijos/downgrades. | No incluye selector automático, writer recovery ni reconciler cableado. |
| B10.5d2b | `13879a05`: el writer persiste en una sola transacción el `EffectReceipt` con la cerca física del intento histórico y el `ActionConsumptionReceipt` con la cerca del claim de recuperación actual. Revalida intento, lease histórico, ref CAS y outbox actual, rechaza un segundo cierre y no crea otro `EffectAttempt`. | Cerrada con positivo durable, camino normal, once cruces negativos con rollback, repetición sin duplicado, focal, paquete SQLite, `-race`, `vet` y revisión xhigh sin P0/P1. | No incluye selector automático ni llamada real a `ReconcileLaunch`. |
| B10.5d3 | `f3c7bcb9`: `ClaimNextAction` detecta la única huella física ambigua antes de la admisión normal, descarta peers con zero-release causal, reutiliza approval y reservas históricas, revalida con la cerca real y persiste la ref exacta en el CAS. Receipt, ambigüedad, esquema anterior y autoridad revocada fallan o aparcan sin fallback; `ExcludeLaunch` sigue excluyendo retries resueltos. | Cerrada con SQLite completo dos veces, focales normal/`-race`, aplicación, `vet`, rollback y revisión xhigh sin P0/P1. | No ejecuta todavía `ReconcileLaunch`. |
| B10.5d4a | `5c6ccfca`: el wrapper bootstrap MicroVM conserva estructuralmente `AgentLaunchReconciler` y reenvía `ReconcileLaunch` bajo la misma compuerta de shutdown. | Cerrada con paquete bootstrap, focal `-race` y prueba de cero llamadas a `Launch`. | Falta despacharlo desde `ProcessClaim`. |
| B10.5d4b0 | `ce195888`: el esquema 029 y el CAS fijan `environment_preservation_required` en `queued -> dispatching`, antes del efecto. Conservan solo la compatibilidad histórica `dispatching -> running` y rechazan reversión o mutación lateral. | Cerrada con reapertura `true/false`, prefijos/downgrade, SQLite completo, focal `-race`, `vet` y revisión xhigh. | No acredita por sí sola reconciliación ni KVM. |
| B10.5d4b1 | `5c048bf4`: `ProcessClaim` despacha `RecoverEffect + LaunchAgent` exclusivamente a `ReconcileLaunch`, reconstruye sesión/accesos y request históricos, recarga antes del writer y no crea otro `EffectAttempt` ni llama a `Launch`. Revocación previa aparca sin liquidar; posterior al efecto conserva el receipt. | Cerrada offline con aplicación completa, SQLite completo, focales `-race`, compilación transversal, `vet` y revisión xhigh sin P0/P1. | El retry temporal tipado y el E2E físico siguen abiertos. |
| B10.5d4b1a | `5c048bf4`: una guarda obligatoria de solo lectura valida antes de sesión/proveedor el claim vigente, lease, disposición/ref, autoridad causal, presupuesto activo, capacidad exacta y placement. SQLite lo hace en una transacción con reloj fiable; memory bajo mutex/reloj inyectado. | Cerrada con negativos de expiración, reclaim, cruces y transición durable de capacidad, sin mutación ni efectos externos. | El TOCTOU posterior es equivalente al camino normal: `ReconcileLaunch` permanece exacto e idempotente y nunca relanza. |
| B10.5d4b2 | `0fb8ad36` y `f768716e`: errores `Temporary()` directos o envueltos de sesión/reconciler reencolan el mismo intento histórico sin settlement, clear ni `Launch`. El esquema 030 conserva `recovery_effect_attempt_ref` durante el estado sin claim, revalida budget/capacity/placement dentro del writer y permite aparcar después de revocación sin reescribir autoridad. | Cerrada con restart/reclaim, texto no tipado, CAS perdido, cruces, revocación posterior, SQLite completo, focales `-race`, dos revisiones xhigh, `vet` y diff. | No acredita todavía acuse perdido extremo a extremo ni KVM. |
| B10.5d5 | `ad3d6483`: E2E application+SQLite con dos reaperturas, acuse perdido tras receipt remoto, retry durable, carrera de dos workers y un único ganador. Las dos reconciliaciones llevan requests idénticos; quedan un attempt, receipt con cerca histórica, consumo con cerca actual, budget intacto, capacity consumida y `observe_agent` siguiente. | Cerrada con focal 20/20, `-race` 10/10, SQLite completo, `vet` y revisión xhigh sin P0/P1. | Acredita solo proceso/SQLite offline; no host reboot, UDS real, KVM, huésped ni Codex. |
| B10.5 | Reinicio del host, concesión byte a byte, cerca, duplicado, receipt/socket perdidos, cursor durable, concurrencia y `-race`. | Cerrada para application+SQLite offline: selección, intento, lease, ref CAS, writer, wrapper, `ProcessClaim -> ReconcileLaunch`, retry, acuse perdido y carrera tienen evidencia. La acreditación física de host/UDS/KVM permanece abierta y no se infiere de este cierre. | Delta offline medido; físico se traslada al corte explícito del primer agente real. |
...
- **Presupuesto:** la suma exacta de las 25 tareas V38 es
  `P=9.283` líneas no generadas y `V=10.712` líneas de verificación y arneses. El
