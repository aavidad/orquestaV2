...
| B10.5 | Reinicio del host, concesión byte a byte, cerca, duplicado, receipt/socket perdidos, cursor durable, concurrencia y `-race`. | Sigue abierta: selección, intento, lease, referencia CAS, writer, wrapper, `ProcessClaim -> ReconcileLaunch` y retry temporal ya son durables; falta el reinicio/acuse perdido/carrera extremo a extremo. | Se mide al cerrar. |
