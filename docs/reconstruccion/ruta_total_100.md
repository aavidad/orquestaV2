...
| V07 Config mutable y doctor | V06 | Registro único, TOML no secreto, effective config redactado, CAS/replace+fsync, `pending_restart`, generación de getters/docs/UI y doctor `reuse|replace|new`. Gate: cero env/key/default/alias fuera del ingress canónico. |
...
| V13 Mailbox y handoff | V06,V12 | Admit, claim, lease, delivery, consume y ACK por destinatario exacto; mensajes y handoffs compactos. Gate: crash claim/ACK permite reclaim sin pérdida; ACK+replay no redeliver; sucesor no suplanta destinatario. |
...
| V24 Web administrativa | V10,V20–V23 | Web responsive/PWA para proyectos, Goals, fases, trabajo, agentes, ramas, artifacts, decisions, cuotas, reviews, effects y config; streaming y rescate sin autoridad paralela. Gate: RBAC, WCAG 2.2 AA automatizado, matriz de teclado, estados comparados con queries canónicas e i18n en E2E browser. Desktop nativo queda rechazado; PWA es la única UI instalada. |
...
- V13, partes de V15 y caracterización de V16 pueden avanzar en paralelo tras
  fijar protocolo/repository.
