# Glosario del wizard de nueva app

Fuente generada desde el catalogo i18n es/en del wizard.

## nueva_app.wizard.help.option.datos.consulta

- es: Consultar datos existentes significa leer informacion que ya vive en otro sitio. Conviene si la app es una ventana a datos externos.
- en: Read existing data means information already lives somewhere else. Use it when the app is a window into external data.

## nueva_app.wizard.help.option.datos.gestion

- es: Gestionar datos propios significa crear y guardar informacion dentro de la app. Conviene si los usuarios registran cosas nuevas.
- en: Manage own data means creating and saving information inside the app. Use it when users record new things.

## nueva_app.wizard.help.option.datos.sin_persistencia

- es: Sin persistencia propia significa que la app no guarda datos duraderos. Conviene para calculos, asistentes o pantallas temporales.
- en: No own persistence means the app does not keep long-lived data. Use it for calculators, helpers, or temporary screens.

## nueva_app.wizard.help.option.deploy.contenedor

- es: Contenedor empaqueta la app para moverla entre servidores de forma repetible. Conviene para web o API que deben desplegarse con control.
- en: Container packages the app so it can move between servers repeatably. Use it for controlled web or API deploys.

- ejemplo es: Ejemplo: como una caja cerrada con la app y sus piezas, lista para abrirse igual en varios servidores.
- example en: Example: like a sealed box with the app and its pieces, ready to open the same way on different servers.

## nueva_app.wizard.help.option.deploy.desktop

- es: Paquete desktop significa instalador o ejecutable de ordenador. Conviene para uso de oficina o offline.
- en: Desktop package means an installer or computer executable. Use it for office or offline use.

## nueva_app.wizard.help.option.deploy.kubernetes

- es: Kubernetes coordina muchos contenedores en servidores. Conviene solo si ya tienes operacion y escala que lo justifiquen.
- en: Kubernetes coordinates many containers on servers. Use it only when operations and scale justify it.

- ejemplo es: Ejemplo: como un encargado que reparte muchas cajas de la app entre varios servidores y las sustituye si fallan.
- example en: Example: like a coordinator that spreads many app boxes across servers and replaces them if they fail.

## nueva_app.wizard.help.option.deploy.local

- es: Local significa ejecutar en una maquina concreta. Conviene para pruebas o uso individual sin publicar.
- en: Local means running on one specific machine. Use it for tests or individual use without publishing.

## nueva_app.wizard.help.option.deploy.mobile_store

- es: Tiendas moviles significa publicar como app de iOS o Android. Conviene si los usuarios la instalaran en el telefono.
- en: Mobile stores means publishing as an iOS or Android app. Use it when users will install it on phones.

## nueva_app.wizard.help.option.deploy.paas

- es: PaaS es una plataforma que aloja la app y simplifica operacion. Conviene si prefieres menos mantenimiento de servidores.
- en: PaaS is a hosted platform that simplifies operations. Use it when you want less server maintenance.

- ejemplo es: Ejemplo: como alquilar un local con luz y mantenimiento incluidos en vez de construir el edificio.
- example en: Example: like renting a place with power and maintenance included instead of building the building.

## nueva_app.wizard.help.option.deploy.serverless

- es: Serverless ejecuta partes de la app solo cuando hace falta. Conviene para tareas eventuales o carga irregular.
- en: Serverless runs parts of the app only when needed. Use it for occasional tasks or uneven load.

- ejemplo es: Ejemplo: como encender una luz solo cuando alguien entra en la habitacion y apagarla despues.
- example en: Example: like turning on a light only when someone enters the room and turning it off afterward.

## nueva_app.wizard.help.option.dominio.flujo_diario

- es: Describir el dia normal cuenta los pasos reales del usuario. Conviene cuando el nombre del dominio no basta.
- en: Describe the normal day tells the user's real steps. Use it when the domain name alone is not enough.

## nueva_app.wizard.help.option.dominio.integraciones

- es: Describir herramientas o datos a conectar enumera sistemas externos. Conviene cuando el valor esta en unir piezas existentes.
- en: Describe tools or data to connect lists external systems. Use it when value comes from joining existing pieces.

## nueva_app.wizard.help.option.integracion.calendar.caldav

- es: CalDAV es una forma comun de hablar con calendarios compatibles. Conviene si tu servidor de calendario no es Google ni Microsoft.
- en: CalDAV is a common way to talk to compatible calendars. Use it when your calendar server is not Google or Microsoft.

- ejemplo es: Ejemplo: como usar un enchufe comun para calendarios compatibles aunque sean de marcas distintas.
- example en: Example: like a common plug for compatible calendars from different brands.

## nueva_app.wizard.help.option.integracion.calendar.enterprise

- es: Agenda empresarial configurable conecta calendarios sin fijar proveedor. Conviene si aun no sabes si sera Google, Microsoft u otro.
- en: Configurable company calendar connects calendars without locking a provider. Use it when Google, Microsoft, or another service is still undecided.

## nueva_app.wizard.help.option.integracion.calendar.google

- es: Google Calendar o Workspace conecta con calendarios de Google. Conviene si tus citas ya viven alli.
- en: Google Calendar or Workspace connects to Google calendars. Use it when your appointments already live there.

## nueva_app.wizard.help.option.integracion.calendar.microsoft

- es: Microsoft 365 conecta con calendarios de Microsoft. Conviene si tu empresa usa Outlook o Teams.
- en: Microsoft 365 connects to Microsoft calendars. Use it when your company uses Outlook or Teams.

## nueva_app.wizard.help.option.integracion.deferred

- es: Decidir mas tarde deja la integracion para otra fase. Conviene si todavia no sabes proveedor, datos o permisos.
- en: Decide later leaves the integration for another phase. Use it when provider, data, or permissions are unknown.

## nueva_app.wizard.help.option.integracion.domain.capability

- es: Capacidad principal del dominio representa la funcion central del negocio. Conviene cuando quieres construir esa funcion dentro de la app.
- en: Main domain capability represents the core business function. Use it when you want that function built inside the app.

## nueva_app.wizard.help.option.integracion.domain.sync

- es: Sincronizar datos del dominio copia o actualiza datos con otra herramienta. Conviene si ya trabajas con un sistema externo.
- en: Synchronize domain data copies or updates data with another tool. Use it when you already work in an external system.

## nueva_app.wizard.help.option.integracion.ecommerce.capability

- es: Catalogo, carrito y pagos agrupa lo necesario para vender. Conviene si el usuario elige productos y paga.
- en: Catalog, cart, and payments group what is needed to sell. Use it when users choose products and pay.

## nueva_app.wizard.help.option.integracion.ecommerce.payments_sync

- es: Pagos y stock sincronizados mantiene cobros y existencias alineados. Conviene si vender cambia el inventario.
- en: Synchronized payments and stock keeps charges and inventory aligned. Use it when a sale changes availability.

- ejemplo es: Ejemplo: cuando alguien compra la ultima entrada, el cobro y el contador de entradas bajan a la vez.
- example en: Example: when someone buys the last ticket, the payment and ticket counter change together.

## nueva_app.wizard.help.option.integracion.gobierno.oauth_alta

- es: OAuth con criticidad alta significa que el usuario autoriza acceso protegido y el fallo importa. Conviene para cuentas, pagos o datos sensibles.
- en: OAuth with high criticality means the user authorizes protected access and failure matters. Use it for accounts, payments, or sensitive data.

- ejemplo es: Ejemplo: el usuario pulsa Autorizar para que la app acceda a su cuenta sin entregar su contrasena.
- example en: Example: the user clicks Authorize so the app can access the account without receiving the password.

## nueva_app.wizard.help.option.integracion.gobierno.publica

- es: Publica y criticidad baja significa que la conexion lee datos abiertos o poco sensibles. Conviene si no hay cuentas ni informacion privada.
- en: Public and low criticality means the connection reads open or low-risk data. Use it when there are no accounts or private information.

## nueva_app.wizard.help.option.integracion.gobierno.simple

- es: Auth simple y criticidad media significa acceso controlado sin asumir alto riesgo. Conviene para empezar sin secretos complejos.
- en: Simple auth and medium criticality means controlled access without assuming high risk. Use it to start without complex secrets.

- ejemplo es: Ejemplo: una llave sencilla para una conexion interna de bajo riesgo.
- example en: Example: a simple key for a low-risk internal connection.

## nueva_app.wizard.help.option.integracion.maps.public_sources

- es: Mapas con fuentes publicas usa datos cartograficos accesibles sin atarse a un proveedor privado. Conviene para ubicaciones generales.
- en: Maps with public sources uses map data without locking to a private provider. Use it for general locations.

- ejemplo es: Ejemplo: usar un mapa publico para ubicar centros sin guardar claves privadas al principio.
- example en: Example: use a public map to place centers without storing private keys at first.

## nueva_app.wizard.help.option.locale.en

- es: Ingles significa que los textos iniciales salen en ingles. Conviene si usuarios o documentacion principal estaran en ingles.
- en: English means the first texts are in English. Use it when users or the main documentation will be in English.

## nueva_app.wizard.help.option.locale.es

- es: Espanol significa que los textos iniciales salen en espanol. Conviene si la mayoria del equipo trabaja en espanol.
- en: Spanish means the first texts are in Spanish. Use it when most of the team works in Spanish.

## nueva_app.wizard.help.option.mobile.android

- es: Solo Android cubre telefonos y tabletas Android. Conviene si el parque de dispositivos es Android.
- en: Android only covers Android phones and tablets. Use it when the device fleet is Android.

## nueva_app.wizard.help.option.mobile.ios

- es: Solo iOS cubre iPhone y iPad. Conviene si todos los usuarios usan dispositivos Apple.
- en: iOS only covers iPhone and iPad. Use it when all users have Apple devices.

## nueva_app.wizard.help.option.mobile.ios_android

- es: iOS y Android cubre los dos sistemas moviles principales. Conviene si no controlas que telefono tendra cada usuario.
- en: iOS and Android covers the two main mobile systems. Use it when you cannot control each user's phone.

## nueva_app.wizard.help.option.mobile.web_responsive

- es: Web responsive es una web que se adapta al movil. Conviene si quieres evitar instalar una app nativa al principio.
- en: Responsive web is a website that adapts to mobile. Use it when you want to avoid a native install at first.

## nueva_app.wizard.help.option.nombre.generico

- es: Nueva app es un nombre temporal. Conviene solo para empezar rapido y cambiarlo despues.
- en: New app is a temporary name. Use it only to start quickly and rename later.

## nueva_app.wizard.help.option.nombre.inferido

- es: Nombre inferido usa el objetivo para proponer un nombre. Conviene si aun no tienes marca definida.
- en: Inferred name uses the goal to suggest a name. Use it when you do not have a brand yet.

## nueva_app.wizard.help.option.objetivo.describir

- es: Describir objetivo verificable obliga a contar que debe pasar al final. Conviene cuando aun falta cerrar el resultado esperado.
- en: Describe a verifiable objective means saying what must be true at the end. Use it when the expected result is still unclear.

## nueva_app.wizard.help.option.plataformas.ambas

- es: Movil y PC cubre telefono y ordenador. Conviene cuando el trabajo empieza en oficina y sigue fuera.
- en: Mobile and desktop covers phone and computer. Use it when work starts in the office and continues outside.

## nueva_app.wizard.help.option.plataformas.movil

- es: Movil prioriza telefono. Conviene para consultas rapidas o trabajo en movimiento.
- en: Mobile prioritizes phones. Use it for quick checks or work on the move.

## nueva_app.wizard.help.option.plataformas.pc

- es: PC web prioriza pantallas grandes o navegador de ordenador. Conviene para tareas largas o administrativas.
- en: Desktop web prioritizes large screens or computer browsers. Use it for long or administrative tasks.

## nueva_app.wizard.help.option.storage.documental

- es: Documental guarda registros flexibles con formas distintas. Conviene para fichas variables, notas o configuraciones cambiantes.
- en: Document storage keeps flexible records with different shapes. Use it for variable files, notes, or changing settings.

- ejemplo es: Ejemplo: como carpetas con fichas que no siempre tienen los mismos apartados.
- example en: Example: like folders with records that do not always have the same sections.

## nueva_app.wizard.help.option.storage.mixta

- es: Mixta combina formas de guardado. Conviene cuando hay datos ordenados y documentos flexibles a la vez.
- en: Mixed combines storage styles. Use it when ordered data and flexible documents coexist.

- ejemplo es: Ejemplo: clientes en tablas y contratos como documentos adjuntos.
- example en: Example: customers in tables and contracts as attached documents.

## nueva_app.wizard.help.option.storage.relacional

- es: Relacional guarda datos ordenados en tablas relacionadas. Conviene para clientes, pedidos, citas o permisos.
- en: Relational stores ordered data in linked tables. Use it for customers, orders, appointments, or permissions.

- ejemplo es: Ejemplo: como hojas de calculo conectadas, una de clientes y otra de pedidos.
- example en: Example: like connected spreadsheets, one for customers and one for orders.

## nueva_app.wizard.help.option.storage.sin_preferencia
...
- en: API is a doorway for other programs to request or send data. Use it when you do not need a main screen.

- ejemplo es: Ejemplo: una app de reservas puede ofrecer una puerta para que la web y el movil consulten los mismos horarios.
- example en: Example: a booking app can offer one doorway so web and mobile read the same schedules.

## nueva_app.wizard.help.option.tipo_app.desktop

- es: Escritorio es una app instalada en un ordenador. Conviene para trabajo local u offline.
- en: Desktop is an app installed on a computer. Use it for local or offline work.

## nueva_app.wizard.help.option.tipo_app.mobile

- es: Movil es una app pensada para telefono. Conviene cuando el uso pasa fuera de una mesa.
- en: Mobile is an app designed for phones. Use it when work happens away from a desk.

## nueva_app.wizard.help.option.tipo_app.web
...
- es: Compartida con usuarios significa que varias personas entran a la misma app. Conviene si hay colaboracion, invitaciones o permisos.
- en: Shared with users means several people enter the same app. Use it for collaboration, invitations, or permissions.

## nueva_app.wizard.help.option.uso.personal

- es: Personal significa que la app es para una sola persona o uso privado. Conviene si no hay cuentas ni colaboracion.
- en: Personal means one person or private use. Use it when there are no accounts or collaboration.

## nueva_app.wizard.help.option.usuarios.clientes

- es: Clientes invitados son personas externas con acceso limitado. Conviene si usuarios de fuera consultan o aportan informacion.
- en: Invited clients are external people with limited access. Use it when outside users read or provide information.

## nueva_app.wizard.help.option.usuarios.equipo

- es: Equipo pequeno significa pocos usuarios con roles simples. Conviene para empezar con permisos claros sin complejidad.
- en: Small team means a few users with simple roles. Use it to start with clear permissions and little complexity.

## nueva_app.wizard.help.option.usuarios.internos

- es: Profesionales internos son usuarios de la organizacion. Conviene si la app es herramienta de trabajo interna.
- en: Internal professionals are users from the organization. Use it when the app is an internal work tool.
