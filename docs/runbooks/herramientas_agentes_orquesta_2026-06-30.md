...
```bash
scripts/bootstrap_agent_tooling.sh --repo "$(pwd)" --index
```

Efectos:

- ejecuta `codebase-memory-mcp update -y` si existe;
- ejecuta `codebase-memory-mcp install -y --ui=false`;
...
`codebase-memory-mcp install -y --ui=false` configuro Codex remoto en
`~/.codex/config.toml` y `~/.codex/AGENTS.md`. El worktree aislado
