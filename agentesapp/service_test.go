...
func TestBuildDetailCompactPromueveWorkQueueAbsorbidaAWorkConfirmed(t *testing.T) {
	now := time.Now().UTC()
	proyectoID := int64(46)
	tmp := t.TempDir()
	manifestPath := filepath.Join(tmp, "manifest.json")
	...
	writeJSON(statusPath, map[string]any{
		...
		"work_queue_reason":     "guidance durable escrita en inbox",
		"work_queue_updated_at": now.Add(-time.Minute).Format(time.RFC3339Nano),
	})
	writeJSON(heartbeatPath, map[string]any{
		"alive":        true,
	...
	if err := controlruntime.RecordWorkQueueFromMetadataJSON(string(metaJSON), controlruntime.WorkQueueRecordInput{
		...
		Action:     "continuar_trabajo",
		State:      "pending",
		Reason:     "guidance durable escrita en inbox",
	...
	store := &fakeStore{
		...
		canonicalHandles: []*db.RuntimeHandle{{ID: 10, Agente: "Codex2", ProyectoID: &proyectoID, Estado: "activo", Transporte: "tmux", HandleKind: "session", MetadataJSON: string(metaJSON)}},
		tasks:            []*db.Tarea{{ID: 41, Titulo: "Frente Codex", Estado: db.EstadoEnProgreso, ProyectoID: &proyectoID, Agente: ptr("Codex2"), Modulo: "cmd"}},
	}
	svc := NewService(store, nil)
...
func TestBuildDetailCompactPromueveSendInstructionDebilSiYaHayTrabajoVivo(t *testing.T) {
	now := time.Now().UTC()
	proyectoID := int64(451)
	heartbeatAt := now
	tmp := t.TempDir()
	manifestPath := filepath.Join(tmp, "manifest.json")
	statusPath := filepath.Join(tmp, "status.json")
	...
	store := &fakeStore{
		...
		runtimes:         []*db.RuntimeInstance{{ID: 91, Agente: "Codex2", ProyectoID: &proyectoID, LogicalState: "activo", ProcessState: "running", UpdatedAt: now}},
		canonicalHandles: []*db.RuntimeHandle{{ID: 92, Agente: "Codex2", ProyectoID: &proyectoID, Estado: "activo", Transporte: "tmux", HandleKind: "session", MetadataJSON: fmt.Sprintf(`{"worker_manifest_path":"%s","worker_status_path":"%s","worker_heartbeat_path":"%s"}`, manifestPath, statusPath, heartbeatPath)}},
		tasks:            []*db.Tarea{{ID: 141, Titulo: "Frente ya arrancado", Estado: db.EstadoEnProgreso, ProyectoID: &proyectoID, Agente: ptr("Codex2")}},
		orders: []*db.RuntimeOrder{{
			ID:            131,
			Agente:        "Codex2",
			ProyectoID:    &proyectoID,
			...
			Tipo:          "send_instruction",
			CreatedAt:     now,
			PayloadJSON:   `{"kind":"autonomia","accion":"continuar_trabajo","motivo":"post_remediation","verification_key":"vk-weak"}`,
			ResultadoJSON: `{"dispatch_state":"delivered","delivery_state":"delivered","receipt_source":"tmux_pane_activity"}`,
		}},
	}
	svc := NewService(store, nil)
	...
	if detail.Row.LastAutonomyState != "work_confirmed" || detail.Entity.LastAutonomyState != "work_confirmed" {
		t.Fatalf("deberia promover send_instruction debil cuando el worker ya tiene trabajo vivo: row=%+v entity=%+v", detail.Row, detail.Entity)
	}
}
...
func TestBuildDetailCompactUsaResumePayloadMailboxComoFallbackAutonomia(t *testing.T) {
	...
	store := &fakeStore{
		...
		canonicalHandles: []*db.RuntimeHandle{{ID: 7, Agente: "Codex1", ProyectoID: &proyectoID, Estado: "activo", Transporte: "tmux", HandleKind: "session"}},
	}
	svc := NewService(store, nil)

	detail, err := svc.BuildDetailCompact("Codex1")
	if err != nil {
		t.Fatalf("BuildDetailCompact: %v", err)
	}
	if detail == nil || detail.Entity == nil {
	...
	if detail.Entity.LastAutonomyAction != "supervisar_proyecto" || detail.Entity.LastAutonomySource != "resume_payload_mailbox" || detail.Entity.LastAutonomyState != "embedded" {
		t.Fatalf("deberia exponer fallback desde resume payload: %+v", detail.Entity)
	}
	if detail.Entity.LastAutonomyReason != "server_autobootstrap" || detail.Entity.LastAutonomyVerificationKey != "vk-1" {
		t.Fatalf("deberia exponer reason/verification desde resume payload: %+v", detail.Entity)
	}
}
...
func TestBuildDetailCompactPromueveResumePayloadAbsorbidoAWorkConfirmed(t *testing.T) {
	...
	store := &fakeStore{
		...
		sessions: []*db.Sesion{{
			...
			Activa:            true,
			Estado:            "activa",
			Inicio:            now.Add(-time.Hour),
			HeartbeatAt:       &heartbeatAt,
			ResumePayloadJSON: `{"mailbox":[{"kind":"autonomia","payload":{"accion":"supervisar_proyecto"}}]}`,
		}},
		tasks:            []*db.Tarea{{ID: 19, Titulo: "Supervisar backlog", Estado: db.EstadoEnProgreso, ProyectoID: &proyectoID, Agente: ptr("Codex1")}},
		runtimes:         []*db.RuntimeInstance{{ID: 21, Agente: "Codex1", ProyectoID: &proyectoID, LogicalState: "activo", ProcessState: "running", UpdatedAt: now}},
		canonicalHandles: []*db.RuntimeHandle{{ID: 31, Agente: "Codex1", ProyectoID: &proyectoID, Estado: "activo", Transporte: "tmux", HandleKind: "session", MetadataJSON: fmt.Sprintf(`{"worker_manifest_path":"%s","worker_status_path":"%s","worker_heartbeat_path":"%s"}`, manifestPath, statusPath, heartbeatPath)}},
	}
	svc := NewService(store, nil)
	...
	if detail.Row.LastAutonomyState != "work_confirmed" || detail.Entity.LastAutonomyState != "work_confirmed" {
		t.Fatalf("deberia promover resume payload absorbido a work_confirmed: row=%+v entity=%+v", detail.Row, detail.Entity)
	}
}
