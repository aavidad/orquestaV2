...
func TestTraceabilityRebuildSourceDispositions(t *testing.T) {
	...
	wantKinds := map[string]int{"task": 164, "bug": 156, "skill": 19}
	wantDecisionCounts := map[string]int{"accepted": 77, "historical": 243, "conditional": 19}
	...
	t.Logf("source_dispositions: entries=%d tasks=164 bugs=156 skills=19 coverage=100%% hashes=verified", len(entries))
}
