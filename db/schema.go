...
CREATE TABLE IF NOT EXISTS agentes (
    ...
    retirado     INTEGER NOT NULL DEFAULT 0,   -- 1 = retirado formalmente vía RetirarAgente
...
-- ─── Especificaciones de función para microprogramación dirigida ──────────
...
CREATE INDEX IF NOT EXISTS idx_especificaciones_funcion_tarea
ON especificaciones_funcion(tarea_id, id DESC);

CREATE INDEX IF NOT EXISTS idx_especificaciones_funcion_proyecto_estado
ON especificaciones_funcion(proyecto_id, estado, id DESC);
...
ON propuestas(estado, proyecto_id, id DESC);
CREATE INDEX IF NOT EXISTS idx_votos_agente_posicion_propuesta
ON votos(agente, posicion, propuesta_id);
...
ON supervisor_subagents(supervisor, session_id, updated_at DESC);

CREATE INDEX IF NOT EXISTS idx_supervisor_subagents_project_status
ON supervisor_subagents(proyecto_slug, supervisor, status, updated_at DESC);
...
ON supervisor_pipeline_states(supervisor, updated_at DESC);
...
ON asignaciones(agente, estado, proyecto_id, id DESC);

CREATE INDEX IF NOT EXISTS idx_asignaciones_proyecto_estado_agente_id
ON asignaciones(proyecto_id, estado, agente, id DESC);
...
ON presupuestos_sesion(sesion_id, checked_at DESC, id DESC);

CREATE INDEX IF NOT EXISTS idx_presupuestos_sesion_sesion_fuente_checked_id
ON presupuestos_sesion(sesion_id, budget_source, checked_at DESC, id DESC);
...
INSERT INTO config (clave, valor) VALUES
    ...
    ('connector_circuit_breaker_cooldown_seconds','300'),
	    ...
	    ('runtime_supervision_interval_seconds','60'),
	    ('runtime_supervision_batch_size','10'),
	    ('runtime_budget_background_observation_interval_seconds','120'),
...
UPDATE conectores
SET comando = 'claude-code'
WHERE slug = 'claude-code'
  AND TRIM(comando) = 'claude';

UPDATE conectores
SET args_json = '["--dangerously-skip-permissions","--permission-mode","bypassPermissions","--add-dir","{{working_dir}}","--session-id","{{session_uuid}}"]'
WHERE slug = 'claude-code'
...
WHERE slug = 'claude-code'
  AND (
    TRIM(args_json) = '["--dangerously-skip-permissions","--permission-mode","bypassPermissions"]'
    OR TRIM(args_json) = '["--dangerously-skip-permissions","--permission-mode","bypassPermissions","--bare","--add-dir","{{working_dir}}","--session-id","{{session_uuid}}"]'
  );

UPDATE conectores
SET metadata_json = '{"familia":"anthropic","reanudable":true,"launch_prompt_positional":true,"can_send_input":false}'
WHERE slug = 'claude-code'
  AND (
    TRIM(metadata_json) = ''
    OR TRIM(metadata_json) = '{}'
    OR TRIM(metadata_json) = '{"familia":"anthropic","reanudable":true}'
  );

UPDATE conectores
SET comando = 'codex',
    args_json = '[]',
    env_json = '{"PATH":"{{orquesta_bin_dir}}:{{host_path}}"}'
WHERE slug = 'codex-cli'
  AND (
    TRIM(comando) = '{{home}}/Trabajo/codex-perfiles/bin/codex-perfil'
    OR TRIM(comando) = 'codex-perfil'
  );
