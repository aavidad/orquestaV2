...
func FusionarAgentes(origen, destino string) (*FusionAgentesResultado, error) {
	...
	operaciones := []struct {
		...
		{nombre: "runtime_transcript.agente", query: `UPDATE runtime_transcript SET agente = ? WHERE agente = ?`},
		...
		{nombre: "agente_scores_locales.agente", query: `UPDATE agente_scores_locales SET agente = ? WHERE agente = ?`},
