...
assert len(plan["worktrees"]) == 6
assert len({item["path"] for item in plan["worktrees"]}) == 6
...
for case in cases:
    ...
    for pair_index, pair in enumerate(manifest["pairs"][:case["complete_pairs"]]):
        ...
        for mode in ("control", "treatment"):
            ...
            candidate = f"candidate-{pair_index}-{mode}"
            ...
            arms.append({"mode": mode, "candidate_ref": candidate, "status": "completed", "config_sha256": domain.config_hash(manifest, pair), "usage_by_role": usage, "metrics": {"duration_ms": 100000 if mode == "control" else 100000 * case["treatment_time_ratio_bps"] // 10000, "tool_calls": 20, "repeated_reads": 2, "reworks": 1, "failed_attempts": 0, "write_set_conflicts": 0, "human_interventions": 0}, "false_green": False, "causal_loss": False, "unauthorized_external_effects": False, "external_effects": 0, "severe_defects": 0, "tests": [{"command": pair["required_tests"][0], "executed": True, "passed": case["quality_ok"], "evidence": proof(f"test-{pair_index}-{mode}")}], "handoffs": handoffs, "independent_review": {"completed": True, "accepted": case["quality_ok"], "candidate_ref": candidate, "evidence": [proof(f"review-{pair_index}-{mode}")]}, "diff": proof(f"diff-{pair_index}-{mode}"), "artifacts": [proof(f"artifact-{pair_index}-{mode}")], "receipts": [proof(f"receipt-{pair_index}-{mode}")]})
...
test "$(find "$TMP/runtime/state/receipts" -type f | wc -l)" -eq 6
