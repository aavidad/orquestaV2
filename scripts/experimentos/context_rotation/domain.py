...
def validate_manifest(manifest: dict) -> list[str]:
    ...
    check(_config(reviewer) and reviewer.get("blinded") is True and reviewer.get("fresh_context") is True and SHA.fullmatch(reviewer.get("blinding_key_sha256", "")) is not None and bool(reviewer.get("reproduction_test")), "reviewer")
    ...
    check(isinstance(pairs, list) and 3 <= len(pairs) <= 20, "pair_count")
...
def plan(manifest: dict) -> dict:
    ...
    for pair in manifest["pairs"]:
        ...
        for mode in ("control", "treatment"):
            ...
            worktrees.append({"operation_ref": operation, "pair_ref": pair["pair_ref"], "mode": mode, "path": os.path.join(manifest["temporary_root"], "worktrees", manifest["experiment_ref"], pair["pair_ref"], mode), "commit": manifest["baseline_commit"], "config_sha256": cfg})
    return {"schema_version": "orquesta.context_rotation.plan.v1", "experiment_ref": manifest["experiment_ref"], "manifest_sha256": digest(manifest), "dry_run": True, "worktrees": worktrees}
...
def stage_request(manifest: dict, pair_ref: str, mode: str, stage_ref: str) -> tuple[dict, str | None]:
    ...
    request = {"schema_version": "orquesta.context_rotation.stage_request.v1", "experiment_ref": manifest["experiment_ref"], "pair_ref": pair_ref, "mode": mode, "stage_ref": stage_ref, "fresh_worker": mode == "treatment", "compact_handoff_only": True, "baseline_commit": manifest["baseline_commit"], "execution": manifest["execution"], "acceptance_criteria": pair["acceptance_criteria"], "required_tests": pair["required_tests"]}
    if mode == "control":
        if stage_ref != "continuous-session":
            raise ValueError("control stage must be continuous-session")
        request.update(objective=pair["objective"], cause="paired-control", write_set=pair["write_set"], context_refs=pair["context_refs"])
        return request, None
...
def _complete(manifest: dict, pair: dict, arm: dict) -> bool:
    ...
    if arm.get("status") != "completed" or arm.get("config_sha256") != config_hash(manifest, pair) or arm.get("independent_review", {}).get("completed") is not True or arm["independent_review"].get("candidate_ref") != arm.get("candidate_ref"):
        return False
    if not REF.fullmatch(arm.get("candidate_ref", "")) or not _evidence(arm.get("diff")) or not _evidence_list(arm.get("artifacts")) or not _evidence_list(arm.get("receipts")) or not _evidence_list(arm["independent_review"].get("evidence")) or usage is None:
        return False
    counters = [metrics.get(k) for k in ("duration_ms", "tool_calls", "repeated_reads", "reworks", "failed_attempts", "write_set_conflicts", "human_interventions")]
    if any(not isinstance(v, int) or v < 0 for v in counters):
        return False
...
def _quality(pair: dict, control: dict, treatment: dict) -> bool:
    ...
    for arm in (control, treatment):
        passed = {test.get("command") for test in arm.get("tests", []) if test.get("executed") and test.get("passed") and _evidence(test.get("evidence"))}
...
def _handoffs(pair: dict, treatment: dict) -> bool:
    ...
    for handoff in treatment.get("handoffs", []):
...
def evaluate(manifest: dict, dataset: dict) -> dict:
    ...
    token_savings, cost_savings, control_times, treatment_times = [], [], [], []
    ...
    for pair in manifest["pairs"]:
        ...
        rows.append({"pair_ref": pair["pair_ref"], "control_tokens": control_use[0], "treatment_tokens": treatment_use[0], "token_reduction_bps": token_bps, "control_cost_micros": control_use[1], "treatment_cost_micros": treatment_use[1], "cost_reduction_bps": cost_bps})
        token_savings.append(token_bps); cost_savings.append(cost_bps); control_times.append(control["metrics"]["duration_ms"]); treatment_times.append(treatment["metrics"]["duration_ms"])
    ...
    median_time = int(statistics.median(treatment_times) * 10000 // statistics.median(control_times)) if len(rows) >= 3 else 0
    if len(rows) < 3: incomplete.append("fewer_than_three_complete_pairs")
    ...
    if improved < 2: failed.append("fewer_than_two_pairs_improved")
    ...
    return {"schema_version": "orquesta.context_rotation.decision.v1", "experiment_ref": manifest["experiment_ref"], "manifest_sha256": expected_hash, "dataset_sha256": digest(dataset), "decision": verdict, "reasons": sorted(set(reasons)), "pair_metrics": rows, "median_token_reduction_bps": median_tokens, "median_cost_reduction_bps": median_cost, "median_time_ratio_bps": median_time, "improved_pairs": improved, "thresholds": {"minimum_pairs": 3, "minimum_token_reduction_bps": 2000, "minimum_cost_reduction_bps": 2000, "maximum_median_time_ratio_bps": 11000, "minimum_improved_pairs": 2}}
...
def blind(manifest: dict, dataset: dict, pair_ref: str, mode: str, key: bytes) -> tuple[dict, dict]:
    ...
    package = {"schema_version": "orquesta.context_rotation.blind_review.v1", "experiment_ref": manifest["experiment_ref"], "pair_ref": pair_ref, "candidate_ref": candidate, "objective": pair["objective"], "acceptance_criteria": pair["acceptance_criteria"], "required_tests": pair["required_tests"], "diff": arm["diff"], "artifacts": arm["artifacts"], "test_evidence": [test["evidence"] for test in arm["tests"]]}
    mapping = {"schema_version": "orquesta.context_rotation.blind_mapping.v1", "experiment_ref": manifest["experiment_ref"], "pair_ref": pair_ref, "candidate_ref": candidate, "mode": mode, "package_sha256": digest(package)}
