...
execute_codex="${ORQUESTA_CODEX_REAL_REQUIRED_TEST_RUNNER_EXECUTE_CODEX:-0}"
if [[ "$execute_codex" == "1" ]]; then
  if [[ "${ORQUESTA_CODEX_REAL_REQUIRED_TEST_RUNNER_CODEX_EXECUTION_CONFIRMED:-0}" != "1" ]]; then
    echo "segunda confirmacion requerida para ejecutar Codex real: exporta ORQUESTA_CODEX_REAL_REQUIRED_TEST_RUNNER_CODEX_EXECUTION_CONFIRMED=1" >&2
    exit 2
  fi
  ...
  if [[ "${ORQUESTA_CODEX_REAL_REQUIRED_TEST_RUNNER_NO_CODEX_EXECUTION_CONFIRMED:-0}" != "1" ]]; then
    echo "segunda confirmacion requerida para modo sin Codex: exporta ORQUESTA_CODEX_REAL_REQUIRED_TEST_RUNNER_NO_CODEX_EXECUTION_CONFIRMED=1" >&2
    exit 2
  fi
fi
...
if [[ "$execute_codex" == "1" ]]; then
  export ORQUESTA_CODEX_REAL_REQUIRED_TEST_RUNNER_E2E_SMOKE=1
  export ORQUESTA_CODEX_REAL_REQUIRED_TEST_RUNNER_CODEX_EXECUTION_CONFIRMED=1
  unset ORQUESTA_CODEX_REAL_REQUIRED_TEST_RUNNER_NO_CODEX_EXECUTION_CONFIRMED
else
  export ORQUESTA_CODEX_REAL_REQUIRED_TEST_RUNNER_NO_CODEX_EXECUTION_CONFIRMED=1
  unset ORQUESTA_CODEX_REAL_REQUIRED_TEST_RUNNER_E2E_SMOKE
  unset ORQUESTA_CODEX_REAL_REQUIRED_TEST_RUNNER_CODEX_EXECUTION_CONFIRMED
fi
...
if [[ -z "${ORQUESTA_CODEX_SMOKE_TIMEOUT_MS:-}" ]]; then
  export ORQUESTA_CODEX_SMOKE_TIMEOUT_MS="180000"
fi
...
if [[ "$execute_codex" == "1" ]]; then
  echo "codex_executed=true (opt-in acotado: un agente, workdir temporal, OPES off)"
  test_regex='^TestCodexStackRealRequiredTestRunnerEndToEndOptInV0$'
  timeout_value="${ORQUESTA_CODEX_REAL_REQUIRED_TEST_RUNNER_TIMEOUT:-720s}"
else
  echo "codex_executed=false (el test valida config y snapshot vacio; no llama LaunchV0)"
  test_regex='^TestCodexStackRealRequiredTestRunnerOptInV0$'
  timeout_value="${ORQUESTA_CODEX_REAL_REQUIRED_TEST_RUNNER_TIMEOUT:-240s}"
fi
